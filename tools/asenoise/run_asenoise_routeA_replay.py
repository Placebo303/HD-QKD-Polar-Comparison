...
SECURITY_TOOLS = REPO_ROOT / "tools" / "security_reports"
PIPELINE_CURRENT = REPO_ROOT / "pipelines" / "current"
...
def _run_stage_c(*, candidate_stage_dir: Path, stage1_dir: Path, stage2_dir: Path, log_path: Path) -> int:
    cmds = [
        [
            sys.executable,
            str(SECURITY_TOOLS / "longrun_build_finite_key_audit_table.py"),
            "--input-dirs",
        ...
        [
            sys.executable,
            str(SECURITY_TOOLS / "round2_build_actual_ir_finite_key_shadow.py"),
            "--output-dir",
            str(stage2_dir),
            "--overwrite",
        ],
        [
            sys.executable,
            str(SECURITY_TOOLS / "round2_build_beta_baseline_shadow.py"),
            "--output-dir",
            str(stage2_dir),
            "--overwrite",
        ],
        [
            sys.executable,
            str(SECURITY_TOOLS / "longrun_build_security_master_table.py"),
            "--actual-ir-dir",
            str(stage2_dir),
            "--beta-baseline-dir",
            str(stage2_dir),
...
def replay(args: argparse.Namespace) -> int:
    ...
    for dataset_dir in selected:
        ...
        rc = _run(
            [
                sys.executable,
                str(PIPELINE_CURRENT / "routeA_run_formal_replay_shards.py"),
                "--candidate-dir",
                str(candidate_stage_dir),
                "--replay-index-dir",
