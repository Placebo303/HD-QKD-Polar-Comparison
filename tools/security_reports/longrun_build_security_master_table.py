...
def main() -> int:
    ...
    proxy_delta = pd.to_numeric(merged["PIE_practical"], errors="coerce") - pd.to_numeric(merged["PIE_main"], errors="coerce")
    beta_delta = pd.to_numeric(merged["PIE_secure_beta_baseline"], errors="coerce") - pd.to_numeric(merged["PIE_main"], errors="coerce")
    perf_best = merged.loc[pd.to_numeric(merged["SKR_measured_bps"], errors="coerce").idxmax()]
    main_best = merged.loc[pd.to_numeric(merged["SKR_main_bps"], errors="coerce").idxmax()]
    lines = [
        ...
        "default_main_source: PIE_reconciled_net, SKR_reconciled_net_bps",
        ...
        f"mean_PIE_gap_performance_minus_reconciled: {proxy_delta.mean()}",
        ...
        f"reconciled_main_best_point: d={int(main_best['dimension'])}, bw={int(main_best['bin_width_ps'])}",
        f"positive_reconciled_rows: {int((pd.to_numeric(merged['SKR_main_bps'], errors='coerce') > 0).sum())}",
        f"reconciliation_table_ready_tag: {ready_tag}",
