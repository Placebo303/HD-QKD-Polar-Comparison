...
RUN_ID="binary_ldpc_v4_10db_transfer_qualification_v2"; STRATA=source.STRATA; FRAME_COUNT=128; SEED_BITS=2623
...
def _plan_root_sets(doc):
 ...
 return roots,seeds
def _prior_sets(s,pred=None,current_plan_sha256=None):
 ...
 for binding in _existing_real_plans(current_plan_sha256):
 ...
 return roots,seeds
def _roots(s,count,pred=None,current_plan_sha256=None):
 old,oldids=_prior_sets(s,pred,current_plan_sha256);out={};ids=set()
 for st in STRATA:
 ...
 return out
def _validate_roots(roots,s,count,pred=None,current_plan_sha256=None):
 old,oldids=_prior_sets(s,pred,current_plan_sha256);seen=set();ids=set()
...
def _validate(p,l,private):
 ...
 actual=l if private else source.build_source_lock()
 if p["source_lock_sha256"]!=actual["lock_sha256"] or l!=actual:raise ValueError("source lock")
 _validate_roots(p["roots"],s,count,pred,p["plan_sha256"])
...
def prepare_plan(output_dir,synthetic_dir=PREREQUISITE):
 ...
 p,l,_=_plan(synthetic_dir);Path(output_dir).mkdir(parents=True);_jsonx(Path(output_dir)/ARTIFACTS[0],p);_jsonx(Path(output_dir)/ARTIFACTS[1],l);_validate(p,l,False);return p
def _prepare_test_plan(output_dir,synthetic_dir,source_lock,count=2):
 if Path(output_dir).exists():raise FileExistsError("fresh output")
 p,l,_=_plan(synthetic_dir,True,count,source_lock);Path(output_dir).mkdir(parents=True);_jsonx(Path(output_dir)/ARTIFACTS[0],p);_jsonx(Path(output_dir)/ARTIFACTS[1],l);_validate(p,l,True);return p
