"""Read-only verifier for the immutable 16 dB transfer package."""
from __future__ import annotations
import argparse,json
from pathlib import Path
from . import run_ldpc_v4_16db_transfer_qualification as lane
...
def verify_output(output_dir,*,_private_test_only=False):
 ...
 if set(run)!={"schema","run_id","run_status","stop_reason","plan_sha256","outcome_count","artifact_index","decoder_reexecution","manifest_sha256"} or run.get("manifest_sha256")!=lane._sha(lane._compact({k:v for k,v in run.items() if k!="manifest_sha256"})) or run.get("schema")!="binary_ldpc_v4_16db_transfer_run_manifest_v1" or run.get("run_id")!=lane.RUN_ID or run.get("run_status") not in {"completed","failed"} or run.get("plan_sha256")!=p["plan_sha256"] or run.get("outcome_count")!=len(rows) or run.get("artifact_index")!=idx or run.get("decoder_reexecution") is not False or (run["run_status"]=="completed" and run["stop_reason"]) or (run["run_status"]=="failed" and not run["stop_reason"]):raise ValueError("run manifest")
 gates={s:syn._gate(groups[s],p["frame_count_per_stratum"],p["gate"]["verified_success_floor"]) for s in lane.STRATA}
 if set(rep)!={"schema","run_id","run_status","stop_reason","plan_sha256","run_manifest_sha256","promotion_gates","promoted","decoder_reexecution","source_relocation","report_sha256"} or rep.get("report_sha256")!=lane._sha(lane._compact({k:v for k,v in rep.items() if k!="report_sha256"})) or rep.get("schema")!="binary_ldpc_v4_16db_transfer_report_v1" or rep.get("run_id")!=lane.RUN_ID or rep.get("run_status")!=run["run_status"] or rep.get("stop_reason")!=run["stop_reason"] or rep.get("plan_sha256")!=p["plan_sha256"] or rep.get("run_manifest_sha256")!=lane._sha((out/lane.ARTIFACTS[7]).read_bytes()) or rep.get("promotion_gates")!=gates or rep.get("promoted") != (run.get("run_status")=="completed" and all(x["promoted"] for x in gates.values())) or rep.get("decoder_reexecution") is not False or rep.get("source_relocation") is not True:raise ValueError("report")
