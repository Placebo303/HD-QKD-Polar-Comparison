"""Immutable 16 dB transfer qualification; production entry points have no test switches."""
from __future__ import annotations
import argparse, copy, csv, hashlib, json, math, secrets, time
...
from ..formal_ir import ldpc_v4_16db_source as source
from ..formal_ir.ldpc_v4 import METHOD, run_ldpc_formal_v4
from ..formal_ir.shared import canonical_event, seed_record

RUN_ID="binary_ldpc_v4_16db_transfer_qualification_v1"; STRATA=source.STRATA; FRAME_COUNT=128; SEED_BITS=2623
...
def _source_hashes():
 root=Path(__file__).resolve().parents[1]; paths={"runner":"cli/run_ldpc_v4_16db_transfer_qualification.py","verifier":"cli/verify_ldpc_v4_16db_transfer_qualification.py","source":"formal_ir/ldpc_v4_16db_source.py","method":"formal_ir/ldpc_v4.py","codebook":"formal_ir/codebook_v4.py","channel":"formal_ir/ldpc_v4_channel.py","shared":"formal_ir/shared.py","synthetic_runner":"cli/run_ldpc_v4_synthetic_qualification.py","synthetic_verifier":"cli/verify_ldpc_v4_synthetic_qualification.py"};return {k:_sha((root/v).read_bytes()) for k,v in paths.items()}
def _existing_real_plans(excluding_plan_sha256=None):
 out=[]
 for p in PREREQUISITE.parent.glob("*/pre_run_plan.json"):
  try:
   doc=json.loads(p.read_bytes())
   if doc.get("run_id")==RUN_ID and doc.get("plan_sha256")!=excluding_plan_sha256:out.append({"path":str(p.resolve()),"sha256":_sha(p.read_bytes())})
...
def _prior_sets(s):
 ...
 return roots,seeds
def _roots(s,count):
 old,oldids=_prior_sets(s);out={};ids=set()
 for st in STRATA:
 ...
 return out
def _validate_roots(roots,s,count):
 old,oldids=_prior_sets(s);seen=set();ids=set()
...
def _validate(p,l,private):
 ...
 keys={"schema","run_id","method_id","frame_count_per_stratum","strata","gate","caps","backend_requirement","failure_finalizer","prior_real_plan_bindings","execution_order","source_lock_sha256","synthetic","roots","source_sha256","_test_only","plan_sha256"}
