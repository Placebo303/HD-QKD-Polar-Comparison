"""V53P0 nested rate-adaptive L2 heldout confirm (conditional HARQ) Δm=8.

Frozen V52P0 protocol (formal-ir-v53-rate-adaptive-l2-heldout-confirm):
...
- 45 fresh held-out blocks 394001-015/101-115/201-215 deterministic 4-frame 1024 pairs per block.
...
Accepted plan SHA: 93c12fa5a8524eb5a8a52d071f135c653c746ebaf
"""
...
CYCLE_ID = "V53P0"
CHANGE_ID = "formal-ir-v53-rate-adaptive-l2-heldout-confirm"
ACCEPTED_PLAN_SHA = "93c12fa5a8524eb5a8a52d071f135c653c746ebaf"
BRANCH_REF = "origin/formal-ir-mainline"
EXECUTION_SCOPE = "v53_heldout_confirm_45_blocks_paired_exactly_once"
...
PLANNED_L1 = 45
PLANNED_L2_BASE = 45
PLANNED_L2_RESCUE_MAX = 45
PLANNED_L2_MAX = 90
HARD_CALL_CAP = 135  # total L1+L2
...
SAMPLING_MODE = "deterministic_four_consecutive_frames_heldout_fresh_v53"
...
_V53_WINDOW_DEFS = [
    ("1M", 394001, 78, 81, [1678, 1679, 1680, 1681]),
    ("1M", 394002, 134, 137, [1734, 1735, 1736, 1737]),
    ("1M", 394003, 156, 159, [1756, 1757, 1758, 1759]),
    ("1M", 394004, 178, 181, [1778, 1779, 1780, 1781]),
    ("1M", 394005, 193, 196, [1793, 1794, 1795, 1796]),
    ("1M", 394006, 215, 218, [1815, 1816, 1817, 1818]),
    ("1M", 394007, 237, 240, [1837, 1838, 1839, 1840]),
    ("1M", 394008, 259, 262, [1859, 1860, 1861, 1862]),
    ("1M", 394009, 274, 277, [1874, 1875, 1876, 1877]),
    ("1M", 394010, 296, 299, [1896, 1897, 1898, 1899]),
    ("1M", 394011, 318, 321, [1918, 1919, 1920, 1921]),
    ("1M", 394012, 333, 336, [1933, 1934, 1935, 1936]),
    ("1M", 394013, 355, 358, [1955, 1956, 1957, 1958]),
    ("1M", 394014, 377, 380, [1977, 1978, 1979, 1980]),
    ("1M", 394015, 392, 395, [1992, 1993, 1994, 1995]),
    ("1p5M", 394101, 35, 38, [2248, 2249, 2250, 2251]),
    ("1p5M", 394102, 103, 106, [2316, 2317, 2318, 2319]),
    ("1p5M", 394103, 150, 153, [2363, 2364, 2365, 2366]),
    ("1p5M", 394104, 204, 207, [2417, 2418, 2419, 2420]),
    ("1p5M", 394105, 230, 233, [2443, 2444, 2445, 2446]),
    ("1p5M", 394106, 263, 266, [2476, 2477, 2478, 2479]),
    ("1p5M", 394107, 296, 299, [2509, 2510, 2511, 2512]),
    ("1p5M", 394108, 329, 332, [2542, 2543, 2544, 2545]),
    ("1p5M", 394109, 362, 365, [2575, 2576, 2577, 2578]),
    ("1p5M", 394110, 388, 391, [2601, 2602, 2603, 2604]),
    ("1p5M", 394111, 421, 424, [2634, 2635, 2636, 2637]),
    ("1p5M", 394112, 454, 457, [2667, 2668, 2669, 2670]),
    ("1p5M", 394113, 487, 490, [2700, 2701, 2702, 2703]),
    ("1p5M", 394114, 520, 523, [2733, 2734, 2735, 2736]),
    ("1p5M", 394115, 546, 549, [2759, 2760, 2761, 2762]),
    ("2M", 394201, 37, 40, [2953, 2954, 2955, 2956]),
    ("2M", 394202, 96, 99, [3012, 3013, 3014, 3015]),
    ("2M", 394203, 162, 165, [3078, 3079, 3080, 3081]),
    ("2M", 394204, 236, 239, [3152, 3153, 3154, 3155]),
    ("2M", 394205, 281, 284, [3197, 3198, 3199, 3200]),
    ("2M", 394206, 326, 329, [3242, 3243, 3244, 3245]),
    ("2M", 394207, 372, 375, [3288, 3289, 3290, 3291]),
    ("2M", 394208, 410, 413, [3326, 3327, 3328, 3329]),
    ("2M", 394209, 455, 458, [3371, 3372, 3373, 3374]),
    ("2M", 394210, 501, 504, [3417, 3418, 3419, 3420]),
    ("2M", 394211, 546, 549, [3462, 3463, 3464, 3465]),
    ("2M", 394212, 591, 594, [3507, 3508, 3509, 3510]),
    ("2M", 394213, 637, 640, [3553, 3554, 3555, 3556]),
    ("2M", 394214, 682, 685, [3598, 3599, 3600, 3601]),
    ("2M", 394215, 721, 724, [3637, 3638, 3639, 3640]),
]
for _src, _bid, _s, _e, _fids in _V53_WINDOW_DEFS:
    BLOCK_WINDOWS[_bid] = {
        "source": _src,
...
OUTPUT_ROOT = (
    REPO_ROOT / "comparison_bench/outputs_comparison/formal_ir_methods/v53_rate_adaptive_l2_heldout_confirm/run_01"
)
FORBIDDEN_WINNER_NPZ_NAME = "v38_winning_matrices.npz"

SCOPED_TRACKED_PATHS: tuple[str, ...] = (
    "comparison_bench/src/comparison_bench/formal_ir/v53_rate_adaptive_l2_heldout_confirm.py",
    "scripts/execute_v53_heldout_confirm.py",
...
MASTER_STOP_RULE = (
    "每块 L1 1 + base L2 1 (兼 old baseline, 同一次确定性译码不重复) + 条件 rescue ≤1；总 45 L1+45 base+≤45 rescue=90–135 硬帽135 L2≤90。"
)

TERMINAL_EVIDENCE_INVALID = "V53_EVIDENCE_INVALID"
TERMINAL_V53_HELDOUT_CONFIRM_PASS = "V53_HELDOUT_CONFIRM_PASS"
TERMINAL_V53_HELDOUT_CONFIRM_FAIL = "V53_HELDOUT_CONFIRM_FAIL"
TERMINAL_NESTED_RESCUE_COMPLETE = "V53_HELDOUT_CONFIRM_PASS"
ALL_TERMINALS = frozenset({TERMINAL_EVIDENCE_INVALID, TERMINAL_V53_HELDOUT_CONFIRM_PASS, TERMINAL_V53_HELDOUT_CONFIRM_FAIL, TERMINAL_NESTED_RESCUE_COMPLETE})
...
CLAIM_BOUNDARY: tuple[str, ...] = (
    "results support ONLY nested incremental L2 rescue Δ8 on frozen Lane C support/prior/MET n=1024 m2 184/190/192 GF32 poly37 decoder 90/1.0 leak 1064/1094/1104 base 1104/1134/1144 joint conditional only rescue frames +40 avg=base+40*N_attempt/15 total 90-135 hard cap 135 old vs pass1 same decode L2-only 2^-64",
    ...
    "not threshold/SKR/formal qualification/promotion; paired descriptive only Δexact = final_V53 - old (old=base_exact) McNemar b/c descriptive four-way G3' undetected==0",
)

STATISTICS_NOTE = (
    "Descriptive only; sample is 45 blocks paired old vs V53 (45 L1 +45 base +≤45 rescue =90-135 L2 45-90 records). "
...
def validate_seed_registry(seeds: Optional[dict[str, list[int]]] = None) -> tuple[bool, str]:
    ...
    # also check overlap with V52 new blocks (393xxx) - they are subset of forbidden? Not yet, but check explicitly
    v52_overlap = set(all_seeds) & {s for lst in [[393001,393002,393003,393004,393005],[393101,393102,393103,393104,393105],[393201,393202,393203,393204,393205]] for s in lst}
    if v52_overlap:
        return False, f"new seeds overlap V52 15: {sorted(v52_overlap)}"
    ...
    if seeds is None and flat_actual != flat_expected:
        return False, "new seeds must be exactly frozen 394001-015/101-115/201-215"
    for src in SOURCE_ORDER:
        seeds_sorted = sorted(reg[src])
        if seeds_sorted != reg[src]:
            return False, f"seeds for {src} must be sorted ascending"
        # no consecutive requirement for 15 dispersed; only check monotonic spacing and gap
    # per-source frame_ids zero overlap with V48/V50/V51/V52
...
def reconstruct_v52_matrices(*args, **kwargs):
    return reconstruct_v53_matrices(*args, **kwargs)
...
def nested_rescue_preflight(
    ...
    field = field or GF2mField.create(DIMENSION)
    if matrices is None:
        matrices = reconstruct_v53_matrices(field=field)
    ...
    for src in SOURCE_ORDER:
        ...
        bseed = {"1M":394001,"1p5M":394101,"2M":394201}[src]
...
class CallAccounting:
    ...
    def register_start(self, layer: str = "total") -> None:
        ...
        if layer == "l2" and self.started_l2 >= PLANNED_L2_MAX:
            raise IntegrityFailure("J10", "l2 cap 90 reached")
        if layer == "rescue" and self.started_rescue >= PLANNED_L2_RESCUE_MAX:
            raise IntegrityFailure("J10", "rescue cap 45 reached")
        self.started += 1
        if layer == "l1":
            self.started_l1 += 1
            self.started_l2 += 0
    ...
    def validate_executed(self) -> list[tuple[str, str]]:
        ...
        if self.completed < 90 or self.completed > 135:
            failures.append(("J10", f"total {self.completed} not in 90-135"))
        return failures
...
def run_v53_diagnostic(
    ...
    # additive output root created empty after all preflight passed, before first decoder call
    ...
    verify_base_count = 0
    undetected_count = 0
    per_source_counts: dict[str, dict[str,int]] = {s: {"first":0,"rescued":0,"final":0,"old":0,"rescue_attempted":0, "base_exact":0, "verify_base":0, "undetected":0} for s in SOURCE_ORDER}
    ...
    try:
        ...
        for source in SOURCE_ORDER:
            for bseed in NEW_BLOCK_SEEDS[source]:
                ...
                if exact_base:
                    base_exact_full+=1
                    per_source_counts[source]["base_exact"]+=1
                if verify_base:
                    verify_base_count+=1
                    per_source_counts[source]["verify_base"]+=1
                # undetected on base: syndrome_ok && tag_ok && not exact
                if verify_base and not exact_base:
                    undetected_count+=1
                    per_source_counts[source]["undetected"]+=1
                ...
                if verify_base:
                    ...
                    if verify2 and not exact_rescue:
                        undetected_count+=1
                        per_source_counts[source]["undetected"]+=1
        ...
        # minimal interrupt durability: only completed records + notice, no aggregate
        records_sorted = sorted(records, key=lambda r: r["call_id"])
        with (output_root / "v53_records.json").open("w", encoding="utf-8") as f:
            json.dump(records_sorted, f, indent=2, ensure_ascii=False)
        with (output_root / "v53_records.csv").open("w", newline="", encoding="utf-8") as f:
        ...
        notice = {
            ...
            "planned": {"total": f"{PLANNED_L1+PLANNED_L2_BASE}-{HARD_CALL_CAP}", "l1": PLANNED_L1, "l2_base": PLANNED_L2_BASE, "l2_rescue_max": PLANNED_L2_RESCUE_MAX, "l2_max": PLANNED_L2_MAX, "hard_cap": HARD_CALL_CAP},
            "started": {"total": accounting.started, "l1": accounting.started_l1, "l2": accounting.started_l2, "base": accounting.started_base, "rescue": accounting.started_rescue},
            "completed": {"total": accounting.completed, "l1": accounting.completed_l1, "l2": accounting.completed_l2, "base": accounting.completed_base, "rescue": accounting.completed_rescue},
            "records_completed": len(records_sorted),
        }
        with (output_root / "v53_interrupted_notice.json").open("w", encoding="utf-8") as f:
            json.dump(notice, f, indent=2, ensure_ascii=False)
        raise
    # validate accounting 90-135
    ...
    for src in SOURCE_ORDER:
        ...
        avg_leak_per_source[src] = float(leak_for(src) + 40 * n_resc_src / 15)
    ...
    # prepare json records etc (output_root already created empty before first decoder call)
    ...
    with (output_root / "v53_records.csv").open("w", newline="", encoding="utf-8") as f:
        ...
        for r in records_sorted:
            w.writerow({k: (",".join(map(str, v)) if k=="frame_ids" and isinstance(v, list) else v) for k,v in r.items()})
    total_disclosed_bits = int(total_leak_sum)
    avg_disclosure_per_attempted_frame = float(avg_overall / 1024) if total_blocks else 0.0
    disclosure_per_final_exact_block = float(total_disclosed_bits / final_success) if final_success else None
    # four-way reclassified counts
    reclassified_counts = {"exact":0, "detected_verification_failure":0, "decoder_non_syndrome_failure":0, "undetected_accepted_wrong": undetected_count}
    for r in records:
        rc = r.get("reclassified")
        if rc in reclassified_counts and rc != "undetected_accepted_wrong":
            reclassified_counts[rc]+=1
    # ensure exact counts include all exact records
    # terminal gate 35/45 &10/15 &undetected==0
    per_source_gate = all(per_source_counts[s]["final"] >= 10 for s in SOURCE_ORDER)
    overall_gate = final_success >= 35
    gate_pass = bool(overall_gate and per_source_gate and undetected_count==0)
    terminal = TERMINAL_V53_HELDOUT_CONFIRM_PASS if gate_pass else TERMINAL_V53_HELDOUT_CONFIRM_FAIL
    # if integrity would fail it would have raised earlier; here we return PASS/FAIL
    summary = {
        "cycle_id": CYCLE_ID,
        "change_id": CHANGE_ID,
        ...
        "leakage": {
            ...
            "formula": "per_source_avg[s]=leak_base[s]+40*N_rescue_attempted[s]/15 overall_avg=(Σ leak_base+40*N_total)/45",
            "n_rescue_attempted": n_rescue_attempted,
            "total_disclosed_bits": total_disclosed_bits,
            "avg_disclosure_per_attempted_frame": avg_disclosure_per_attempted_frame,
            "disclosure_per_final_exact_block": disclosure_per_final_exact_block,
        },
        "counts": {
            "base_exact_full": base_exact_full,
            "verify_base": verify_base_count,
            ...
            "undetected_accepted_wrong": undetected_count,
    ...
    with (output_root / "v53_summary.json").open("w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)
    return {"output_root": output_root, "terminal_state": terminal, "summary": summary, "records": records_sorted}



def run_v52_diagnostic(*args, **kwargs):
    return run_v53_diagnostic(*args, **kwargs)