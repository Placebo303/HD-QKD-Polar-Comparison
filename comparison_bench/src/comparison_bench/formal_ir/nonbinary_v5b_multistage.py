"""Public-input-only three-level incremental-redundancy NBLDPC v5b core.

Route B reuses the frozen Route-A policy structure (ir56 candidate, ir48
control) and the identical three-level warm state machine, bound to the
NBLDPC5B mother code (:mod:`nonbinary_v5b_mother`).  This module is a
Route-B-specific binding: it does not modify any Route A file.  Qualification,
Alice-side tags, and files live in the shared runtime
(:mod:`nonbinary_v5_runtime`) bound through
:mod:`nonbinary_v5b_ir_qualification`.

Policies (frozen):
- ``nbldpc_v5b_ir56`` (candidate): p=.20 two-level 32->40, p=.30 three-level
...
- ``nbldpc_v5b_ir48`` (control): p=.20 two-level 32->40, p=.30 two-level
  40->48, warm retention — the v4-equivalent two-level behavior under the
  v5b identity and fresh data; at most 2 attempts (bound 2^-63).  It never
...
POLICIES = ("nbldpc_v5b_ir56", "nbldpc_v5b_ir48")
Q, N, STAGE_ITERATIONS, MAX_BYTES = 1024, 64, 12, 24 * 1024 * 1024
CHECK_COUNTS = (32, 40, 48, 56)
LADDERS = {"nbldpc_v5b_ir56": {0.20: (32, 40), 0.30: (40, 48, 56)},
           "nbldpc_v5b_ir48": {0.20: (32, 40), 0.30: (40, 48)}}
VERIFICATION_CAPS = {"nbldpc_v5b_ir56": 3, "nbldpc_v5b_ir48": 2}
PROTOCOL_EPSILON = {"nbldpc_v5b_ir56": 2.0 ** -62, "nbldpc_v5b_ir48": 2.0 ** -63}
...
def policy_spec(policy_id: str, p: float) -> dict[str, Any]:
    if policy_id not in POLICIES or float(p) not in (0.20, 0.30):
        raise ValueError("invalid v5b policy/stratum")
...
def extension_mode(policy_id: str) -> str:
    if policy_id not in POLICIES:
        raise ValueError("invalid v5b policy")
    return "warm"  # both v5b policies are warm-retention policies
...
def production_runner(state, matrices, stage):
    """Production adapter: the live v5b state machine is the decoder."""
