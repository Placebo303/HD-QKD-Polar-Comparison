"""V80 B2F soft-marginal L2 prior FER campaign executor (EXPLORE code — NOT execution).

Frozen contract: ``docs/research_cycles/V80-NBLDPC-JAN21/
B2F_EXPERIMENT_PACKET_20260921.md`` (G-B2F, frozen, NOT granted) +
``B2F_EXPERIMENT_PROMPT_20260921.md``. Execution needs a fresh explicit
grant + Pre-EXECUTE; this module only provides the executor +
...
Why this thin NEW module (packet §5/X1): B2E falsified only the HARD
argmax u1 commitment; B2F replaces the u1 POINT ESTIMATE by EXACT BAYES
MARGINALIZATION of the frozen bundle — the Bob-side L2 prior becomes
π_i(e) = Σ_{u1} γ1(u1|b_i)·γ2(y_i⊕e | u1, b_i), y_i = b_i&31, so the u1
dependence disappears from the prior entirely: no genie true-u1, no
argmax û1, no L1 code, no estimator, no accounting change (D-u1 = 0.0
MEASURED label; gates unchanged). This is the d7 ``q @ P`` contraction
(``transfer_prior_l1_to_l2`` v72p2d7 L603-610) with q = the γ1 column,
i.e. the same production transfer the d5 ``app_fed_l2_prior``
(v72p2d5 L354-376) uses, evaluated on the frozen empirical bundle.

Semantics (packet §§1–5; identical to O1R/P0/A208/A202 except the prior):
ONE code n=1024, GF(32), λ={2:1}; acceptance unit = ONE BLOCK = one
n=1024 decode with ``exact_match is True`` (x̂==u2). Two arms only:
**F208 PRIMARY** (m=208, the A208 construction instance — no L1-row
funding ⇒ genie retires with no L1 code at all) and **F202 SECONDARY**
(m=202, the A202 construction instance — m1=6 accounting room). Both
built via ``construct_arm("A208"|"A202", seed=2026092001, trials=20)``
⇒ byte-identical paired instances (recorded; never rebuilt). Pins:
...
precedent). Per block: triple draw on the ``o1_blk:{seed}`` stream
(frozen sampler, draw order unchanged; seeds 2026095601+idx, paired with
O1R/P0/L1B/b2e — NO independence claim); the FROZEN marginal formula
(packet §2, never genie, never argmax); XOR-centered prior into
...
cross-arm pooling; no rerun; no tuning.
...
at bar+1 block fails; root policy (fresh additive ``workspace/b2f_<uuid8>``
...
B2F freezes EXACTLY ONE runnable configuration (arms F208/F202 only;
...
__all__ = [
    "B2F_CONSTRUCT_SEED", "B2F_MAX_TRIALS", "B2F_BLOCK_BASE",
    "B2F_N_BLOCKS", "B2F_N", "B2F_ROOT_PREFIX",
    "ARMS", "CAMPAIGN_LABEL", "U1_SOURCE",
    "MAX_ITER", "H_L2_B2F", "H_FULL_B2F", "H_L1_B2F",
...
#: Frozen B2F construction seed (packet §3; both arms; the A208/A202
#: instance — imported read-only from the frozen O1 module, X1).
B2F_CONSTRUCT_SEED = o1.O1_CONSTRUCT_SEED
#: Frozen constructor trials (packet §3; both arms).
B2F_MAX_TRIALS = o1.O1_MAX_TRIALS
#: Frozen code length (packet §3): n=1024 GF(32) symbols/block.
B2F_N = o1.O1_N
#: Frozen literal block-seed base (packet §4; LITERAL, paired reuse with
#: O1R/P0/L1B/b2e — SAME 240 frames; NO independence claim).
B2F_BLOCK_BASE = 2026095601
#: Frozen campaign width (packet §4): 240 blocks = 240 decodes/arm.
B2F_N_BLOCKS = 240
#: Fresh additive B2F run-root prefix (packet §6: workspace/b2f_<uuid8>).
B2F_ROOT_PREFIX = "workspace/b2f_"
ROOT_PREFIX = B2F_ROOT_PREFIX
...
CAMPAIGN_LABEL = "B2F-soft-marginal"
...
#: 'exploratory' P0 precedent — NO new DE run in B2F).
DE_LABELS = ("covered", "exploratory")
#: Frozen arms table (packet §3; EXACTLY two — no more, no substitutions).
#: ``source_arm`` names the byte-identical paired construction instance
#: in the frozen O1 table (F208→A208, F202→A202); every other m is OUT
#: OF SCOPE (not frozen, not runnable).
ARMS: dict[str, dict[str, Any]] = {
    "F208": {"m": 208, "source_arm": "A208", "lambda": {2: 1.0},
             ...
             "role": "PRIMARY (m=208; A208 construction instance; "
                     "m2=208 leaves NO L1-row funding ⇒ genie retires "
                     "with no L1 code at all)"},
...
def refuse(reason: str) -> "Any":
    print(f"B2F-REFUSAL rc=2: {reason}", file=sys.stderr)
    raise Refusal(2)
...
def block_seed(arm: str, idx: int, base: int | None = None) -> int:
    ...
    Default base is the frozen B2F literal 2026095601, idx=0..239
    ...
    b = B2F_BLOCK_BASE if base is None else base
...
def construct_arm(arm: str, seed: int = B2F_CONSTRUCT_SEED,
                  max_trials: int = B2F_MAX_TRIALS) -> dict[str, Any]:
    """B2F arm constructor (packet §3/X2): delegate to the FROZEN O1
    constructor on the paired source instance — F208 → ``A208``, F202 →
...
def leak_basis(arm: str) -> dict[str, Any]:
    """Frozen B2F accounting (packet §4; the UNCHANGED O1 basis).
    ...
    return {
        ...
        "f_super_label": ("UNCHANGED O1 basis, D-u1 = 0.0 MEASURED "
                          ...
                          f"NEVER assume zero in a claim (B2F packet §4)"),
        ...
        "d_u1_label": ("D-u1 = 0.0 MEASURED label: no u1 disclosure — "
                       ...
                       "NEVER-ASSUME-ZERO note retained (B2F packet "
                       "§4/X6)"),
        "h_l2": H_L2_B2F,
        "h_l1": H_L1_B2F,
        "h_full": H_FULL_B2F,
...
def _allows_root(root: str) -> bool:
    """Fresh additive prefix: B2F ``workspace/b2f_<uuid>`` (packet §6)."""
    return root.startswith(ROOT_PREFIX)


def _check_root(root: str) -> None:
    if not root:
        refuse("root required (fresh additive workspace/b2f_<uuid>)")
...
def _de_cover_record(arm: str, de_label: str | None) -> dict[str, Any]:
    ...
    'exploratory' (P0 precedent). NO new DE run in B2F; gates unchanged.
    """
    if arm not in ARMS:
        refuse(f"unknown arm {arm} (frozen: F208|F202 only)")
...
def _build_manifest(*, arm: str, construction: dict, rows: list[dict],
    ...
    bb = B2F_BLOCK_BASE if block_base is None else int(block_base)
    nb = B2F_N_BLOCKS if n_blocks is None else int(n_blocks)
    bar = fail_bar(nb)
    ...
    return {
        ...
        "single_code": ("acceptance unit = ONE BLOCK = one n=1024 decode "
                        ...
                        "group-of-4 rule (B2F packet §1; block≠S2c-group, "
        ...
        "seeds": {
            "policy": "literal-frozen (B2F packet §4; NOT derived)",
            "block_base": bb,
            ...
            "paired_note": ("Blocks REUSED paired with O1R/P0/L1B/b2e: "
                            ...
                            "(O1R/P0/L1B/b2e roots+docs = expected hits); "
                            "NO independence claim B2F↔O1R↔P0↔L1B↔b2e "
                            "(B2F packet §4)"),
        },
        "channel": {
        ...
        "n_rows": n,
        "resume_policy_note": ("B2F packet §6: checkpoint-per-block + at "
...
def _validate_partial(manifest: dict, rows: list, arm: str,
                      block_base: int = B2F_BLOCK_BASE,
                      n_blocks: int = B2F_N_BLOCKS,
                      construct_seed: int = B2F_CONSTRUCT_SEED) -> dict:
    ...
    if (not isinstance(seeds, dict)
            or seeds.get("policy") != "literal-frozen (B2F packet §4; NOT derived)"
...
def _construct_gate(arm: str, construct_fn: Callable,
                    seed: int = B2F_CONSTRUCT_SEED,
                    trials: int = B2F_MAX_TRIALS) -> dict:
    ...
    if fc != 0:
        refuse(f"{arm} four_cycles {fc} != 0 (STOP-BLOCKED; B2F packet "
               f"§3/X3; measured girth {girth})")
    if rank != m:
        refuse(f"{arm} rank {rank} != {m} (STOP-BLOCKED; B2F packet "
               f"§3/X3; measured girth {girth})")
...
def execute(*, root: str, arm: str,
            ...
            block_base: int = B2F_BLOCK_BASE,
            n_blocks: int = B2F_N_BLOCKS,
            construct_seed: int = B2F_CONSTRUCT_SEED,
            construct_trials: int = B2F_MAX_TRIALS) -> dict:
    """Run (or once-continue) the frozen 240-block B2F campaign under
    ``root``.
    ...
    if decode_fn is None:
        ...
        def decode_fn(construction, seed, _b=_bundle):  # noqa: B023
            return decode_block_marginal(construction, seed, _b, B2F_N, m)
...
def run_execution(root: str, arm: str,
                  ...
                  n_blocks: int = B2F_N_BLOCKS,
                  construct_seed: int = B2F_CONSTRUCT_SEED,
                  construct_trials: int = B2F_MAX_TRIALS) -> int:
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--block-base", type=int, default=B2F_BLOCK_BASE)
    ap.add_argument("--n-blocks", type=int, default=B2F_N_BLOCKS)
    ap.add_argument("--construct-seed", type=int,
                    default=B2F_CONSTRUCT_SEED)
    ap.add_argument("--construct-trials", type=int,
                    default=B2F_MAX_TRIALS)
    ...
    if isinstance(args.construct_trials, bool) \
        ...
        refuse("construct-trials must be a positive int")
    # B2F has EXACTLY ONE frozen configuration: any deviation is a
    # science-input change → STOP (packet §6). The execute() parameters
    # exist for resume validation; the CLI cannot drift them.
    if args.construct_seed != B2F_CONSTRUCT_SEED:
        refuse(f"construct-seed must be the frozen {B2F_CONSTRUCT_SEED} "
               f"(got {args.construct_seed}; B2F packet §3 — any seed "
               f"change is a science-input change → STOP)")
    if args.construct_trials != B2F_MAX_TRIALS:
        refuse(f"construct-trials must be the frozen {B2F_MAX_TRIALS} "
               f"(got {args.construct_trials}; B2F packet §3 — any trials "
               f"change is a science-input change → STOP)")
    if args.block_base != B2F_BLOCK_BASE:
        refuse(f"block-base must be the frozen {B2F_BLOCK_BASE} "
               f"(got {args.block_base}; B2F packet §4 — any block-seed "
               f"change is a science-input change → STOP)")
    if args.n_blocks != B2F_N_BLOCKS:
        refuse(f"n-blocks must be the frozen {B2F_N_BLOCKS} "
               f"(got {args.n_blocks}; B2F packet §4 — any width change "
