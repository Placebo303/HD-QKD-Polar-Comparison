"""V72P2D4R2 CAL GF32 model rate audit — U/G/F/L frame-blocked nested CV.
...
Models U reference + G/F/L closed + M3 direct EXIT + circulant DEFERRED.
F is full P(A|B) single lambda grid30 inner-selected; L is hierarchical
P(U1|B)*P(U2|U1,B) reusing the same outer-fold F-selected lambda
(no independent search, target mean joint CE). No VAL fit, no decoder
call, no parquet VAL read, single audit output only.
"""
...
CYCLE_ID = "V72P2D4R2-CAL-GF32-MODEL-RATE"
AUDIT_SCHEMA = "v72p2d4r2_cal_gf32_model_rate_audit_v1"
MANIFEST_SCHEMA = "v72p2d4r2_cal_gf32_model_rate_manifest_v1"
OUT_DIR_NAME = "v72p2d4r2_cal_gf32_model_rate_audit_20260905"
...
NORM_TOL = 1e-12
CHAIN_TOL = 1e-10
# Lambda grid: 30 points, inner-only (outer forbidden).
LAMBDA_GRID = tuple(float(v) for v in np.logspace(-2.0, 3.0, 30))
M3_STATUS = "M3_EXIT_AMBIGUOUS"
CIRCULANT_STATUS = "CIRCULANT_DEFERRED"
# U uniform reference (exact, descriptive only).
UNIFORM_CE_L1 = 5.0
UNIFORM_CE_L2 = 5.0
UNIFORM_CE_JOINT = 10.0
UNIFORM_STATUS = "uniform_reference_descriptive"
...
def validate_output_target(out_dir: str | Path) -> Path:
    """R2 output guard: exact pre-registered root or fresh workspace dir only."""
    raw = Path(out_dir)
...
def uniform_reference_record() -> dict[str, Any]:
    """U uniform reference: P=1/1024, CE 5/5/10 exact, descriptive only."""
    return {
        "model": "U",
        "status": UNIFORM_STATUS,
        "p": 1.0 / 1024.0,
        "ce_l1": float(UNIFORM_CE_L1),
        "ce_l2": float(UNIFORM_CE_L2),
        "ce_joint": float(UNIFORM_CE_JOINT),
        "in_selection": False,
        "in_budget": False,
        "in_route": False,
    }
...
def uniform_ce() -> dict[str, float]:
    """Exact uniform CE: log2 32=5, log2 1024=10."""
    return {"ce_l1": 5.0, "ce_l2": 5.0, "ce_joint": 10.0}


def build_g(a_train: np.ndarray) -> dict[str, Any]:
    """G pin train marginal: P(a)=count(A=a)/n, B-independent, floor only for log."""
    a = np.asarray(a_train, dtype=np.int64).reshape(-1)
    if a.size == 0:
        raise ValueError("G train must be non-empty")
    if np.any(a < 0) or np.any(a >= Q):
        raise ValueError("G symbols outside 0..1023")
    n = int(a.size)
    cnt = np.zeros(Q, dtype=np.float64)
    np.add.at(cnt, a, 1.0)
    p_a = cnt / float(n)
    if abs(float(p_a.sum()) - 1.0) > NORM_TOL:
        raise ValueError("G marginal must sum to 1 within 1e-12")
    ...
    if abs(float(p_u1.sum()) - 1.0) > NORM_TOL:
        raise ValueError("G P(U1) must sum to 1 within 1e-12")
...
def build_f(a_train: np.ndarray, b_train: np.ndarray, lam: float) -> dict[str, Any]:
    """F full P(A|B): column smoothing with inner-selected lam, then chain split.

    Estimator: P(a|b)=(counts[a,b]+lam*p_global[a])/(n_b[b]+lam).
    Unseen Bob column falls back to the train marginal p_global (lam-weighted).
    lam must come from the 30-point grid via inner TRAIN fit + inner
    held-out CE only; outer TEST must never enter selection.
    """
    a = np.asarray(a_train, dtype=np.int64).reshape(-1)
    b = np.asarray(b_train, dtype=np.int64).reshape(-1)
    if a.shape != b.shape or a.size == 0:
        raise ValueError("F train must be non-empty equal length")
    lam_f = float(lam)
    ...
    if not np.all(np.abs(col - 1.0) <= NORM_TOL):
        raise ValueError("F P(A|B) columns must sum to 1 within 1e-12")
...
def build_l(a_train: np.ndarray, b_train: np.ndarray, lam: float) -> dict[str, Any]:
    """L hierarchical P(U1|B)*P(U2|U1,B) with Laplace lam (TRAIN-only).

    Ponytail contract: lam is the same outer-fold F-selected value
    (no independent search); objective is the mean joint CE.
    """
    a = np.asarray(a_train, dtype=np.int64).reshape(-1)
    b = np.asarray(b_train, dtype=np.int64).reshape(-1)
    if a.shape != b.shape or a.size == 0:
        raise ValueError("L train must be non-empty equal length")
    lam_f = float(lam)
    if not np.isfinite(lam_f) or lam_f <= 0:
        raise ValueError("lam must be finite positive")
    n = int(a.size)
...
def circulant_deferred_record() -> dict[str, Any]:
    """Any circulant structural variant is DEFERRED this round, excluded."""
    return {"model": "circulant", "status": CIRCULANT_STATUS, "in_selection": False, "in_budget": False, "in_route": False}
...
def ce_g(fit: dict[str, Any], a_test: np.ndarray, b_test: np.ndarray) -> dict[str, float]:
    ...
    if err >= CHAIN_TOL:
        raise ValueError(f"G chain error {err} exceeds 1e-10")
    return {"ce_l1": ce_1, "ce_l2_oracle": ce_2, "ce_joint": ce_j, "chain_err": err, "n": int(a.size)}


def ce_f(fit: dict[str, Any], a_test: np.ndarray, b_test: np.ndarray) -> dict[str, float]:
    a = np.asarray(a_test, dtype=np.int64).reshape(-1)
    ...
    err = abs(ce_j - ce_1 - ce_2)
    if err >= CHAIN_TOL:
        raise ValueError(f"F chain error {err} exceeds 1e-10")
    return {"ce_l1": ce_1, "ce_l2_oracle": ce_2, "ce_joint": ce_j, "chain_err": err, "n": int(a.size)}


def ce_l(fit: dict[str, Any], a_test: np.ndarray, b_test: np.ndarray) -> dict[str, float]:
    a = np.asarray(a_test, dtype=np.int64).reshape(-1)
    b = np.asarray(b_test, dtype=np.int64).reshape(-1)
    if a.shape != b.shape or a.size == 0:
        raise ValueError("eval arrays must be non-empty equal length")
    ...
    err = abs(ce_j - ce_1 - ce_2)
    if err >= CHAIN_TOL:
        raise ValueError(f"L chain error {err} exceeds 1e-10")
    return {"ce_l1": ce_1, "ce_l2_oracle": ce_2, "ce_joint": ce_j, "chain_err": err, "n": int(a.size)}
...
def evaluate_outer_fold(
    bundle: dict[int, dict[str, np.ndarray]], k: int, selected_lam_f: float
) -> dict[str, Any]:
    """Fit G/F/L on outer TRAIN, score on outer TEST (TEST never enters fit).

    L reuses the same outer-fold F-selected lambda (no independent search).
    """
    outer = outer_folds()
    train_ids = list(outer[int(k)]["train_ids"])
    ...
    # G train marginal (B-independent).
    fg = build_g(a_tr)
    cg = ce_g(fg, a_te, b_te)
    loss_g = -np.log2(np.maximum(np.asarray(fg["P_A"])[a_te], FLOOR))
    sg = _seen_split_scores(b_tr, b_te, loss_g)
    out["G"] = {**cg, **sg}
    # F full with inner-selected lam.
    ff = build_f(a_tr, b_tr, float(selected_lam_f))
    cf = ce_f(ff, a_te, b_te)
    loss_f = -np.log2(np.maximum(np.asarray(ff["P_A_given_B"])[a_te, b_te], FLOOR))
    sf = _seen_split_scores(b_tr, b_te, loss_f)
    out["F"] = {**cf, **sf, "lam": float(selected_lam_f)}
    # L hierarchical reusing the same F lam (Ponytail, no independent search).
    fl = build_l(a_tr, b_tr, float(selected_lam_f))
    cl = ce_l(fl, a_te, b_te)
    p1m = np.asarray(fl["P1"])[b_te, symbols_to_layers(a_te)[1]]
    low_te, high_te = symbols_to_layers(a_te)
    p2m = np.asarray(fl["P2"])[high_te, b_te, low_te]
    loss_l = -np.log2(np.maximum(np.maximum(p1m, FLOOR) * np.maximum(p2m, FLOOR), FLOOR))
    sl = _seen_split_scores(b_tr, b_te, loss_l)
    out["L"] = {**cl, "lam": float(selected_lam_f), **sl}
    return out


def inner_select_lambda_for_f(
    bundle: dict[int, dict[str, np.ndarray]], k: int, lam_grid: tuple[float, ...] | None = None
) -> dict[str, Any]:
    """Inner 3-fold selection for F: fit on inner TRAIN, score inner held-out only.

    Never reads outer TEST. Returns selected lam (minimal inner mean joint).
    L reuses this value without its own search.
    """
    ...
    for lam in grid:
        ...
        for inn in inners:
            ...
            fit = build_f(a_tr, b_tr, lam)
            sc = ce_f(fit, a_te, b_te)
...
def r5_synth_repro() -> dict[str, Any]:
    ...
    Hierarchical L path with R5_LAM reproduces the R1 M2 fixture numbers.
    """
    ...
    for kk in range(4):
        ...
        fit = build_l(a_tr, b_tr, R5_LAM)
        sc = ce_l(fit, a_te, b_te)
...
def select_model(outer_results: list[dict[str, Any]]) -> dict[str, Any]:
    """Fold-mean minimal over G/F/L + delta 0.02 simple priority + stability downgrade."""
    stats: dict[str, Any] = {}
...
def build_audit_summary(
    *,
    ...
    # Root-cause decomposition U->G/G->F/F->L descriptive, same caliber.
    gm = float(selection["stats"]["G"]["mean"])
    fm = float(selection["stats"]["F"]["mean"])
    lm = float(selection["stats"]["L"]["mean"])
    u_joint = float(UNIFORM_CE_JOINT)
    return {
        "schema": AUDIT_SCHEMA,
        "status": "READY",
        "cycle": CYCLE_ID,
        ...
        "lambda_scope": "grid30 inner-only; outer TEST never enters selection",
        "m3": m3_exit_record(),
        "circulant": circulant_deferred_record(),
        "uniform_reference_descriptive": {
            "ce_l1": float(UNIFORM_CE_L1),
            "ce_l2": float(UNIFORM_CE_L2),
            "ce_joint": float(UNIFORM_CE_JOINT),
            "in_selection": False,
        ...
        "outer_results": [
            {
                ...
                "n_test": int(r["n_test"]),
                "G": {"ce_l1": float(r["G"]["ce_l1"]), "ce_l2_oracle": float(r["G"]["ce_l2_oracle"]), "ce_joint": float(r["G"]["ce_joint"]), "chain_err": float(r["G"]["chain_err"]), "seen_frac": float(r["G"]["seen_frac"])},
                "F": {"ce_l1": float(r["F"]["ce_l1"]), "ce_l2_oracle": float(r["F"]["ce_l2_oracle"]), "ce_joint": float(r["F"]["ce_joint"]), "chain_err": float(r["F"]["chain_err"]), "lam": float(r["F"]["lam"]), "seen_frac": float(r["F"]["seen_frac"])},
                "L": {"ce_l1": float(r["L"]["ce_l1"]), "ce_l2_oracle": float(r["L"]["ce_l2_oracle"]), "ce_joint": float(r["L"]["ce_joint"]), "chain_err": float(r["L"]["chain_err"]), "lam": float(r["L"]["lam"]), "seen_frac": float(r["L"]["seen_frac"])},
            }
        ...
        "root_cause_decomposition": {
            "u_to_g": float(u_joint - gm),
            "g_to_f": float(gm - fm),
            "f_to_l": float(fm - lm),
            "u_to_selected": float(u_joint - float(sel_stats["mean"])),
...
def write_audit_outputs(out_dir: str | Path, summary: dict[str, Any]) -> Path:
    ...
    manifest = {
        ...
        "models": ["U(uniform-5/5/10-reference)", "G(train-marginal)", "F(full,grid30-inner)", "L(hierarchical,reuse-F-lam)", "M3_EXIT_AMBIGUOUS", "CIRCULANT_DEFERRED"],
        "selected": sel,
    ...
    lines = [
        ...
        f"- U: `5/5/10 exact ({UNIFORM_STATUS})` (reference, not in selection)",
    ...
    rc = summary["root_cause_decomposition"]
    lines.append(f"- U->G `{rc['u_to_g']:.6f}` G->F `{rc['g_to_f']:.6f}` F->L `{rc['f_to_l']:.6f}` U->selected `{rc['u_to_selected']:.6f}`")
    lines.append(f"- M3: `{M3_STATUS}` (excluded)")
    lines.append(f"- circulant: `{CIRCULANT_STATUS}` (excluded)")
...
def run_cal_audit(
    *,
    ...
    grid = tuple(float(v) for v in (LAMBDA_GRID if lam_grid is None else lam_grid))
    inner_selections = [inner_select_lambda_for_f(bundle, k, grid) for k in range(4)]
