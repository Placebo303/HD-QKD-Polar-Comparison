"""V46P0 L1-APP soft-transfer with L2-only verification semantics: guarded runner and writer.

Implements frozen V46P0 protocol (formal-ir-v46-verification-semantics):

- Single phase of exactly 27 decoder invocations: 9 L1 (H1 16x1024 QC-cyclic) + 9 control L2 + 9 treatment L2
  (9 never-used TRAIN blocks 390122-124/390222-224/390322-324, each sampled once and paired)
...
- Per-condition gates G1'/G2'/G3' (exact_l2, G3'=undetected_accepted_wrong==0) and 5-state machine with four reclassified {exact, detected_verification_failure, decoder_non_syndrome_failure, undetected_accepted_wrong} and tag_ok gate.
- L2-only verification: tag = compute_tag_64(empty_uint8, x2) via V35, empty=np.empty(0,dtype=np.uint8), tag_scope=l2_only, SHA-trunc64 engineering verification (approx 2^-64 random-hash approx, not information-theoretic).
...
CYCLE_ID = "V46P0"
CHANGE_ID = "formal-ir-v46-verification-semantics"
ACCEPTED_PLAN_SHA = "613f9e3c8402cb079c99dac72e115a7131fb8fa6"
BRANCH_REF = "origin/formal-ir-mainline"
EXECUTION_SCOPE = "v46_verification_semantics_27_calls_exactly_once"
...
MECHANISM_ID = "l1_app_soft_transfer_H1_syndrome_derived_with_v35_tag_l2_only"
MECHANISM_CANDIDATE = "l1_app_soft_transfer"
TAG_SCOPE = "l2_only"
TAG_SOURCE_STR = "v35:compute_tag_64(empty,x2)[:16] tag_scope=l2_only"
LEAKAGE_ALREADY_ACCOUNTED_NOTE = "leakage already accounted: Control 984/1014/1024 vs Treatment 1064/1094/1104 includes 64-bit tag; SHA-trunc64 random-hash-model approximate 2^-64, not information-theoretic; strict bound requires universal2+seed; L2-only"
...
FROZEN_WORKLOAD: tuple[dict[str, Any], ...] = (
    {"call_id": "C01", "condition": COND_CONTROL, "source": "1M", "block_seed": 390122},
    {"call_id": "C02", "condition": COND_L1_APP, "source": "1M", "block_seed": 390122},
    {"call_id": "C03", "condition": COND_CONTROL, "source": "1M", "block_seed": 390123},
    {"call_id": "C04", "condition": COND_L1_APP, "source": "1M", "block_seed": 390123},
    {"call_id": "C05", "condition": COND_CONTROL, "source": "1M", "block_seed": 390124},
    {"call_id": "C06", "condition": COND_L1_APP, "source": "1M", "block_seed": 390124},
    {"call_id": "C07", "condition": COND_CONTROL, "source": "1p5M", "block_seed": 390222},
    {"call_id": "C08", "condition": COND_L1_APP, "source": "1p5M", "block_seed": 390222},
    {"call_id": "C09", "condition": COND_CONTROL, "source": "1p5M", "block_seed": 390223},
    {"call_id": "C10", "condition": COND_L1_APP, "source": "1p5M", "block_seed": 390223},
    {"call_id": "C11", "condition": COND_CONTROL, "source": "1p5M", "block_seed": 390224},
    {"call_id": "C12", "condition": COND_L1_APP, "source": "1p5M", "block_seed": 390224},
    {"call_id": "C13", "condition": COND_CONTROL, "source": "2M", "block_seed": 390322},
    {"call_id": "C14", "condition": COND_L1_APP, "source": "2M", "block_seed": 390322},
    {"call_id": "C15", "condition": COND_CONTROL, "source": "2M", "block_seed": 390323},
    {"call_id": "C16", "condition": COND_L1_APP, "source": "2M", "block_seed": 390323},
    {"call_id": "C17", "condition": COND_CONTROL, "source": "2M", "block_seed": 390324},
    {"call_id": "C18", "condition": COND_L1_APP, "source": "2M", "block_seed": 390324},
)
...
PREFLIGHT_BLOCK_SEEDS: dict[str, int] = {"1M": 390122, "1p5M": 390222, "2M": 390322}
...
OUTPUT_ROOT = (
    REPO_ROOT / "comparison_bench/outputs_comparison/formal_ir_methods/v46_verification_semantics/run_01"
)
FORBIDDEN_WINNER_NPZ_NAME = "v38_winning_matrices.npz"

SCOPED_TRACKED_PATHS: tuple[str, ...] = (
    "comparison_bench/src/comparison_bench/formal_ir/v46_verification_semantics.py",
    "scripts/execute_v46_verification_semantics.py",
...
MASTER_STOP_RULE = (
    ...
    "无论结果如何：不追加 blocks、不补跑、不做第二轮诊断、不并行测试多方案、不启动 V47。"
)

TERMINAL_EVIDENCE_INVALID = "V46_EVIDENCE_INVALID"
TERMINAL_BOTH_RETAINED = "V46_BOTH_RETAINED"
TERMINAL_L1APP_NO_VALUE_OR_HARM = "V46_L1APP_NO_VALUE_OR_HARM"
TERMINAL_GO_STRUCTURE = "V46_GO_STRUCTURE"
TERMINAL_L1APP_ADDED_VALUE_SIGNAL = "V46_L1APP_ADDED_VALUE_SIGNAL"
...
CLAIM_BOUNDARY: tuple[str, ...] = (
    ...
    "V46_L1APP_NO_VALUE_OR_HARM attributes loss ONLY to this L1-APP soft transfer limitation (M_{H1,s1} insufficient or H1 non-convergence, extra 80-bit not transferred), NOT Lane C graph structure nor concrete upstream coding scheme or real system",
    "V46_L1APP_ADDED_VALUE_SIGNAL means ONLY finite-sample/iteration/prior differences require inspection; never unconditional evidence treatment outperforms control",
)
...
def get_l1_prior_p_u1_given_b(counts: np.ndarray, bob: np.ndarray) -> np.ndarray:
    ...
    assert MECHANISM_ID == "l1_app_soft_transfer_H1_syndrome_derived_with_v35_tag_l2_only", "mechanism identity drift"
...
# aliases
reconstruct_v43_matrices = reconstruct_v46_matrices
reconstruct_v45_matrices = reconstruct_v46_matrices
reconstruct_v42_matrices = reconstruct_v46_matrices
...
def dual_posterior_binding_preflight(
    ...
    for source in SOURCE_ORDER:
        ...
        # verify V35 tag import (empty prefix L2-only)
        try:
            _empty = np.empty(0, dtype=np.uint8)
            _x2_sample = np.asarray(u2_alice[:4], dtype=np.uint8) if len(u2_alice) >=4 else np.asarray([0,1,2,3], dtype=np.uint8)
            _t = compute_tag_64(_empty, _x2_sample)
            assert isinstance(_t, str) and len(_t)==16
            int(_t,16)
            # x1 invariance: changing x1 must not change tag
            _x1_a = np.array([1,2,3,4], dtype=np.uint8)
            _x1_b = np.array([5,6,7,8], dtype=np.uint8)
            _tag_a = compute_tag_64(_empty, _x2_sample)
            _tag_b = compute_tag_64(_empty, _x2_sample)
            assert _tag_a == _tag_b
            # x2 sensitivity: changing x2 must change tag (for this anchor)
            _x2_alt = np.asarray(_x2_sample, dtype=np.uint8).copy()
        ...
        checks: dict[str, Any] = {
            ...
            "v35_tag_import_ok": True,
            "tag_scope_l2_only": TAG_SCOPE == "l2_only",
            "tag_l2_only_empty_prefix_ok": True,
...
# ---------------------------------------------------------------------------
# V46 L2-only tag helpers (reuse V35 compute_tag_64, empty prefix)
# ---------------------------------------------------------------------------

def compute_l2_tag(x2: np.ndarray) -> str:
    """L2-only tag via V35 compute_tag_64(empty, x2)."""
...
def classify_reclassified(exact_l2: bool, syndrome_ok_l2: bool, tag_ok: bool) -> str:
    """Four-class reclassification (mutually exclusive)."""
    if exact_l2:
        # exact implies tag_ok must be true (otherwise inconsistent)
        return "exact"
...
def build_record(spec: dict[str, Any], raw: dict[str, Any], setting: tuple[int, float]) -> dict[str, Any]:
    ...
    # plus target_tag/candidate_tag/tag_ok/reclassified (L2-only, empty prefix reuse V35)
    ...
    # derive reclassified if not supplied
...
def _evaluate_one_condition(
    ...
    # L2-only verification (empty prefix + x2) via V35 compute_tag_64
    empty = np.empty(0, dtype=np.uint8)
    target_tag = compute_tag_64(empty, np.asarray(u2_alice, dtype=np.uint8))
    # For fake_runner exact=true already x_hat == true; for non-exact, x_hat != true but tags handled via injected raw fields (tag_ok may be forced by test). Here we compute candidate from actual x_hat.
    # If caller injected tag_ok override (for fake tests that force undetected), respect it; otherwise compute.
    ...
    *,
    ...
    return {
        ...
        "leakage": {
            ...
            "note": "Control 984/1014/1024 vs Treatment 1064/1094/1104 (syndrome 920/950/960+64 f_total; leakage already accounted; SHA-trunc64 random-hash-model approx 2^-64 L2-only)",
            "leakage_already_accounted": True,
        ...
        "control_arm_undetected_anomaly": bool(control_arm_wrong_codeword_anomaly),
...
# compatibility
build_v43_summary = build_v46_summary
build_v45_summary = build_v46_summary
...
def write_v46_outputs(output_root: Path | str, records: list[dict[str, Any]], summary: dict[str, Any]) -> Path:
    ...
    dump("v46_records.json", records)
    write_records_csv(root / "v46_records.csv", records, list(RECORD_FIELDS))
    dump("v46_summary.json", summary)
    return root
...
def write_invalid_notice(output_root: Path | str, integrity_failures: list[tuple[str, str]], partial_records_retained: bool) -> Path:
    ...
    path = root / "v46_invalid_notice.json"
...
def _persist_invalid_evidence(root: Path, *, records: list[dict[str, Any]], accounting: CallAccounting, summary_ctx: dict[str, Any], failures: list[tuple[str, str]], partial_records_retained: bool) -> None:
    try:
        if not root.exists():
            root.mkdir(parents=True)
        with (root / "v46_records.json").open("w", encoding="utf-8") as handle:
...
def run_v46_diagnostic(
    ...
    try:
        ...
        matrices = reconstruct_v46_matrices(reference_metrics_path=spath, field=field, constructors=constructors)
    ...
    try:
        ...
        for cond in CONDITION_ORDER:
            und = gate_evaluation[cond]["g3_undetected_zero"]["undetected_count"]
            stopped[cond] = bool(und > 0)
        control_wrong = gate_evaluation[COND_CONTROL]["g3_undetected_zero"]["undetected_count"] > 0
        ...
        terminal_state, terminal_reason, routing_trace = determine_v46_terminal(
            integrity_ok=True,
        ...
        write_v46_outputs(root, records, summary)
