"""V80 B2G soft-marginal campaign executor tests (EXPLORE, FAKE-ONLY).

Frozen spec: B2G_EXPERIMENT_PACKET_20260921.md + B2G_EXPERIMENT_PROMPT_
20260921.md (G-B2G, arms F208|F202, 240 blocks, construct seed
2026092011 = O1R R2 second construction instance). Every test injects
fake construct/decode/clock/rss/writer or synthetic dict bundles — zero
...
constructor in-memory for F208/F202 at seed 2026092011 (allowed: pins
recorded, no writes, no decodes). The dry 1-block probe (T13) runs the
REAL frozen bundle + REAL sampler + REAL marginal formula + REAL
XOR-centering with a FAKE kernel (no production decode at scale) and
reports the measured ``prior_entropy_bits``.
"""
import inspect
import json
import os
...
def _fake_construct(arm):
    ...
    # seed, max_trials) and asserts the gate forwards the B2G arm id.
    spec = c.ARMS[arm]
    assert spec["source_arm"] in ("A208", "A202")

    def _build(arm_in, seed, max_trials):
        assert arm_in == arm
        assert (seed, max_trials) == (2026092011, 20)
        m = int(spec["m"])
...
def test_t1_dual_flag_refusal(capsys):
    with pytest.raises(c.Refusal):
        c.main([])
    with pytest.raises(c.Refusal):
        c.main(["--execute-real", "--arm", "F208",
                "--root", "workspace/b2g_deadbeef"])
    with pytest.raises(c.Refusal):
    ...
    for bad_arm in ("A208", "B208", "F200", "f208", ""):
        with pytest.raises(c.Refusal):
            c.main(["--execute-real", "--execution-authorized",
                    "--arm", bad_arm, "--root", "workspace/b2g_deadbeef"])
    with pytest.raises(c.Refusal):
        c.main(["--execute-real", "--execution-authorized",
                "--arm", "F208", "--root", "workspace/b2g_aaaaaaaa",
                "--resume-from", "workspace/b2g_bbbbbbbb"])
    with pytest.raises(c.Refusal):
        c.main(["--execute-real", "--execution-authorized",
                "--arm", "F208", "--root", "workspace/b2g_deadbeef",
                "--de-label", "maybe"])
    with pytest.raises(c.Refusal):
        c.main(["--execute-real", "--execution-authorized",
                "--arm", "F208", "--root", "workspace/b2g_deadbeef",
    ...
    with pytest.raises(c.Refusal):
        c.main(["--execute-real", "--execution-authorized",
                "--arm", "F208", "--root", "workspace/b2g_deadbeef",
                "--construct-trials", "0"])
    # Frozen-configuration refusals: B2G has EXACTLY ONE runnable config
    # (construct seed 2026092011 ONLY); any science-input deviation
    # refuses rc=2 BEFORE any root contact (packet §6 STOP) — INCLUDING
    # 2026092001, which is b2f's frozen construct seed, NOT b2g's.
    # NOTE: no test ever passes the dual-flag gate with a valid
    # workspace/b2g_ root — that path is the real campaign and is never
    # invoked from tests (repo rule: never invoke production work
    # implicitly from tests).
    for extra in (["--construct-seed", "2026092001"],   # b2f's value
                  ["--construct-seed", "2026092021"],
                  ["--construct-seed", "2026095601"],
                  ["--construct-trials", "10"],
                  ["--construct-trials", "21"],
                  ["--block-base", "2026095501"],
                  ["--block-base", "2026095600"],
                  ["--n-blocks", "60"],
                  ["--n-blocks", "241"]):
        with pytest.raises(c.Refusal):
            c.main(["--execute-real", "--execution-authorized",
                    "--arm", "F208", "--root", "workspace/b2g_deadbeef"]
                   + extra)
    # The frozen-config refusal names the ONE preregistered seed change
    # and explicitly rejects b2f's 2026092001 (not b2g's value).
    with pytest.raises(c.Refusal) as exc:
        c.main(["--execute-real", "--execution-authorized",
                "--arm", "F208", "--root", "workspace/b2g_deadbeef",
                "--construct-seed", "2026092001"])
    assert exc.value.code == 2
    err = capsys.readouterr().err
    assert "science-input change" in err
    assert "2026092011" in err and "2026092001" in err


# T2: root absent/present matrix + forbidden trees. ------------------------
def test_t2_root_absent_present_matrix(tmp_path):
    w, clk = MemWriter(), FakeClock()
    mf = _run("/tmp/opencode/b2g_fake_absent_01", w, clk, _decode_ok,
              max_blocks=1)
    ...
    with pytest.raises(c.Refusal):
        _run(str(tmp_path), MemWriter(), FakeClock(), _decode_ok,
             max_blocks=1)
    with pytest.raises(c.Refusal):
        c.execute(root="/tmp/opencode/b2g_fake_absent_02", arm="F208",
                  resume_from="/tmp/opencode/b2g_fake_absent_02",
                  construct_fn=_fake_construct("F208"),
                  decode_fn=_decode_ok,
                  clock=FakeClock(), rss_fn=lambda: 0, writer=MemWriter())
    with pytest.raises(c.Refusal):
        c.execute(root="/tmp/opencode/b2g_fake_a", arm="F208",
                  resume_from="/tmp/opencode/b2g_fake_b",
                  construct_fn=_fake_construct("F208"),
                  decode_fn=_decode_ok,
                  clock=FakeClock(), rss_fn=lambda: 0, writer=MemWriter())
    for bad in ("/tmp/opencode/results/b2g_fake_x",
...
def test_t3_arms_table_and_accounting(arm, m, leak, f_super, headroom):
    ...
    assert c.B2G_BLOCK_BASE == 2026095601
    assert c.B2G_N_BLOCKS == 240
    assert c.B2G_CONSTRUCT_SEED == 2026092011
    assert c.B2G_MAX_TRIALS == 20
    assert c.B2G_N == 1024
    ...
    assert c.CAMPAIGN_LABEL == "B2G-soft-marginal-replication"
    assert c.D_U1 == 0.0
...
def test_t3_unknown_arm_refuses():
    ...
    with pytest.raises(c.Refusal) as exc:
        c.execute(root="/tmp/opencode/b2g_fake_badarm", arm="A208",
                  construct_fn=_fake_construct("F208"),
...
def test_t4_girth_recorded_not_gated():
    ...
    con = c._construct_gate("F202", _g4, 2026092011, 20)
...
def test_t4_pin_mismatch_stop_blocked(pin, capsys):
    ...
    with pytest.raises(c.Refusal) as exc:
        c.execute(root="/tmp/opencode/b2g_fake_pinbad", arm="F208",
                  construct_fn=_bad,
                  decode_fn=_counting, clock=FakeClock(),
                  rss_fn=lambda: 0, writer=MemWriter(), max_blocks=1)
    assert exc.value.code == 2
...
def _measured(arm):
    if arm not in _MEASURED:
        a = c.construct_arm(arm, 2026092011, 20)
        b = c.construct_arm(arm, 2026092011, 20)
...
def test_t5_dry_construct_pins_recorded(arm, m):
    ...
    # MEASURED girth 6 at construct seed 2026092011 (both arms) —
    # recorded-not-gated covariate (O1R R2 precedent: A208 girth 6 at
    # this seed; A202 girth 6 measured here). Girth never gates.
...
def test_t5b_real_pins_through_real_gate(arm, m):
    ...
    # delegating to the real constructor asserts the B2G arm id + frozen
    ...
    assert seen[0] == (arm, 2026092011, 20)
    assert seen[1] == (arm, 2026092011, 20)  # construct-twice
    assert con["four_cycles"] == 0
    assert con["rank"] == m
    assert con["min_girth"] == 6  # measured at seed 2026092011; recorded
...
def test_t9_all_ok_pass_and_manifest(arm):
    w, clk = MemWriter(), FakeClock()
    root = f"/tmp/opencode/b2g_fake_allok_{arm.lower()}"
    seen = []
    ...
    assert mf["campaign"] == "B2G-soft-marginal-replication"
    assert mf["arm"] == arm
...
def test_t9_early_stop_thirteenth_fail():
    w, clk = MemWriter(), FakeClock()
    mf = _run("/tmp/opencode/b2g_fake_early240", w, clk, _decode_fail)
...
def test_t9_bar_boundary_twelve_fails_pass():
    ...
    w, clk = MemWriter(), FakeClock()
    mf = _run("/tmp/opencode/b2g_fake_bar12", w, clk, _twelve_then_ok)
    assert mf["verdict"] == "PASS"
...
def test_t9_budget_terminal_noresume(tmp_path):
    ...
    mf = _run("/tmp/opencode/b2g_fake_overrun", w, clk, _overrun,
              max_blocks=1)
    assert mf["verdict"] == "FAIL(budget)"
    assert mf["partial"] is True
    rows = json.loads(w.store["/tmp/opencode/b2g_fake_overrun/rows.json"])
    assert len(rows) == 1 and rows[0]["status"] == "overrun"
    root = str(tmp_path / "b2g_fake_budget_noresume")
    os.makedirs(root, exist_ok=True)
    with open(f"{root}/manifest.json", "w") as fh:
        fh.write(w.store["/tmp/opencode/b2g_fake_overrun/manifest.json"])
    with open(f"{root}/rows.json", "w") as fh:
        fh.write(w.store["/tmp/opencode/b2g_fake_overrun/rows.json"])
    with pytest.raises(c.Refusal):
...
def test_t9_malformed_decode_output_refuses():
    ...
    with pytest.raises(c.Refusal):
        _run("/tmp/opencode/b2g_fake_nope", MemWriter(), FakeClock(),
             _no_pe, max_blocks=1)
...
def test_t10_resume_arm_mismatch_refuses(tmp_path):
    root = str(tmp_path / "b2g_fake_resume_swap")
    w1, clk1 = MemWriter(), FakeClock()
...
def test_t11_no_writes_outside_root_and_csv():
    assert c._allows_root("workspace/b2g_deadbeef")
    ...
    w, clk = MemWriter(), FakeClock()
    root = "/tmp/opencode/b2g_fake_nowrite"
...
def test_t12_manifest_labels(arm, de_default):
    ...
    mf = _run(f"/tmp/opencode/b2g_fake_labels_{arm}", w, clk, _pe826,
              arm=arm, max_blocks=4)
    assert mf["campaign"] == "B2G-soft-marginal-replication"
    assert mf["arm_role"] == c.ARMS[arm]["role"]
    # Second construction instance label (packet §3 deviation): NEVER
    # "byte-identical paired construction".
    inst = mf["construction_instance"]
    assert inst["source_arm"] == c.ARMS[arm]["source_arm"]
    assert "second construction instance 2026092011 (O1R R2 precedent)" \
        in inst["note"]
    assert "byte-identical paired construction" not in inst["note"]
    assert "NOT" in inst["note"] and "different construct seed" \
        in inst["note"]
    ...
    assert "b2f" in mf["seeds"]["paired_note"]
    assert "NO independence claim" in mf["seeds"]["paired_note"]
...
def test_t12_de_label_override_and_refusal():
    w, clk = MemWriter(), FakeClock()
    mf = _run("/tmp/opencode/b2g_fake_delabel", w, clk, _decode_ok,
    ...
    with pytest.raises(c.Refusal):
        _run("/tmp/opencode/b2g_fake_delabel_bad", MemWriter(),
...
def test_t13_dry_block_probe_real_bundle_fake_kernel(monkeypatch):
    ...
    print(f"DRY u1_mismatches (report-only) = {out['u1_mismatches']}")
    print(f"DRY packet §1 parity anchor (genie H_L2*1024) = "
          f"{c.GENIE_L2_CONTENT_BITS:.4f}; H_full*1024 = "
          f"{c.CONTENT_BITS:.4f}")
    # Paired-draw consistency: block seed 2026095601 + o1_blk: stream is
    # construct-seed-independent, so the b2g dry probe measures the SAME
    # prior entropy as the b2f dry probe (853.0211 b, report-only).
