from comparison_bench.src.comparison_bench.formal_ir.nonbinary_field import GF2mField
from comparison_bench.src.comparison_bench.formal_ir.nonbinary_v19_osd import gf_rref, solve_with_free, osd_decode


def test_solve_small_system():
    field = GF2mField.create(4)
    # H = [[1,1,0],[0,1,1]] over GF(4), syndrome [a,b] chosen from a known x.
    x = [1, 2, 3]
    H = [[1, 1, 0], [0, 1, 1]]
    s = []
    for row in H:
        acc = 0
        for coeff, val in zip(row, x):
            acc = field.add(acc, field.mul(coeff, val))
        s.append(acc)
    rref, pivots = gf_rref(field, H, s)
    free_cols = [j for j in range(3) if j not in set(pivots)]
    assign = {j: x[j] for j in free_cols}
    sol = solve_with_free(field, rref, pivots, assign, 3)
    assert sol == x


def test_osd_decode_returns_solution():
    field = GF2mField.create(4)
    H = [[1, 1, 0], [0, 1, 1]]
    x = [1, 2, 3]
    s = []
    for row in H:
        acc = 0
        for coeff, val in zip(row, x):
            acc = field.add(acc, field.mul(coeff, val))
        s.append(acc)
    sol = osd_decode(field=field, matrix=H, syndrome=s, e_hat=x)
    assert sol == x


def test_osd_decode_candidates_returns_at_least_one():
    from comparison_bench.src.comparison_bench.formal_ir.nonbinary_v19_osd import osd_decode_candidates
    field = GF2mField.create(4)
    H = [[1, 1, 0], [0, 1, 1]]
    x = [1, 2, 3]
    s = []
    for row in H:
        acc = 0
        for coeff, val in zip(row, x):
            acc = field.add(acc, field.mul(coeff, val))
        s.append(acc)
    cands = osd_decode_candidates(field=field, matrix=H, syndrome=s, e_hat=x,
                                  order=1, top_info=1, max_candidates=10)
    assert len(cands) >= 1
    assert x in cands


def test_osd_decode_candidates_order2():
    from comparison_bench.src.comparison_bench.formal_ir.nonbinary_v19_osd import osd_decode_candidates_order2
    field = GF2mField.create(4)
    H = [[1, 1, 0], [0, 1, 1]]
    x = [1, 2, 3]
    s = []
    for row in H:
        acc = 0
        for coeff, val in zip(row, x):
            acc = field.add(acc, field.mul(coeff, val))
        s.append(acc)
    cands = osd_decode_candidates_order2(field=field, matrix=H, syndrome=s, e_hat=x,
                                         top_info=2, top_symbols=4, max_candidates=20)
    assert x in cands


def test_osd_decode_candidates_fast_matches_small():
    from comparison_bench.src.comparison_bench.formal_ir.nonbinary_v19_osd import osd_decode_candidates_fast
    field = GF2mField.create(4)
    H = [[1, 1, 0], [0, 1, 1]]
    x = [1, 2, 3]
    s = []
    for row in H:
        acc = 0
        for coeff, val in zip(row, x):
            acc = field.add(acc, field.mul(coeff, val))
        s.append(acc)
    fast = osd_decode_candidates_fast(field=field, matrix=H, syndrome=s, e_hat=x,
                                      top_info=None, max_candidates=100)
    assert x in fast
    # Compare count with old full enumeration on this tiny system.
    old = __import__('comparison_bench.src.comparison_bench.formal_ir.nonbinary_v19_osd', fromlist=['osd_decode_candidates']).osd_decode_candidates(
        field=field, matrix=H, syndrome=s, e_hat=x, order=1, top_info=1000, max_candidates=100)
    assert len(fast) == len(old)


def test_osd_decode_candidates_order2_fast_matches_small():
    from comparison_bench.src.comparison_bench.formal_ir.nonbinary_v19_osd import osd_decode_candidates_order2_fast
    field = GF2mField.create(4)
    H = [[1, 1, 0], [0, 1, 1]]
    x = [1, 2, 3]
    s = []
    for row in H:
        acc = 0
        for coeff, val in zip(row, x):
            acc = field.add(acc, field.mul(coeff, val))
        s.append(acc)
    fast = osd_decode_candidates_order2_fast(field=field, matrix=H, syndrome=s, e_hat=x,
                                             top_info=2, top_symbols=4, max_candidates=100)
    assert x in fast


def test_osd_decode_candidates_order3_fast_small():
    from comparison_bench.src.comparison_bench.formal_ir.nonbinary_v19_osd import osd_decode_candidates_order3_fast
    field = GF2mField.create(4)
    H = [[1, 1, 0], [0, 1, 1]]
    x = [1, 2, 3]
    s = []
    for row in H:
        acc = 0
        for coeff, val in zip(row, x):
            acc = field.add(acc, field.mul(coeff, val))
        s.append(acc)
    fast = osd_decode_candidates_order3_fast(field=field, matrix=H, syndrome=s, e_hat=x,
                                             top_info=2, top_symbols=4, max_candidates=100)
    assert x in fast


def test_osd_decode_candidates_order4_fast_small():
    from comparison_bench.src.comparison_bench.formal_ir.nonbinary_v19_osd import osd_decode_candidates_order4_fast
    field = GF2mField.create(4)
    H = [[1, 1, 0], [0, 1, 1]]
    x = [1, 2, 3]
    s = []
    for row in H:
        acc = 0
        for coeff, val in zip(row, x):
            acc = field.add(acc, field.mul(coeff, val))
        s.append(acc)
    fast = osd_decode_candidates_order4_fast(field=field, matrix=H, syndrome=s, e_hat=x,
                                             top_info=2, top_symbols=4, max_candidates=100)
    assert x in fast


def test_osd_decode_candidates_fast_generic_small():
    from comparison_bench.src.comparison_bench.formal_ir.nonbinary_v19_osd import osd_decode_candidates_fast_generic
    field = GF2mField.create(4)
    H = [[1, 1, 0], [0, 1, 1]]
    x = [1, 2, 3]
    s = []
    for row in H:
        acc = 0
        for coeff, val in zip(row, x):
            acc = field.add(acc, field.mul(coeff, val))
        s.append(acc)
    fast = osd_decode_candidates_fast_generic(field=field, matrix=H, syndrome=s, e_hat=x,
                                              order=2, top_info=2, top_symbols=4, max_candidates=100)
    assert x in fast


def test_osd_decode_candidates_mrb_returns_solution():
    from comparison_bench.src.comparison_bench.formal_ir.nonbinary_v19_osd import osd_decode_candidates_mrb
    field = GF2mField.create(4)
    H = [[1, 1, 0], [0, 1, 1]]
    x = [1, 2, 3]
    s = []
    for row in H:
        acc = 0
        for coeff, val in zip(row, x):
            acc = field.add(acc, field.mul(coeff, val))
        s.append(acc)
    cands = osd_decode_candidates_mrb(field=field, matrix=H, syndrome=s, e_hat=x,
                                      order=1, top_info=None, top_symbols=None,
                                      max_candidates=100)
    assert x in cands


def test_osd_decode_candidates_mrb_order2_small():
    from comparison_bench.src.comparison_bench.formal_ir.nonbinary_v19_osd import osd_decode_candidates_mrb
    field = GF2mField.create(4)
    H = [[1, 1, 0], [0, 1, 1]]
    x = [1, 2, 3]
    s = []
    for row in H:
        acc = 0
        for coeff, val in zip(row, x):
            acc = field.add(acc, field.mul(coeff, val))
        s.append(acc)
    cands = osd_decode_candidates_mrb(field=field, matrix=H, syndrome=s, e_hat=x,
                                      order=2, top_info=2, top_symbols=4,
                                      max_candidates=100)
    assert x in cands


def test_osd_decode_candidates_bounded_weight_small():
    from comparison_bench.src.comparison_bench.formal_ir.nonbinary_v19_osd import osd_decode_candidates_bounded_weight
    field = GF2mField.create(4)
    H = [[1, 0, 0], [0, 1, 0], [0, 0, 1]]
    x = [1, 2, 3]
    s = []
    for row in H:
        acc = 0
        for coeff, val in zip(row, x):
            acc = field.add(acc, field.mul(coeff, val))
        s.append(acc)
    cands = osd_decode_candidates_bounded_weight(
        field=field, matrix=H, syndrome=s, max_weight=3, max_candidates=100)
    assert x in cands
