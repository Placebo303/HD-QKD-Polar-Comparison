...
import numpy as np
from comparison_bench.src.comparison_bench.cli import run_ldpc_v4_16db_transfer_qualification as lane
from comparison_bench.src.comparison_bench.cli import verify_ldpc_v4_16db_transfer_qualification as verify
...
class TestTransfer16dB(unittest.TestCase):
 def setUp(self):
  self.root=Path(os.environ.get("FORMAL_IR_TEST_TMP","workspace"))/f"v4_16db_transfer_{uuid.uuid4().hex}";self.root.mkdir(parents=True)
  ...
  self.lock={"schema":"binary_ldpc_v4_16db_source_lock_v1","datasets":[],"selected_frames":rows};self.lock["lock_sha256"]=lane._sha(lane._compact(self.lock))
 def tearDown(self):shutil.rmtree(self.root,ignore_errors=True)
 def fake_synthetic(self,*a,**kw):return copy.deepcopy(self.syn)
 def fake_prior(self,s):return set(),set()
 ...
 def test_prior_real_plan_binding_drift_and_prior_real_plan_appears_after_prepare(self):
  ...
  prior_doc={'run_id':lane.RUN_ID,'plan_sha256':'prior'};own_doc={'run_id':lane.RUN_ID,'plan_sha256':'own'};(prior/'pre_run_plan.json').write_bytes(lane._compact(prior_doc));(own/'pre_run_plan.json').write_bytes(lane._compact(own_doc))
  with patch.object(lane,'PREREQUISITE',synthetic):
   self.assertEqual(lane._existing_real_plans('own'),[{'path':str(prior.resolve()/'pre_run_plan.json'),'sha256':lane._sha((prior/'pre_run_plan.json').read_bytes())}])
  out=self._executed();ps=self._patches()
