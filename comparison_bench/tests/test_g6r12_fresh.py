"""R12 FRESH staged tests — FAKE fixtures only (never real pool).

R12b readiness: zero real-payload decoder calls; no UUID root; temp fake
roots; real R11 CSV touched READ-ONLY (frame/call counts only, never
symbols) for the honest-baseline import check. Covers: T-W0 frozen
constants/ARGV; T-W1 fresh-window admission (accept 2187..2250; reject
R9 window/V55 blocks/short window/foreign session); T-A1 stage
accounting (S0/S1/S2-success/exhausted -> 564/604/664/664; sum-over-stages
FAILs); T-A2 staged-increasing + accepted-non-final FAIL; T-A3
undetected-at-Sk global STOP + undetected-merge FAIL; T-R refusal/
no-overwrite/verifier; T-B honest-baseline import from the R11 root
(zero fixed calls, byte-identical); T-M deterministic replay +
admission predicate on fake prefix slices.
"""
...
RUNNER_PATH = ROOT / "scripts" / "g6r12_fresh.py"


def _load_runner():
    spec = importlib.util.spec_from_file_location("g6r12_fresh_test",
                                                  str(RUNNER_PATH))
    assert spec is not None and spec.loader is not None
    mod = importlib.util.module_from_spec(spec)
    sys.modules["g6r12_fresh_test"] = mod
...
@pytest.fixture()
def fake_root():
    path = ROOT / "workspace" / ("g6r12_fake_%s" % uuid.uuid4().hex)
    assert not path.exists()
    assert "c97777aa" not in str(path)
    try:
        yield path
    finally:
...
def _fake_mother(seed):
    H = np.zeros((g12.M_MOTHER, g12.N), dtype=np.uint8)
    for v in range(g12.N):
        H[v % g12.M_MOTHER, v] = 1
    prefixes = {int(m): {"m": int(m), "rows": (0, int(m)),
                         "matrix": H[:int(m), :].copy()}
                for m in g12.STAGE_MS}
    return {"arm": g12.ARM, "width": g12.N, "graph_seed": int(seed),
            "n": g12.N, "m": g12.M_MOTHER, "E": int(np.count_nonzero(H)),
...
def _fake_pre_execute(registry, session, frames, arm, prior):
    assert registry == g12.FROZEN_REGISTRY
    assert session == g12.FROZEN_SESSION
    assert frames == g12.FROZEN_FRAMES
    assert arm == g12.ARM
    assert prior == g12.FROZEN_PRIOR_ROOT
    return list(range(2187, 2251))
...
def _fake_adapters(actions, calls, ms_log):
    ...
    def fake_decode(H, priors, syn, **kw):
        ...
        action = plan.pop(0)
        if action == "hit":
            return _FakeResult(np.zeros(g12.N, dtype=np.int64))
        return _FakeResult(np.ones(g12.N, dtype=np.int64))
    ...
    def fake_build(seed):
        assert int(seed) in g12.MOTHER_SEEDS
...
def _run_script(actions, out_dir, tag_fn=None):
    ...
    bundle = g12.run_authorized_batch(
        str(out_dir), g12.FROZEN_REGISTRY, g12.FROZEN_SESSION,
        g12.FROZEN_FRAMES, g12.ARM, g12.FROZEN_PRIOR_ROOT,
        g12.BASELINE_ROOT, adapters=adapters,
...
def _assert_stage_order(bundle, ms_log):
    ...
    assert list(g12.STAGE_MS) == [100, 108, 120]
    ...
    for block in bundle["block_records"]:
        ...
        expect = list(g12.STAGE_MS[:int(block["stages_attempted"])])
    ...
    flat: list[int] = []
    for block in bundle["block_records"]:
        flat.extend(list(g12.STAGE_MS[:int(block["stages_attempted"])]))
    assert ms_log == flat  # decoder saw exactly the staged prefixes


# T-W0: frozen constants + ARGV lock (PREREG values).
def test_w0_frozen_constants_and_argv():
    _assert_no_production()
    assert g12.FROZEN_REGISTRY == "v71_data_registry.json"
    assert g12.FROZEN_SESSION == "20260107_PPLN_1p5M"
    assert g12.FROZEN_FRAMES == "2187..2250"
    assert g12.ARM == "L020"
    assert g12.FROZEN_PRIOR_ROOT == \
        "workspace/v72p2d5_model_f_input/20260907_r1"
    assert g12.BASELINE_ROOT == \
        "workspace/g6r11_adaptive_3c2b5b2e-897b-467e-a3e6-0cba005914ae"
    assert g12.FUTURE_ROOT == \
        "workspace/g6r12_fresh_c97777aa-620d-448f-8a6c-8abda5fb3b48"
    assert g12.MOTHER_SEEDS == (2026094722, 2026094723)
    assert tuple(g12.STAGE_MS) == (100, 108, 120)
    assert (g12.MAX_ITER, g12.DAMPING) == (90, 1.0)
    assert (g12.SCI_CEILING, g12.SETUP_CEILING) == (384, 8)
    assert g12.PAIRED_LIFT_GATE == 4
    assert g12.FROZEN_COMMAND == (
        ".venv/bin/python scripts/g6r12_fresh.py --execute-real "
        "--execution-authorized --registry v71_data_registry.json "
        "--session 20260107_PPLN_1p5M --frames 2187..2250 --arm L020 "
        "--prior-root workspace/v72p2d5_model_f_input/20260907_r1 "
        "--baseline-root "
        "workspace/g6r11_adaptive_3c2b5b2e-897b-467e-a3e6-0cba005914ae "
        "--out-dir workspace/g6r12_fresh_c97777aa-620d-448f-8a6c-8abda5fb3b48")


# T-W1: fresh-window admission (pure gates; no real reads).
def test_w1_accept_fresh_window():
    _assert_no_production()
    plan = g12.build_call_plan(list(range(2187, 2251)))
    assert len(plan) == 128
    assert [p["frame_id"] for p in plan[:2]] == [2187, 2187]
    assert [p["frame_id"] for p in plan[-2:]] == [2250, 2250]
    assert [p["graph_seed"] for p in plan[:4]] == \
        [2026094722, 2026094723, 2026094722, 2026094723]
    assert all(p["stages"] == [100, 108, 120] for p in plan)


def _gate(frames_spec="2187..2250", session=g12.FROZEN_SESSION):
    return g12._pre_execute_check(
        g12.FROZEN_REGISTRY, session, frames_spec, g12.ARM,
        g12.FROZEN_PRIOR_ROOT)


def test_w2_reject_r9_window():
    _assert_no_production()
    with pytest.raises(ValueError, match="PRE_EXECUTION_BLOCKED"):
        _gate("2123..2186")
    with pytest.raises(ValueError, match="exactly frames 2187..2250"):
        g12.build_call_plan(list(range(2123, 2187)))


def test_w3_reject_v55_blocks():
    _assert_no_production()
    # 64-frame spec starting at V55 start 2119 (overlaps 2119..2122 block).
    with pytest.raises(ValueError, match="PRE_EXECUTION_BLOCKED"):
        _gate("2119..2182")
    # The V55 4-frame block proper.
    with pytest.raises(ValueError, match="PRE_EXECUTION_BLOCKED"):
        _gate("2119..2122")
    # 64-frame spec starting at V55 start 2295 (overlaps 2295..2298 block).
    with pytest.raises(ValueError, match="PRE_EXECUTION_BLOCKED"):
        _gate("2295..2358")


def test_w4_reject_short_window():
    _assert_no_production()
    with pytest.raises(ValueError, match="PRE_EXECUTION_BLOCKED"):
        _gate("2187..2249")
    with pytest.raises(ValueError, match="exactly frames 2187..2250"):
        g12.build_call_plan(list(range(2187, 2250)))


def test_w5_reject_foreign_session():
    _assert_no_production()
    with pytest.raises(ValueError, match="PRE_EXECUTION_BLOCKED"):
        _gate(session="20260123_1M_600k_0dB")


# T-A1: stage accounting — final-prefix disclosure, never summed.
def test_a1_s0_success_564(fake_root):
    _assert_no_production()
    out = fake_root / "s0"
    bundle, calls, ms_log = _run_script(["hit"] * 128, out)
    g12.write_batch_root(bundle)
    s = bundle["summary"]
    ...
    assert s["baseline_F"] == 87 and s["paired_lift"] == 41
    assert s["lift_gate_pass"] is True
    _assert_stage_order(bundle, ms_log)
    assert g12.verify_root(str(out)) is True


def test_a1_s1_success_604(fake_root):
    _assert_no_production()
    out = fake_root / "s1"
...
def test_a1_s2_success_664(fake_root):
    _assert_no_production()
    out = fake_root / "s2"
...
def test_a1_exhausted_664(fake_root):
    _assert_no_production()
    out = fake_root / "exh"
    ...
    assert s["paired_lift"] == -87 and s["lift_gate_pass"] is False
    _assert_stage_order(bundle, ms_log)
    assert g12.verify_root(str(out)) is True


def test_a1_sum_over_stages_fails(fake_root):
    _assert_no_production()
    ...
    def rewrite(value):
        ...
        with (out / "block_records.csv").open("w", newline="",
            ...
            writer = csv.DictWriter(fh, fieldnames=list(g12.BLOCK_COLUMNS))
            writer.writeheader()
            writer.writerows(rows)

    rewrite(564 + 604)
    assert g12.verify_root(str(out)) is False  # partial sum 1168 FAILs
    rewrite(564 + 604 + 664)
    assert g12.verify_root(str(out)) is False  # full sum 1832 FAILs


# T-A2: staged-increasing + accepted-non-final FAIL.
def test_a2_accepted_nonfinal_stage_fails(fake_root):
    _assert_no_production()
    out = fake_root / "nonfinal"
    bundle, _, ms_log = _run_script(["miss", "hit"] * 128, out)
    g12.write_batch_root(bundle)
    assert g12.verify_root(str(out)) is True
    srows = list(csv.DictReader(
        (out / "stage_records.csv").read_text(encoding="utf-8").splitlines()))
    first = [r for r in srows if int(r["block_id"]) == 0]
    assert [int(r["m"]) for r in first] == [100, 108]  # staged-increasing
    first[0]["accepted"] = "True"  # tamper: accepted at non-final stage
    with (out / "stage_records.csv").open("w", newline="",
                                          encoding="utf-8") as fh:
        writer = csv.DictWriter(fh, fieldnames=list(g12.STAGE_COLUMNS))
        writer.writeheader()
        writer.writerows(srows)
    assert g12.verify_root(str(out)) is False


# T-A3: undetected at Sk -> global STOP + retained; undetected-merge FAIL.
def test_a3_undetected_stop_retained(fake_root):
    _assert_no_production()
    out = fake_root / "undet"
    ...
    g12.write_batch_root(bundle)
    assert g12.verify_root(str(out)) is False  # retained-but-FAIL
...
def test_a3_undetected_merge_fails(fake_root):
    _assert_no_production()
    out = fake_root / "merge"
    ...
    rows = list(csv.DictReader(
        (out / "block_records.csv").read_text(encoding="utf-8").splitlines()))
    rows[0]["undetected"] = "True"  # tamper: merge undetected into exact
    with (out / "block_records.csv").open("w", newline="",
                                          encoding="utf-8") as fh:
        writer = csv.DictWriter(fh, fieldnames=list(g12.BLOCK_COLUMNS))
        writer.writeheader()
...
# T-R: refusal / no-overwrite / verifier.
def test_r1_refusal_missing_grant_zero_calls(fake_root):
    _assert_no_production()
    rc = g12.main(["--execute-real", "--registry", g12.FROZEN_REGISTRY,
                   "--session", g12.FROZEN_SESSION, "--frames",
                   g12.FROZEN_FRAMES, "--arm", g12.ARM, "--prior-root",
                   g12.FROZEN_PRIOR_ROOT, "--baseline-root",
                   g12.BASELINE_ROOT,
                   "--out-dir", str(fake_root / "refuse")])
    assert rc == 2


def test_r2_refusal_existing_root_zero_calls(fake_root):
    _assert_no_production()
    ...
    with pytest.raises(FileExistsError):
        g12.run_authorized_batch(
            str(target), g12.FROZEN_REGISTRY, g12.FROZEN_SESSION,
            g12.FROZEN_FRAMES, g12.ARM, g12.FROZEN_PRIOR_ROOT,
            g12.BASELINE_ROOT,
...
def test_r3_no_overwrite():
    _assert_no_production()
    import comparison_bench.formal_ir.v72p2d10_mixed_degree_l1 as r2
    with pytest.raises((ValueError, FileExistsError)):
        r2.refuse_out_root("results/g6r12_fake")
    with pytest.raises((ValueError, FileExistsError)):
        r2.refuse_out_root("comparison_bench/outputs_comparison/g6r12_fake")
    for forbidden in [ROOT / "src", ROOT / "experiments", ROOT / "tools"]:
        if forbidden.exists():
            hits = list(forbidden.rglob("*g6r12*"))
    ...
    for prior in [ROOT / "workspace/g6r9_confirm_3f9a1c2e-7b4d-4e8a-9c1f-2d5e6a7b8c9d",
        ...
        if prior.exists():
            hits = list(prior.rglob("*g6r12*"))
            assert hits == [], "prior root touched: %r" % hits
    assert not (ROOT / g12.FUTURE_ROOT).exists()  # UUID root absent


def test_r3_verifier_beta_tamper_fails(fake_root):
    _assert_no_production()
    out = fake_root / "beta"
    bundle, _, _ = _run_script(["hit"] * 128, out)
    g12.write_batch_root(bundle)
...
# T-B: honest-baseline import from the R11 root (read-only; zero calls).
def test_b1_honest_baseline_import_r11():
    _assert_no_production()
    calls: list = []
    base = g12.load_fixed_baseline(g12.BASELINE_ROOT)
    assert calls == []
    assert base["F"] == 87
    assert base["successes"][:2] == [(0, 2123), (1, 2123)]
    assert len(base["successes"]) == 87
    assert base["rows"] == 128
    raw = Path(g12.BASELINE_ROOT, "block_records.csv").read_bytes()
    assert hashlib.sha256(raw).hexdigest() == base["sha256"]
    base2 = g12.load_fixed_baseline(g12.BASELINE_ROOT)  # byte-identical
    assert base2["sha256"] == base["sha256"] and base2["F"] == 87
    assert calls == []


def test_b2_baseline_shape_rejects_nonpaired(fake_root):
    _assert_no_production()
    ...
    with pytest.raises(ValueError, match="!= 128"):
        g12.load_fixed_baseline(str(root))


# T-M: deterministic replay + admission predicate on fake prefix slices.
def test_m1_deterministic_replay_fake_mother():
    _assert_no_production()
...
def test_m1_admission_predicate_prefix_slices():
    _assert_no_production()
    ok, _ = g12.check_nested_prefixes(_fake_mother(2026094722))
    assert ok is True
    bad_missing = _fake_mother(2026094722)
    del bad_missing["prefixes"][108]
    ok, reason = g12.check_nested_prefixes(bad_missing)
    ...
    ok, _ = g12.check_nested_prefixes(bad_rows)
    assert ok is False  # non-nested slice rejected
    # Out-of-pair mother seed refused (construction gate).
    with pytest.raises(ValueError, match="outside frozen R12 pair"):
        g12.build_mother(2026094720)
    # Reused pair disjoint from the frozen ban set (numeric proof).
    banned = ({2026094601, 2026094602}
              | set(range(2026094701, 2026094709))
              | set(range(2026094711, 2026094720))
              | {2026094720, 2026094721})
    assert not (set(g12.MOTHER_SEEDS) & banned)
    assert g12.verify_seed_disjointness()["passed"] is True
...
def test_r16_net_secret_bits_staged_frozen_tamper(fake_root):
    ...
    assert g12.net_secret_bits(1, 564, g12.N) == 76
    assert g12.net_secret_bits(1, 604, g12.N) == 36
    assert g12.net_secret_bits(1, 664, g12.N) == -24
    assert g12.net_secret_bits(0, 664, g12.N) == -664
    ...
    # Mixed R12-shape 3/15/61/49: 79*640 - 83792 = -33232.
    assert g12.net_secret_bits(79, 3 * 564 + 15 * 604 + 61 * 664 + 49 * 664, g12.N) == -33232
    # Beta unchanged on one bundle (recompute identically).
    g12.write_batch_root(bundle)
    s = bundle["summary"]
    leak = s["disclosure_sum"]
    assert s["beta_eff_empirical_primary"] == 1.0 - leak / (128 * 128 * g12.H_FROZEN)
    assert g12.verify_root(str(out)) is True
    summary = json.loads((out / "summary.json").read_text(encoding="utf-8"))
