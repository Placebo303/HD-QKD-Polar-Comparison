"""Route C formal qualification lane tests: fake qualification, tamper
rejection, strict replay, boundaries — mirroring the v5b lane suite."""
...
from comparison_bench.src.comparison_bench.formal_ir import nonbinary_v5c_ir_qualification as q

def _out(name):
    root = Path("workspace") / "nbldpc_v5c_ir_tests" / uuid.uuid4().hex / name
...
def test_three_level_failed_lane_emits_full_transcript():
    ...
    for policy in ("nbldpc_v5c_sched", "nbldpc_v5c_ems"):
...
def test_selection_prefers_verified_candidate_policy(monkeypatch):
    ...
    doc = json.loads((out / "formal_policy_manifest.json").read_text())
    assert doc["selection"]["selected_policy"]["policy_id"] == "nbldpc_v5c_sched"
...
def test_fake_runner_does_not_fall_back_to_production(monkeypatch):
    from comparison_bench.src.comparison_bench.formal_ir import nonbinary_v5c_decoders as core
    out = _out("trap")
    q.create_plan(out, _test_only=True)
