...
from comparison_bench.src.comparison_bench.formal_ir import nonbinary_v5b_ir_qualification as q

def _out(name):
    root = Path("workspace") / "nbldpc_v5b_ir_tests" / uuid.uuid4().hex / name
...
def test_three_level_failed_lane_emits_full_transcript():
    ...
    ir56_p30 = [r for r in rows if r["policy_id"] == "nbldpc_v5b_ir56" and r["stratum_p"] == "0.3"]
    assert all(r["extension_used"] == "True" and r["final_prefix"] == "56" and r["stage3_iterations"] == "1"
               for r in ir56_p30)
    ir48_p30 = [r for r in rows if r["policy_id"] == "nbldpc_v5b_ir48" and r["stratum_p"] == "0.3"]
...
def test_selection_prefers_verified_candidate_policy(monkeypatch):
    ...
    def selective(state, mats, stage):
        return success(state, mats, stage) if state.policy_id == "nbldpc_v5b_ir56" else failed(state, mats, stage)
    out = _out("selective")
    q.create_plan(out, _test_only=True)
    q.run(out, runner=selective, _test_only=True)
    doc = json.loads((out / "formal_policy_manifest.json").read_text())
    assert doc["selection"]["selected_policy"]["policy_id"] == "nbldpc_v5b_ir56"
...
def test_fake_runner_does_not_fall_back_to_production(monkeypatch):
    from comparison_bench.src.comparison_bench.formal_ir import nonbinary_v5b_multistage as core
    out = _out("trap")
    q.create_plan(out, _test_only=True)
