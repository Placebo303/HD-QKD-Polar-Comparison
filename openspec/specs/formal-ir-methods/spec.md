...
# Formal LDPC v2 Improvement (merged 2026-07-26)
...
The v2 rate margin, decoder method, and decoder order SHALL be selected from
exactly nine pre-registered policies using sacrificed development frames only:
rate margins `{0,1,2}` crossed with `(OSD_0,0)`, `(OSD_CS,1)`, and
`(OSD_CS,2)`. One global policy SHALL be frozen before confirmation.
...
plane, screen all 32/40/48/56-row prefixes, select one master per plane, and
retain a reproducible selection audit. Structural unique-syndrome probes are
not verified decoding evidence.

## Requirement: Promotion Before Real Use

V2 SHALL NOT receive a real-data lock unless fresh synthetic qualification
...
The verified fresh v2 result is `non_promoted`: 28/32 at p=.01 and 29/32 at
p=.02. No real LDPC lock or run is authorized, and confirmation data SHALL NOT
