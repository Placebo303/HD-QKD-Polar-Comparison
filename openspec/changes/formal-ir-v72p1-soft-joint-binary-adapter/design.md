...
**禁令**：`SHALL NOT` 任何 `decode_* / construct_*_business / gf_rank_business / nested_business` 业务码；`SHALL NOT` 调 `Q/N/M/f/tag/col/LF/mother/Rs/leak`；`SHALL NOT` 新生成 mother（删 `V72P1新seed` 新 seed，必须精确复用 V72P0 `nnz=49620 indptr/indices相等`）；`SHALL NOT` 引 `MET/protograph/SC/Gray`（除 adapter 注释）；`SHALL NOT` 读 2M/TEST；`SHALL NOT` 启动 V72；`SHALL NOT` 执行 `run_ldpc_formal_v5`；`SHALL NOT` 改 `src/`；`SHALL NOT` 用 `1pct容差` 容差（nnz 精确 49620）。
...
configs: (k=2,n=6)→total_bits 6 exhaustive 64 (k=3,n=9)→total_bits 9 exhaustive 512 各8 trials 种子0..7
  ...
  附加: tiny 经 adapter 的 syndrome+tag exact (S6/S8) 亦需 PASS，落盘 worst/wall/c3_observed + 10条消息 finite
```
...
P1C 真消息传递 3 档 (SYNTHETIC_ONLY，真消息传递，非空 smoke，不判收敛)：
  ...
  落盘 per iter {wall_s, peak_MiB, finite, maxLLR, residual, per_invocation_ns, nnz=49620, row_deg, col_deg, CSR_bytes, 10数组分项, syndrome_inc_ok, tag_ok, disclosed}
```
...
### 5.8 边/内存/复杂度 10数组分项 float 复杂度探针（落盘，R72P1-06）

```
edges:
  nnz = H_mother.nnz = 49620 精确值（无1pct容差容差，精确复用 V72P0 indptr/indices相等）
...
complexity (ponytail: O(nnz + N*Q*10) 已显式):
  ...
  per LF 1024态枚举 10条消息 O(N*Q*10) = 1024*1024*10 = 10,485,760 logsumexp 操作
  ...
  # ponytail: O(nnz + N*Q*10) 10数组 已显式，per-account packs if throughput matters 升级路径已标
```
...
```
if not synthetic_bits_ok or not log_prior_finite or not H_mother_exists or not indptr_indices_equal:
    ...
    classification = ADAPTER_PLAN_READY; successor = v72p1_adapter_integration  # 需 4 硬 AND 全 PASS，P1D 仅描述，AND gate 精确（非 OR_SYM）
else:
    classification = MATRIX_SMOKE_FAIL  # 兜底 first-match
# P1D loopy 仅 capacity_warning 正交旗标，不过硬门禁，但报告单独章节；无 OR_SYM，AND gate 精确；无 1pct容差容差
```
...
if all cases ADAPTER_PLAN_READY and smoke_counts[ADAPTER_PLAN_READY]==total_cases:
    ...
    overall = NOT_READY
# audit = {per_case_classification[total], overall, 5 counts, per_case P1A/P1B/P1C/P1D, edges nnz49620 indptr/indices相等, memory 10数组, wall, finite/maxLLR/residual, f1.3 NOT_MEASURED, P1D capacity_warning}
```
