...
## 1. 变更类型与生命周期
- **Type**: `SYNTHETIC_ADAPTER_MINIMAL` `10步 S1-S10 + 3类型 SoftJointConfig 8字段/Result/Adapter + 5公式 + P1A plumbing / P1B tiny 64/512 8trials tree-only 1e-9 c3观测 / P1C 真消息传递 1/3/10 iter 报告finite/maxLLR/residual 9036×10240 nnz49620 精确复用 / P1D small loopy n12 k3 描述性 + 11数组 memory O(nnz+N*Q*10) + 5态 first-match AND`
...
- `Rs_Δ r0 160 Δ8 max9036 r∈{160,168,...,9032,9036} 与 checkpoint 72批量 r_checkpoint∈{160,288,...,8992,9032,9036} max9036 上限 分离 报告disclosed`
...
### 5.4 Adapter 3 类型 SoftJointConfig 8字段 11数组
...
`H_mother 9036×10240 nnz49620 精确 indptr/indices相等 sparse CSR IRA dual-diagonal C1 rank9036 C2 row_min>0 C3 dup0 C4 prefix indptr相等 C5 incremental Δ8 C6 tag pivot checkpoint72 checkpoint 160,288,416,8992,9032,9036 72批量 disclosed 全PASS ∧ 1 iter wall<1s finite==true ∧ 3 iter <5s ∧ 10 iter ≤30s ∧ peak≤2048MiB ∧ syndrome增量/tag exact 三档全PASS 报告 finite/maxLLR/residual ⇒ P1C_PASS AND`
...
`01 精确复用 mother nnz49620 indptr/indices相等 删新 seed / 02 5公式 / 03 SoftJointConfig 8字段 / 04 Δ8与 checkpoint 分离 72批量 9036 max disclosed / 05 P1C 真消息 1/3/10 finite/maxLLR/residual / 06 11数组 memory 分项 float O(nnz+N*Q*10) / 07 5态 first-match AND 无OR SYM / 08 机械修正 无1 pct容差 保持未完成 / 09 本轮只四工件 / 10 SYNTHETIC_ONLY不创run_01不改V70/V72P0不启V72 精确复用`
...
后续 `scripts/v72p1_backend_audit.py` `AdapterSoftJointConfig 8字段/Result/Adapter 3类型 + S1-S10 + 11数组 + 5态 IntEnum` `rg "\"ADAPTER_PLAN_READY\"" 0` `py_compile PASS` 产 `audit_report/BACKEND_REPORT` 下一轮
本轮证据 `proposal/design/tasks/specs` 四工件已修订 `head TBD` `EXTERNAL_BINDING` `no_run_01` `V72_not_started` `本轮只四工件`

## 10. 验收 本轮只四工件 保持未完成
`py_compile PASS` `rg "V72P1 新seed" 0 hits` `rg "旧 nnz" 0 hits` `rg "1 pct容差" 0 hits` `rg "OR SYM" 0 hits` `rg "decode" 0 hits` `rg "\"ADAPTER_PLAN_READY\"" 0` `git diff src==0` `git diff V70==0` `git diff V72P0==0` `used_2m false used_test false` `T_LF01-08 1e-12` `SoftJointConfig 8字段 {checkpoint_rows, max_iter_per_checkpoint, max_total_iterations, llr_clip, convergence_tol, warm_start, dtype, tag_bits}` `P1A pack/prefix/incremental/tag indptr/indices相等 wall≤30` `P1B 64/512 8trials c3 observed_zero&&observed_one 负向测试 worst<1e-9 tree wall≤30 11数组` `P1C nnz49620 indptr/indices相等 zero0 dup0 checkpoint 160,288,416,8992,9032,9036 disclosed 1/3/10 wall<1/5/30s finite==true syndrome增量tag exact 报告finite/maxLLR/residual` `P1D n12 k3 cycle1 capacity_warning 描述性` `edges nnz49620 精确 CSR≈278192B 11数组≈9.2MiB peak<2GiB O(nnz+N*Q*10)` `5-state first-match 4 硬 AND wall≤30 finite==true P1D不过门禁 无OR SYM` `overall ADAPTER_PLAN_READY/NOT_READY 5counts` `T0-T3 true` `S1-S10 10步 3类型8字段 11数组` `Δ8 vs checkpoint 72批量 disclosed` `V72_not_started` `TBD 0` `报告与json一致` `四工件已修订 保持未完成 本轮只四工件`
