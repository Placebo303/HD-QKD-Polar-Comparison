...
- 目标：在小 q 上证明"位面分解/多位信道 + 对应 DE 更新"机制与文献/
  解析锚点一致。
- **锚点 A（内部一致性，冻结）**：q=4（2-bit 分解，p_1 = p_2 = p 退化
  情形）——位面分解 DE（每平面二元 BSC(p) 独立 DE + 联合收敛判定）与
  符号级 QSC(p) DE 的阈值差 **≤ 0.005**（同 n_samples、同 max_iter、
  同种子协议下逐点比较）。
- **锚点 B（文献交叉，冻结）**：q=4、R=0.75、QSC 的符号级 DE 阈值 vs
  发表值 0.069，**|δ| ≤ 0.012**（直接复用 V8-60/V14 Stage 0 已验证的
  锚点与复现路径——同一 q=4 R=0.75 系综、n_samples=1e5、max_iter=150、
  熵≤0.01×20 迭代协议）。
- PASS 判据（冻结）：**锚点 A 且锚点 B 全过**（无布尔 fallback）。
- FAIL：任一锚点不过 → 机制不可信 → 门冻结（不进入 Stage 1/2）。
...
- 候选族（预注册 3–5 个，不搜索、执行前冻结）：
  ...
  - 每个候选冻结其 (λ, ρ) 或机制参数、评估点集；**评估点集固定为
    m∈{15,16,17,18} 全集**（与 V14 同协议；不允许执行前/后收窄子集，
    任何收窄视为 amendment 需主线程授权）。
