...
Pre-registered closeout contingency: if the COMPLETE n=1024 window yields no
passing family (n=1024 failure already establishes global `finite_graph_fail`
regardless of n=2048), the n=2048 window is still started but may be closed
after a bounded n=2048 prefix (at least one source, any number of blocks) when
extreme non-converging L2 decoder runtime would otherwise exhaust the resource
meter. The n=2048 prefix is then documented as diagnostic-only; the global
terminal is `finite_graph_fail`. The verifier accepts this bounded-prefix
closeout only when the persisted n=2048 evidence contains at least one block
and the recomputed terminal is `finite_graph_fail` (already fixed by n=1024).
