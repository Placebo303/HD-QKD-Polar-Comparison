...
record registration completeness (full window required for n=1024; for the
bounded-prefix closeout contingency, the n=2048 window needs only at least one
persisted block and an `finite_graph_fail` terminal already fixed by n=1024),
per-source FER/syndrome/waterfall/failure positions, and terminal equality. It MUST return `ok=true` and `problems=[]` for
