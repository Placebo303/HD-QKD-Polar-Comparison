...
**HEAD**: `01b4493f8c78f5033b9259efdd4a0e4e99f8189e` (动态绑定 `git rev-parse HEAD`; `origin/formal-ir-mainline == HEAD` 40位重核，不一致阻塞；predecessor V71/V70 数值只读，不改)
...
6. **5终态 wall first-match (REDO: P0A k=2/3 BP vs brute 7cover, 8trials; P0B sparse CSR IRA 9036x10240; provenance synced 01b4493f) + overall 2态**：每 synthetic `EVIDENCE > MODEL > BACKEND_NOT_COMPATIBLE > TINY_FAIL > MATRIX_RANK_FAIL > SYNDROME_NESTED_FAIL > READY > ADAPTER` 互斥；overall `READY(全READY)/ADAPTER_OR_FAIL` + `8正交计数`。
