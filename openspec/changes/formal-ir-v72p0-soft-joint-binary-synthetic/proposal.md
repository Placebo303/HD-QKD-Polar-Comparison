# OpenSpec Proposal: REDO V72P0 — retain LOCAL_FACTOR_KERNEL_PASS; P0A k=2/3 BP vs brute 7cover no true self-compare; P0B sparse CSR IRA 9036x10240; 5-state ADAPTER_PLAN_READY needs 3 passes

# OpenSpec Proposal: formal-ir-v72p0-soft-joint-binary-synthetic

**Status**: `PLAN_CANDIDATE / SYNTHETIC_ONLY / EXECUTE_NOT_AUTHORIZED` — 仅计划四工件 + 合成校验链条 correctness（1024→10bit local factor ↔ binary LDPC mother BP incremental syndrome exact 64-bit tag），冻结 Q1024 N1024 Nbit10240 M2 required9036 f1.3 f_actual NOT_MEASURED 2M禁止，local factor 去 self-message LLR定义 8测试，backend 只读审计6问分三态（不字符串判READY），synthetic P0A tiny 2-4 symbols exact + P0B 1024 synthetic mother 9036×10240 nested rank，5终态 wall first-match (REDO: P0A k=2/3 BP vs brute 7cover, 8trials; P0B sparse CSR IRA 9036x10240; provenance synced b360efe9)，T0-T3 矩阵，不跑 decoder 不改 V70/V70R1/V71
...
**HEAD**: `b360efe9828484c1022c90c5a14819ebbb9dadcb` (动态绑定 `git rev-parse HEAD`; `origin/formal-ir-mainline == HEAD` 40位重核，不一致阻塞；predecessor V71/V70 数值只读，不改)
...
### 4. synthetic P0A tiny 2-4 symbols exact
- **范围**：synthetic 合成域 tiny 块 `N_small ∈ {2,3,4} symbols`（`Nbit_small ∈ {20,30,40}`），`Q=1024` 保持，每 `N_small` 各 `num_trials=64` 合成样本，信道为已知 synthetic 先验 `P_synth(a|b)`（`δ uniform` + `peak 0` 可配，seed `V72P0-SYN-P0A` 确定性），**exhaustive exact 校验**：枚举 `Q^{N_small}` 全码字空间（2→1M, 3→1B 采样, 4→采样 Monte-Carlo + exact tag/syndrome 小矩阵对照），校验 `local factor 去 self + 小 mother (m_small = ceil(1.3·N_small·CE_synth))` 的 `syndrome = H_small · bit_hat (GF2)` 与 `exact tag` 链条合成 correctness（`syndrome_ok ⇒ tag_ok` 或 `tag mismatch` 精确一致），`wall_s/peak_MiB` 记录。
...
4. **synthetic P0A tiny 2-4 symbols exact**：tiny 合成块 `N_small 2/3/4` exhaustive `Q^{N_small}` 或采样 + 小 `H_small` 增量 syndrome + exact tag `0 mismatch`，`wall≤30s`。
...
6. **5终态 wall first-match (REDO: P0A k=2/3 BP vs brute 7cover, 8trials; P0B sparse CSR IRA 9036x10240; provenance synced b360efe9) + overall 2态**：每 synthetic `EVIDENCE > MODEL > BACKEND_NOT_COMPATIBLE > TINY_FAIL > MATRIX_RANK_FAIL > SYNDROME_NESTED_FAIL > READY > ADAPTER` 互斥；overall `READY(全READY)/ADAPTER_OR_FAIL` + `8正交计数`。
...
- [ ] `proposal/design/tasks/specs/spec.md` 齐全一致，`lifecycle PLAN_CANDIDATE / SYNTHETIC_ONLY / EXECUTE_NOT_AUTHORIZED`，`branch formal-ir-mainline` + `HEAD b360efe9828484c1022c90c5a14819ebbb9dadcb` (动态绑定 `git rev-parse HEAD == origin/formal-ir-mainline` 重核) + `data 84d62779 + synthetic_v72p0` + `predecessor V71 487be113 / V70 9bc34be6 / V70R1 0509d10b` 已绑定，显式声明 SYNTHETIC_ONLY、零 decoder/real 扫描、Q1024 N1024 Nbit10240 M9036 f1.3 NOT_MEASURED 2M禁止、local factor 去 self 8测试、backend 6问三态非字符串 READY、P0A tiny 2-4 exact、P0B 9036×10240 nested rank、5终态 wall first-match (REDO: P0A k=2/3 BP vs brute 7cover, 8trials; P0B sparse CSR IRA 9036x10240; provenance synced b360efe9)、T0-T3、四工件产出。
...
- [ ] **synthetic P0A tiny 2-4 symbols exact 已验**：`N_small 2/3/4` 各 `64 trials` exhaustive（或采样）`0 mismatch`，`syndrome = H_small·bits (GF2)` 与 `tag = SHA256(bits)[:8B]` exact `0 mismatch`，`wall≤30s ∧ peak≤2048MiB`，`v72p0_results.json:P0A` 已验。
