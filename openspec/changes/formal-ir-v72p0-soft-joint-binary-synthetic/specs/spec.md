...
- **Branch**: `formal-ir-mainline`；`HEAD` `b360efe9828484c1022c90c5a14819ebbb9dadcb` (动态绑定 `git rev-parse HEAD`; `git rev-parse HEAD == origin/formal-ir-mainline` 重核，不一致阻塞) 已与 `git rev-parse HEAD` 一致。`TBD` 0 hits。
...
- `P0A tiny N_small 2/3/4 各 64 trials` exhaustive `0 mismatch` 冻结；`wall≤30s peak≤2048MiB` 冻结。
...
N_small 2/3/4 各 64 trials 独立合成，N=1024 mother 仅构造
...
### 5.5 P0A tiny 2-4 symbols exact
...
- **本轮仅 plan 四工件+registry+双脚本+双报告（去 self 8test + Q1-Q6 enum + P0A tiny 2-4 + P0B 9036×10240 + 8term wall + T0-T3）**，任何 `V72` 后续 real mother/decoder 需 `Plan SHA` + `v72p0_data_registry_synthetic.json` 实表 + `T_LF/Q/P0A/P0B/wall/T0-T3` 已验后、且独立 `PLAN_ACCEPT` + `EXECUTE_AUTH` 后才允许创建正式实现并执行；`SYNTHETIC_ONLY` 保持至授权，**V72 保持 NOT_STARTED**。
