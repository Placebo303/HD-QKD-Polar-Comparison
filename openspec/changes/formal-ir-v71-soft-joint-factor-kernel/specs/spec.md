...
- **Branch**: `formal-ir-mainline`；`HEAD` `a047d2a09613616144ad58efe7c4ac8e4ce7acf7` (三头偏差：`dc18fd2f` 初始 → `4afc4eec2a3adfca88a8f79bc9c051e3819405ac` → `a047d2a0` 当前 HEAD，provenance deviation `dc18fd2f/4afc4eec` vs predecessor `9bc34be6`，`git rev-parse HEAD == origin/formal-ir-mainline` 重核，不一致阻塞) 已与 `git rev-parse HEAD` 一致。`TBD` 0 hits。
