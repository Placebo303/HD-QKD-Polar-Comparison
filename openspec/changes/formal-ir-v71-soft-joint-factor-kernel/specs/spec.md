...
- **Branch**: `formal-ir-mainline`；`HEAD` `e038114db5095a57158b0e1cd36884d6a1a5d8be` (动态绑定 `git rev-parse HEAD`; 已修正历史三头偏差 `历史三头` vs predecessor `9bc34be6`，`git rev-parse HEAD == origin/formal-ir-mainline` 重核，不一致阻塞) 已与 `git rev-parse HEAD` 一致。`TBD` 0 hits。
