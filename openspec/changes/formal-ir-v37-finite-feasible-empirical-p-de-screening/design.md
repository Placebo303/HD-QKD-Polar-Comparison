...
**Parent Baseline**: `a258c5a7549f33f43a32b193b134f1ca0df15aea` (V37-P0R1 accepted; provenance in `docs/research_cycles/V37P0/REVIEW_VERDICT.md`)
...
For block length $n=1024$ and check dimensions $m \in \{184, 190, 192\}$, a cycle-free degree-2 subgraph across all three sources requires the necessary condition:
$$N_2 \le \min(m) - 1 = 183$$
...
Because $L_2 \le 183 / 1024 \approx 0.1787$, the average variable degree is theoretically bounded below by:
$$\bar{d}_v = \sum_d d \cdot L_d \ge 2 \cdot L_2 + 3 \cdot (1 - L_2) = 3 - L_2 \ge 2.8213$$
Across the 547 candidates satisfying $N_2 \le 183$, the actual variable degree range is $\bar{d}_v \in [2.8571, 5.0000]$ and total sockets $E \in [2926, 5120]$, which for $m=184$ corresponds to mean check degrees $\bar{d}_c \in [15.9022, 27.8261]$. $N_2 \le 183$ alone does not bound check degrees $\le 20$.

### 1.2 Additional Pre-Registered Design Cap: Maximum Check Degree
To prevent excessively high check degrees while accommodating $\bar{d}_v \ge 2.82$, we pre-register an explicit search cap on realized maximum check degree:
$$d_{c,\max} \le 20$$
evaluated using P0's exact integer floor/ceil check-degree allocation algorithm across all three sources ($m \in \{184, 190, 192\}$).

### 1.3 Pre-Registered Candidate Universe
- Variable degrees: $d_v \in \{2, 3, 4, 5\}$, $\sum \lambda_d = 1.0$ on a $0.05$ grid.
  ...
  - Filter 2 (Check Realizability Cap): $d_{c,\max} \le 20 \implies$ **259 pre-registered candidates**.
...
1. **Primary Ranking Metric**: Area Under the Entropy Trajectory ($\text{AUT}_{30}$):
   ...
   Lower $\text{AUT}_{30}$ indicates lower cumulative early-iteration entropy (not pointwise strict inequality).
2. **Secondary Diagnostic Metrics**:
   ...
   - Paired relative difference $\Delta_s = \frac{\overline{\text{AUT}}_{30}(s, C) - \overline{\text{AUT}}_{30}(s, B)}{\overline{\text{AUT}}_{30}(s, B)}$.
3. **Effect-Size Threshold**:
   - The threshold of $\ge 5\%$ reduction is pre-registered as a practical development effect-size gate.

### 2.3 Reference Controls
- **Matched Baseline**: Regular $d_v=2$ ($\lambda = \{2: 1.0\}$) run with identical $N=4000, I=60$, screening and confirmation seeds.
...
### 3.1 Two-Stage Advance Gate
A candidate $C^*$ passes the V37-P1 DE gate if and only if:
1. $N_2(C^*) \le 183$ and $d_{c,\max} \le 20$ (P0 feasibility and design cap passed).
...
4. In **P1-B Confirmation** on fresh disjoint seeds, for **every source** $s \in \{\text{1M}, \text{1p5M}, \text{2M}\}$:
...
If multiple candidates pass screening:
...
### 3.3 Terminal States
- `P1_DE_ADVANCE_CANDIDATE_FOUND`: Best candidate satisfies per-source $\ge 5\%$ reduction in screening and confirmation.
- `P1_SCREEN_SIGNAL_NOT_CONFIRMED`: Screening passed but confirmation failed on fresh seeds.
- `P1_NO_FINITE_FEASIBLE_DE_ADVANCE`: No candidate passing $N_2 \le 183, d_{c,\max} \le 20$ meets the screening threshold. Closes the tested candidate set.
