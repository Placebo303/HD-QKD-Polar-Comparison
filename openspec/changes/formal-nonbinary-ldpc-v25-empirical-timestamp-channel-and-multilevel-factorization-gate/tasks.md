...
- [x] **I01** labeling L01/L02 实现与可逆性（T01）。
- [x] **I02** factorization F01–F05 实现与可逆重构（T02/T03）。
- [x] **I03** 数据切分器（60/20/20 时间序、frame 不重叠、T04）+ split_manifest。
- [x] **I04** delta 诊断（signed/modular 分离 T05；run 不跨边界 T06）。
- [x] **I05** conditional count 表 N[A,B,Z(+labels)] 与 chain-rule 计算（T07）。
- [x] **I06** 模型家族 C01–C06 实现（预测 NLL、条件熵、calibration、zero-prob、
  per-source）。
- [x] **I07** delay/alignment 门（source-dependent，锁 delay）。
- [x] **I08** factorization gate（顺序规则、H_i、R_i_ref、复杂度代理）。
- [x] **I09** 输出层：channel_summary/delta_by_source/gray_joint_masks/
...
- [x] **I10** Alice-oracle 访问检测（T10）、holdout 不回写（T11）、verifier 重算
  terminal state（T12）。

## T — tests (T01–T12)

- [x] **T01** natural/Gray labeling 可逆。
- [x] **T02** F01–F05 分层可逆。
- [x] **T03** 所有分层重构回原始 0–1023。
- [x] **T04** train/validation/holdout 无 frame 重叠。
- [x] **T05** signed delta 与 modular delta 不混淆。
- [x] **T06** run 不跨文件/frame。
- [x] **T07** chain rule 在 tiny synthetic joint table 上闭合。
- [x] **T08** 公开 residual 会增加 leakage 的负例。
- [x] **T09** Bob-full 与 Bob-coarse 条件信道不可混用。
- [x] **T10** Alice oracle 访问检测。
- [x] **T11** holdout 参数不可回写 train model。
- [x] **T12** 输出 verifier 从原始统计重算 terminal gate。

## M — scientific stages (authorized only after P0/P1 ACCEPT)

- [x] **M001** 生成 data_inventory.json。
- [x] **M0** 时间戳误差图谱（16 项指标，按 source/file/time block）。
- [x] **M1** 模型比较 C01–C06（validation + sealed holdout；per-source + pooled 补充）。
- [x] **M2** ±1 结构 / 方向 / 时间稳定性分析（既有 delay 配置，不重估 delay、不读
  .ttbin）；方向随 source/delay_used_ps 变化视为待建模 channel 特征。
- [x] **M3** multilevel factorization gate（F01–F05 × L01–L02；chain rule 闭合；
  R_i_ref；复杂度代理）。
- [x] **M4** 产出供 V26 的信道候选：**高域候选 GF(512)/GF(256)** + **中域对照
...
- [x] **V001** 独立只读 verifier：从 counts+frozen config 重算主要指标与 terminal
  state；ok=true/false + 问题列表。
- [x] **V002** 决策写入 decision-log；更新 CURRENT_TASK/AGENT_HANDOFF/
  AGENT_PROJECT_MEMORY。
- [x] **V003** PASS 仅允许提出 V26（不得自动启动）；FAIL/BLOCKED 停止并保留全部证据。
- [x] **V004** 本地提交，不 push。
