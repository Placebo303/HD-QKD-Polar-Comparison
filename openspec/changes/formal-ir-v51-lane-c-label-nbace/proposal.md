...
**Domain**: Formal IR / Lane C label-only NB-ACE optimization
...
1. **对 V50 三源 Lane C ordinal-2 support**（`lane_c_1M_s383102 / lane_c_1p5M_s383202 / lane_c_2M_s383302`）各执行**一次确定性 NB-ACE 标签优化**（优先改善 6-环，再改善 8-环；词典序目标：`degenerate_6 ↓ → min NB-ACE ↑ → degenerate_8 ↓`；只运行一次确定性优化，不根据译码结果搜索 seed）。
2. **decoder-free 输出原标签 vs 新标签**的：`support_exact_equal`, `rank`, `4/6/8 支撑环数量不变`, `algebraically-degenerate 6/8 环数量`, `NB-ACE/generalized-girth 谱`。三源中至少一源在 `degenerate_6` 或 `NB-ACE` 上明确改善，才具备进入实验资格。
...
4. **若标签优化无法改善代数环谱，则不进入实验，直接报告 blocker**（`V51_LABEL_NO_IMPROVEMENT`）。
...
2. **确定性 NB-ACE 标签优化器**：输入为各源二值 support，二值支撑 4/6/8 环枚举（`enumerate_canonical_simple_cycles`）+ 代数退化分类（`classify_cycle_algebraic_degeneracy`）+ 行度基 ACE（`ACE(c)=Σ_{check∈cycle}(row_deg-2)`）→ NB-ACE（degenerate 惩罚 `-100`）；目标词典序 `(deg6, -min_nbace6, deg8, -min_nbace8, cand)`；按 canonical 边序 greedy 扫描 `1..31`，至多 2 sweeps，零随机回搜；support/秩/`4/6/8` 数量不变由构造保证。
3. **decoder-free 谱对比**：每源输出 `support_exact_equal (=True), rank==m2, support_cycles_4/6/8 不变, degenerate_4/6/8, min_nbace6/8, min_deg_ace6/8, generalized_girth (=min degenerate length), nondeg_frac6/8` 原 vs 新并列。
4. **条件准入**：三源至少一源满足 `degenerate_6_new < degenerate_6_old` 或 `min_nbace6_new > min_nbace6_old`（以惩罚后 min_nbace 计），否则 `V51_LABEL_NO_IMPROVEMENT` blocker，不建 45-call 实验。
...
- **未来实现（本轮不创建）**：`comparison_bench/src/comparison_bench/formal_ir/v51_lane_c_label_nbace.py`（仅组合 Lane C 重建 + 确定性 NB-ACE 重标 + 45-call paired runner，不新增 decoder）+ `scripts/execute_v51_label_nbace.py`；均直接 import `v38_architecture_triage` (Lane C) 与 `v35_algorithm_development::compute_tag_64`。
...
- [ ] 四工件 + spike 脚本/报告齐全一致且 lifecycle 为 `PLAN_CANDIDATE / EXECUTE_NOT_AUTHORIZED`，明确“不实现不执行，等待独立评审；若谱无改善则 blocker 不实验”。
- [ ] 三源 Lane C support/置换/`m2=184/190/192`/`90/1.0` 冻结不可调，仅标签可变可校验。
- [ ] 确定性 NB-ACE 优化器冻结：词典序 `deg6 ↓, min_nbace ↑, deg8 ↓`，优先 6 环再 8 环，一次确定性运行、禁止根据译码结果搜 seed；decoder-free 谱对比字段冻结（support_equal/rank/4/6/8 不变/deg 6/8/NB-ACE/girth）。
- [ ] Spike 可行性已实证（decoder-free 三支撑重建 + 三标签重标，报告 rank/support/cycles/deg/NB-ACE/girth 原 vs 新；若三源均无 `deg6` 或 `min_nbace6` 改善则 verdict 为 `BLOCKER`）。
...
- [ ] 禁止清单冻结：禁止改 support/置换/m2/decoder、禁止第二候选、禁止译码驱动的 seed 搜索、禁止创建正式 output。

## Tasks

见 `tasks.md`（Phase A 冻结支撑/泄漏/decoder 与单次确定性 NB-ACE 优化器；Phase B 仅 fake-runner 聚焦测试；Phase C decoder-free preflight；Phase D 需 EXECUTE_AUTH 的至多 45 calls paired；Phase E 结果复核；显式禁止清单）。

## Lifecycle

V50 前代 `V50_FACTORIAL_COMPLETE` (E_structure=-6, E_prior=0)，分支 `formal-ir-mainline` plan HEAD 绑定规划 SHA（执行时重绑至未来 implementation SHA）；V51 当前 `PLAN_CANDIDATE / EXECUTE_NOT_AUTHORIZED`，`implementation_started=false`，`production_outputs_created=false`；任何执行需独立 plan ACCEPT + 显式 `EXECUTE_AUTH` 绑定到精确未来实现 SHA；谱无改善时终态 `V51_LABEL_NO_IMPROVEMENT`，否则条件实验终态 `V51_PAIRED_COMPLETE / V51_EVIDENCE_INVALID`。
