...
**Feasibility**: 三源 Lane C support 184/190/192×1024 decoder-free 可重建且满秩；确定性 NB-ACE 重标 decoder-free 可执行且保持支撑/秩/4/6/8 不变，仅改变退化谱；V50 已验证 held-out 池 1683 frames / 430k pairs 富余；45-call paired 预算冻结

## 1. 科学问题（单因子标签谱）

> 在**相同 support/置换/m2/泄漏/decoder**下，**确定性 NB-ACE 边标签重标**能否改善 Lane C 的代数退化环谱（优先 6-环），并在配对 held-out 块上体现 `exact_full` 增益？

- **因子**：`Lane C 原标签` vs `Lane C 新标签`（同支撑、同学置换、同 m2/泄漏/decoder，仅 `1..31` 标签不同）。
- **谱目标**：词典序 `degenerate_6 ↓`（主）、`min NB-ACE ↑`（次，degenerate 惩罚后最小值）、`degenerate_8 ↓`（三）；6-环优先于 8-环。
- **准入**：三源至少一源在 `deg6` 或 `min_nbace6` 上改善，才进入 decoder 实验；否则 `V51_LABEL_NO_IMPROVEMENT` blocker。
...
### 2.2 确定性 NB-ACE 标签优化器（decoder-free，单次）

- **输入**：各源二值 support `B=(H≠0)`，行度 `row_deg`，枚举 `c4/c6/c8 = enumerate_canonical_simple_cycles(B)`，`edge→cycle` 索引。
- **ACE 定义**：`ACE(C)= Σ_{check∈C} (row_deg(check)-2)`（对 `dv=2` 正则，纯 check 外向度）。支撑冻结故 `ACE` 仅由拓扑决定，标签无关。
- **代数退化**：`is_deg(C,H)= classify_cycle_algebraic_degeneracy(C,H)`（`rank < |C|` → 退化）。`deg6= Σ is_deg(c6)` 等。
- **NB-ACE 定义**：`NB-ACE(C)= ACE(C) -100` 若 `is_deg(C)`，否则 `ACE(C)`。退化惩罚保证任何退化环 `min NB-ACE` 低于非退化环。备选等价：`min_deg_ace = min{ ACE(C) | is_deg(C)}` 用作次级指标可互换报告。
- **Generalized girth**：`g = min{ len(C) | is_deg(C)}`（4/6/8/None）；`nondeg_frac = (support-deg)/support`。
- **优化目标（词典序，越小越好）**：`key = (deg6, -min_nbace6, deg8, -min_nbace8, cand)`，其中 `min_nbace6 = min{NB-ACE(C)| C∈6}`，无退化时取 `min ACE`；6-环优先于 8-环。
- **算法**：canonical 边序 `get_canonical_support_edges(B)`，对每条边依次试 `1..31`，增量评 `incident cycles` 的 `is_deg` 变化，计算候选 `key`，选最小 `key`（tie 最小 `cand`）；至多 `max_sweeps=2`，早停于零更新；全程确定性、零随机、`SeedSequence` 不参与（仅保留支撑生成时的 perm tie-break 已冻结）。
- **约束**：`support_exact_equal`, `rank==m2`, `support_cycles_4/6/8 不变`；`4-环退化`仅报告（`deg4` 支撑 41/5 不变，其 deg 仅标签影响但非优先）。
- **禁止**：根据译码 `exact`/`errors_final` 回搜标签或多 seed 择优；标签优化为一次 decoder-free 过程。
...
| 字段 | 冻结 |
|---|---|
| `support_exact_equal` | `True` |
| `rank` | `==m2` 原/新一致 |
| `support_cycles_4/6/8` | 不变 |
| `degenerate_4/6/8` | 标签决定，需报告 `Δdeg6, Δdeg8` |
| `min_nbace6/8, min_deg_ace6/8, generalized_girth, nondeg_frac6/8` | NB-ACE 谱 |

三源中至少一源 `Δdeg6<0` 或 `Δmin_nbace6>0` 为准入条件。
...
- **Lane C 新**：`lane_c_1M_s383102_nbace / lane_c_1p5M_s383202_nbace / lane_c_2M_s383302_nbace` 同 support 确定性重标（`spike_label_nbace.py` 复现）。
...
谱准入：`label_improved = ∃source: deg6_new < deg6_old ∨ min_nbace6_new > min_nbace6_old`。否则 `V51_LABEL_NO_IMPROVEMENT`，不建 45-call。
...
1. else if !label_improved -> V51_LABEL_NO_IMPROVEMENT (blocker, no decoder)
...
2. **科学 preflights（decoder-free, write-free）**：lane C 三 support 确定性重建与 committed `rank/support_cycles_4/6/8` 比对；NB-ACE 重标 decoder-free 重建与 `support_exact_equal/rank/cycles 不变` 校验；谱准入 `label_improved` 布尔；TRAIN counts 形态；新 15 held-out 未使用池可达且与 FORBIDDEN 156 及 V48/V50 `frame_ids` 零重叠；首块哨兵 `392001/392101/392201` 各 `L1→P_i(U2)` 通路 + `tag_import_ok` + `leakage_accounted` + `tag_scope_l2_only`。
...
```
Summary 含：谱对比表（原 vs 新 per source：rank/support/cycles/deg/NB-ACE/girth）、记账 `planned 45/l1 15/l2 30`、`30` 条 L2 聚合（overall/per-source）、配对表 `old vs new exact_full`、McNemar `b/c/discordance`、残留误码与 runtime 分布、四类计数、`f_total`、claim boundary、provenance（含 NB-ACE 定义与一次确定性重标 ID 与新 held-out 溯源）。
...
断言边界 verbatim：结果仅支持 `n=1024, m2=184/190/192` 上 Lane C 同支撑同 m2 同泄漏同 decoder `90/1.0` 的单次确定性 NB-ACE 标签重标（`deg6 ↓, min_nbace ↑, deg8 ↓` 优先 6 环，禁止译码回搜）在 15 新未使用 held-out 块上的配对有界归因（`exact_full` oracle 不经 tag；L2-only tag `≈2^-64` 工程近似），`support/秩/4/6/8 支撑数不变`；`exact_full` paired 为主判，无 TRAIN+VAL 臂；均非真帧 FER 全集/阈值/SKR/资格/晋升证据。
...
```
文件：`v51_records.json/.csv` (30 L2 行)、`v51_summary.json`、`v51_invalid_notice.json`（失败时）、`v51_label_spectrum.json` (decoder-free 谱原 vs 新)。CSV/JSON 行对等；禁写 NPZ。

## 10. 实现草图（后继轮次，当前未授权）

- `comparison_bench/src/comparison_bench/formal_ir/v51_lane_c_label_nbace.py`：import `construct_lane_c_prototype` 常量与 `v35.compute_tag_64`，实现 §2.2 确定性 NB-ACE 重标（`enumerate_canonical_simple_cycles` + `classify_cycle_algebraic_degeneracy` + ACE + 贪心 2-sweep）+ 15 新 held-out paired runner (per block `1 L1+2 L2`)。
- `scripts/execute_v51_label_nbace.py`：默认拒绝；`--execution-authorized --authorized-target-sha <sha>`；HEAD/origin 精确绑定未来实现 SHA；四文件 SCOPED dirty；谱准入 `label_improved` 布尔门；budget 硬帽 45；任一 gate 失败非零退出。
- 仅 fake-runner 测试；不以译码结果定标签。

## 11. 自由裁量 D1–D10

- D1 单次确定性 NB-ACE 标签重标（同支撑 184/190/192，词典序 deg6/-min_nbace/deg8）。
- D2 谱对比字段 freeze（support_equal/rank/4/6/8 不变/deg/NB-ACE/girth）。
- D3 准入 `deg6` 或 `min_nbace6` 改善，否则 blocker 不实验。
...
- D10 文件集 `30` 行记录 + `label_spectrum.json` + 聚合 + 配对。
