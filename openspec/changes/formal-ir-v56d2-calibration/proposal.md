...
**Revision**: `V56D2R1` — 终态由 `FAIL_NEED_FIX` 更正为 `CALIBRATION_EVIDENCE_INCOMPLETE / ENVIRONMENT_AND_CONTRACT_NOT_REPRODUCED`（TimeTagger 缺失致 timing 未重算，缺证据非证伪；`frac_B≥40%`不合理，1M/2M 原始 `36.4%` 来自探测效率非 routing 错误证明）
**Domain**: Formal IR / V56D2 calibration (V56D1 唯一后继)
**Change ID**: `formal-ir-v56d2-calibration`
**Cycle ID**: `V56D2R1` calibration (predecessor `V56D1` `formal-ir-v56d1-raw-a2-diagnosis`)
...
3. **仅 decoder-free 验证，满足双合同才算校准通过（V56D2R1 修正）**：
   - **时间对齐合同**：`timing_contract_verified` — 必须真实重算 `TimeTagger` 环境下 `peak_center/σ/p2bg`（`V56D1 成功读取 TimeTagger 的同解释器/同版本环境`，记录 `interpreter_path/TimeTagger版本/输入事件数`），`peak` 健康（窄 `σ 50-150ps`）且 `|peak - delay_used|<50ps` 且 `sign一致` 且 `peak_to_bg` 兼容性已评估且 `gate/threshold/frame_start` 已显式落盘；**缺失时只能 `EVIDENCE_INCOMPLETE`，不得判 `FAIL_NEED_FIX` 证伪**；
   - **通道路由合同（R1 修正）**：不再强制 `frac≥40%` 硬门，改为 `指定 channel 存在且非零（count_A>0 && count_B>0 && unique⊇{1,5}）且配对无跨 channel/丢列/错误合并（other<20% 检查配对完整性）` — 比例仅报告（`1M/2M 36.4%` 可能来自探测效率非 routing 错误）；避免修 delay 后仍错 mapping 但不以比例一刀切证伪；
   - **V13 复用**：直接复用 `V13 已验证 channel/delay/peak/gate/frame-start/mapping/pairing` 实现（`workspace/v13r3fresh_20260816/sidecars/*/sidecar_meta.json: used_params`），不重发明近似物化；
   - **基础相关性**：`A==B >60%` 在 calibration 小批量上（8 帧 `[7,8,9,10,15,16,17,18]`）回升（从 V55 `27-41%`），`V25 prior` 下 `NLL` 从 `28-35` 向 `0.82` 靠近、`q_mass` 从 `57-71%` 向 `3%` 回落；**timing 完整仍 `NLL 20-30` 才是真域问题**；
...
2. **Raw TTBin 显式重算与落盘**（decoder-free，V56D2R1 环境锁定）：固定使用 `V56D1 成功读取 TimeTagger 的同环境`（记录 `interpreter_path、TimeTagger 版本、输入事件数`），由 `v55_authoritative_registry.json:provenance` 解析三源双文件 `*.ttbin+*.1.ttbin`，直接复用 `V13 已验证 read_ttbin_events + compute_cross_correlation_histogram(ch_a=1,ch_b=5,bin100/max_lag819200/n16384, lag=t_B-t_A)` 实现重算 `peak_center/σ/p2bg/delay_sign`（不重发明近似物化），按源声明唯一 `delay_used -50/+50/-50`（不择优）落盘 `sidecar_meta.json: used_params={delay_used_ps, peak_center_ps, peak_sigma_ps, corr_argmax, corr_bins, peak_to_bg, peak_status, channels, gate_width_ps, pairing_threshold_ps, frame_start_ps, frame_anchor, mapping, wrap_rule, frame_period_ps, occupancy_filter}` + `provenance/sha256/merge_verified` + `environment{interpreter, timetagger_version, total_events}`；`timing` 字段必须真实重算，缺失时只能 `EVIDENCE_INCOMPLETE`
3. **Calibration frames 预注册**（零重叠，可机械校验）：每源固定 `8 frames [7,8,9,10,15,16,17,18] = 2 blocks×4` 连续帧（仍可用相同 8 帧，写入 additive `run_02` 或修订轮目录不覆盖），`pairs_per_frame 256`，`frame_id` 全部落在 `[0, F-1]` 且与 V55 `selected_frame_ids` 零交集；脚本校验 `gap≥4` 与 `set ∩ ==∅`，输出 `calibration_registry.json` + `calibration_manifest.json`
4. **Decoder-free 双合同验证**（timing + routing，R1 修正）：`timing_contract_verified (|peak-delay|<50 && sign && σ 50-150 && gate/threshold/frame_start 已落盘，缺失→EVIDENCE_INCOMPLETE)` + `routing 指定 channel 存在且非零且无跨 channel/丢列/错误合并（other<20%），比例仅报告不再强制各占≥40%` + `A==B>60%` + `NLL/q_mass` 向 V13 回落 + `p2bg>1000` 分源兼容性评估（`708/629/378` 现状下不得硬阈一刀切）；校正后仍要求 `A==B>60%` 且 `NLL/q_mass` 回落，若 timing 完整仍 `NLL 20-30` 才是真域问题
...
- [ ] **双合同验证**可复现（R1）：`timing_contract_verified`（缺失→`EVIDENCE_INCOMPLETE`非证伪）+ `routing 指定 channel 存在且非零且无跨 channel/丢列/错误合并（比例仅报告，不再强制≥40%）` 双门已逐源判定；`A==B>60%` 在 calibration `8 frames [7,8,9,10,15,16,17,18]` 小批量上已核验；`p2bg>1000` 已作分源兼容性评估（`708/629/378` 现状下不硬套共阈，报告 `per-source HEALTHY/PARTIAL/LOW` 与是否需分源阈值）；**环境已锁定 `V56D1 同解释器/TimeTagger版本/输入事件数`**
- [ ] `V25 prior` 下 `NLL` 从 `28-35` 向 `~0.82` 回落、`q_mass_on_p_zero` 从 `57-71%` 向 `3%` 回落已逐源报告；若 timing 完整仍 `NLL 20-30` 则为真域问题
- [ ] `v56d2_calibration.py` 为 decoder-free 可运行脚本（`rg "decode_" 0 hits`），仅 `numpy/pandas/pyarrow + ttbin_pipeline(TimeTagger optional)`，直接复用 `V13 channel/delay/peak/gate/frame-start/mapping/pairing` 实现，输出 `v56d2_calibration.json`（保留）+ `v56d2_calibration_run02.json` additive + 控制台摘要，`py_compile` PASS；不覆盖 `run_01`
- [ ] `CALIBRATION_REPORT.md` 已记录每源落盘值/双合同/相关性/NLL/p2bg 分源评估与总体结论，终态为 `CALIBRATION_EVIDENCE_INCOMPLETE / ENVIRONMENT_AND_CONTRACT_NOT_REPRODUCED`（缺 TimeTagger 时），数据与 json 一致，结论不扩大为 FER/阈值/SKR/晋升，明确原 90 已揭盲不可复用
- [ ] 已推送新 SHA 并停留在 `DIAGNOSIS_PLAN_READY / DECODE_FORBIDDEN`，未创建任何 `.../v56d2_*/run_01` decoder 执行，不碰 V55 90 块
