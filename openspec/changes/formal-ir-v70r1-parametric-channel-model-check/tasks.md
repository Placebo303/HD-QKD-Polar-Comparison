...
- [x] **F2 补齐** `DEVELOPMENT_RESULT_CANDIDATE / DECODER_FREE_EXECUTION_COMPLETE`：写入 `4SHA 0509d10b/179916f7/99e6b25f/36d493d8` `f_actual=NOT_MEASURED` `decoder_calls=0` `used_test=false` `V72_NOT_STARTED=true`，REPORT 扩逐源×三模型 9字段完整表 + `terminal_priority_note` + `PRE_RESULT_ORDERING_DEVIATION`，新增6项测试，推送。
...
- [x] **G2** `terminal_priority_note` + `PRE_RESULT_ORDERING_DEVIATION`：`terminal_priority_note` 记录 `CHANGES(1) precedes REDUCES(2)`，`PRE_RESULT_ORDERING_DEVIATION=true` 显式标记 ordering vs 计数最多者 deviation（CHANGES 1 vs REDUCES 2）
