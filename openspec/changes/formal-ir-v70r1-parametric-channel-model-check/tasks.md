...
- [x] **A3 SHA 绑定**：`HEAD == origin/formal-ir-mainline == 13b38b79` 已核（`accepted_plan_sha` frozen `13b38b79`），`git diff -- src/ == 0` 已核。
...
- `proposal/design/tasks/specs` 一致，HEAD `13b38b79`（`accepted_plan_sha` frozen `13b38b79`，`6bc06d4d..487be113` 区间 latest accepted `487be113` `V71_KERNEL_ADAPTER_FEASIBLE / ADAPTER_REQUIRED` successor `v71_kernel_adapter_design`），Data `84d62779`，lifecycle `PLAN_CANDIDATE / DECODER_FREE / EXECUTE_NOT_AUTHORIZED` + `V72_NOT_STARTED=true`，V70 数值不变。
