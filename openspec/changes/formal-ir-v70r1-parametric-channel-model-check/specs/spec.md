...
见 tasks.md「本变更显式禁止」。要点：不重做输入合同、不读 ttbin、不换处理点、不用 VAL/TEST 择优、不 cap `required`、不改 `1.3` 宣称提效、不声称 CE 分解、不报 `f_actual`（本轮 `f_actual=NOT_MEASURED`）、不建 `run_01`、不启 V72、不改 `src/`。
