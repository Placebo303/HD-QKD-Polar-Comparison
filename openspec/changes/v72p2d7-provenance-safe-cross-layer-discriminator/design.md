...
## RSS telemetry rework A2 (frozen delta; telemetry only, no science change)

> Authority: `.workbuddy/tasks/D7_E_RSS_TELEMETRY_REWORK_VENV_A2_TASK_PACKET.md`
> §§0–1, 3–5 (T0–T1 freeze). Scientific contract (R08–R13), budgets'
> numeric values, root contract, seven-file schema, and authorization
> lifecycle are untouched. The renewed Pre-EXECUTE review becomes stale
> until A2 implementation review + fresh E09 pass.

On Linux/WSL, current-process peak RSS is read from `/proc/self/status`,
field `VmHWM`, whose unit must be exactly `kB`; convert with
`bytes = value * 1024`.

1. `VmHWM` is authoritative for this WSL execution path. Do not compare
   against or fall back to `ru_maxrss` when `/proc/self/status` is present.
2. Missing file, missing field, duplicate field, malformed/non-integer/
...
3. Do not silently substitute `VmRSS`, `/proc/<pid>/statm`, psutil, shell
   commands, or another process.
4. The limit remains strict `< 2 GiB`; equality or greater is blocked.
   Units and threshold do not change.
5. The stored scalar key remains `rss_bytes`; no new scientific output
   field or schema revision is required.
6. `resource.ru_maxrss` may remain only for non-Linux legacy code if
...
7. A single fresh E09 probe after implementation is evidence. Repeating
   probes until one passes is forbidden.

Parser acceptance: recognize exactly one ASCII `VmHWM: <positive integer>
kB` line; reject missing/duplicate/malformed/decimal/signed/zero/
negative/wrong-unit/non-ASCII cases; overflow rule — reject digit strings
longer than 18 digits (≥10^18 kB is physically impossible; prevents
pathological int parsing). Production read opens `/proc/self/status`
once per probe call: no subprocess/shell/psutil/caching/retries/
averaging/polling/env-switch/provider abstraction.
