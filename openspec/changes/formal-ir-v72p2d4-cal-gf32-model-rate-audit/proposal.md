...
3. 冻结模型族 U/G/F/L（U 参考不参选，G/F/L 参选闭合）+ `M3_EXIT_AMBIGUOUS` + `CIRCULANT_DEFERRED`（本轮直接记录，不再候选）：U 为均匀参考（`P=1/1024`，`CE_L1/CE_L2/CE_joint=5/5/10 bit/symbol` 精确，另记 `uniform_reference_descriptive` 下界对照不参选）；G pin 为 train 边际（`P_G(a)=count_TRAIN(A=a)/n_TRAIN`，与 B 无关经验边际）；F 为全符号经验 `P(A|B)` 单 lambda、30 点 grid 仅 inner 选择；L 为 GF32 两层 `P(U1|B)*P(U2|U1,B)` Ponytail、复用同 outer 折 F 选定 lambda（无独立搜索），目标为 mean joint CE；circulant 变体本轮直接 `CIRCULANT_DEFERRED` 不候选；M3 本轮仍 `M3_EXIT_AMBIGUOUS` 不参选；数学合同与 lambda 反泄漏统一口径：lambda（Laplace/回退/收缩单系数）SHALL 为 30 点 grid 上仅由对应 inner TRAIN 拟合 + inner held-out CE 选中（F 选中、L 复用），禁 outer TEST/VAL/held-out 调参、早停或选模（grid30 outer 禁入），不得窥视 outer TEST。
4. 冻结指标 CE（含 seen/unseen 分解）与链式检查 `|CE_joint-CE_L1-CE_L2_oracle|<1e-10`。
5. 先复现 R5：`6.422 / 5.083 / 11.505`（`CE_L1 / CE_L2_oracle / CE_joint`）；R5 来源为 synthetic 故 `REAL_CAL_EXACT_MATCH=false`，只以同 fixture 复现公式口径核对（同 counts 轴约定 + 同链式 + 同 CE 定义），不作真实 CAL 精确匹配断言；再做 G/F/L 审计（U 仅 5/5/10 参考不参选；M3 本轮已直接 `M3_EXIT_AMBIGUOUS`、circulant 已 `CIRCULANT_DEFERRED`，均不参选）。根因：若复现偏离，先判口径/数据域差异，不得调参凑数。
6. 冻结选择规则：G/F/L 间 fold-mean 最小 + 简单优先（G<F<L）+ 稳定性；U 仅参考不参选。
...
- 模型：U/G/F/L 闭合 + 本轮直接 `M3_EXIT_AMBIGUOUS` + `CIRCULANT_DEFERRED`（冻结，互斥）：U 为均匀参考（`P=1/1024`，`CE_L1/CE_L2/CE_joint=5/5/10` 精确，另记 `uniform_reference_descriptive` 不参选、不进选择与路线）；G pin 为 train 边际（`P_G(a)=count_TRAIN(A=a)/n_TRAIN`，与 B 无关）；G/F/L 同 design §3（F 单 lambda grid30 inner 选、L 复用同折 F lambda）；lambda 统一口径同 Goal 3（grid30 outer 禁入）；M3 无唯一定义直接记录退出、circulant 直接 DEFERRED，审计在 G/F/L 上闭合，不猜测替代。
- 指标：`CE=-mean(log2 P)`（bit/symbol）；seen/unseen 按 held-out 的 Bob 值是否在 train 出现划分，报告覆盖率与分 CE；链式误差 `<1e-10`。
- R5 复现：同 fixture 同口径复算 R5 三数 `6.422/5.083/11.505`，容差 `1e-6`（链式另按 `1e-10`）；R5 来源 synthetic 故 `REAL_CAL_EXACT_MATCH=false`，只核公式复现，不作真实 CAL 精确匹配；失败即 `BLOCKED`并记根因（口径/数据域/实现），不进入 G/F/L 比较。
- 选择：`mean(CE_joint)` 最小为主；接近（`Δ<0.02` bit/symbol，实现前冻结：`SELECT_DELTA=0.02` 冻结不可调，纠正 R1 事后回写违规）时简单优先（G<F<L，U 参考、M3 退出、circulant DEFERRED 均不参选）；稳定性数值门限冻结：`CE_joint` 跨外层折 `std>0.10 bit/symbol 或 max-min>0.20 bit/symbol` 即不稳定，降级并显式标记，不得仅凭单折最优选中。
- 科学分解（冻结）：报告 `U→G`（边际 vs 均匀增益）、`G→F`（全依赖增益）、`F→L`（分层增益）及 `U→选中` 总增益（同口径 bit/symbol，描述性，不作下界/失败定论）。
