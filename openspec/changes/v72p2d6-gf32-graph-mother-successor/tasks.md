...
- [ ] T3 Implement `v72p2d6_gf32_graph_mother.py`: 8 arms, common
  ...
  tables. Reuse D5 by import; no D5 edits.
- [ ] T4 Implement `comparison_bench/tests/test_v72p2d6_gf32_graph_mother.py`
...
- [ ] T5 Implement `scripts/v72p2d6_graph_mother_development.py`: explicit
  `--model-f-root` + `--out-root` (fresh UUID, no formal default), watchdog
  worker (120 s/call, pid log), single-sample-per-`(n, seed)` reuse, six
  evidence files + scalar provenance.
- [ ] T6 Run focused tests, then the frozen eight-file suite with a fresh
...
- [ ] T7 Independent code review (`reviewer-go`, read-only): one-axis
  contract, arm formulas, gates, isolation, accounting, tests →
  `D6_GRAPH_MOTHER_IMPLEMENTATION_REVIEW_R1.md` (PASS required).
- [ ] T8 Independent Pre-EXECUTE review (read-only): scope, frozen
  ...
  Max two implement/review cycles, then BLOCKER.
