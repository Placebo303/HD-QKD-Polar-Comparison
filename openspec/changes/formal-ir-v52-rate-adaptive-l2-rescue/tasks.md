...
**Lifecycle**: `PLAN_REVISE_REQUIRED / EXECUTE_NOT_AUTHORIZED` — **已按阻塞点修订（去重 old 臂、修正泄漏分类），仍不实现不执行 decoder，不写产出。等待独立复审。本轮仅完成 decoder-free 嵌套 spike。**
...
**Delta m**: `Δm=8` per source 冻结（总 30–45 硬帽45，去重后）
...
- [ ] **A2** 冻结增量矩阵 `H_inc` (`Δm=8` per source)：`8×1024 GF32 poly37`、`col_degree_inc∈{0,1}`、`row_degree≤16`、`无零增量行`、`rank_joint==m2+8`、`nested H_base==H_joint[:m2]`、`independence==Δm` (rank 增量)、确定性 PEG-增量 `SeedSequence([60000x,1/2/3])`、禁止 seed 搜索、第二候选与调 `Δm`；`E_inc` 报告、`leak_joint=leak_base+40` 公式冻结、`first_pass_success 已成功帧不增泄漏(1064/1094/1104)，rescued/仍失败均为 leak_joint(1104/1134/1144)`。
...
- [ ] **A5** 冻结 paired workload（old vs V52 去重）：每块同 `bob` 同 `P_i(U2)`，`base L2/pass1` 同一次确定性译码兼作 `old` baseline 与 `V52` pass1（old Lane C 与 V52 pass1 完全同参同算子同输入，结果必然相同，不重复译码，`old_exact = base_exact`）；每块 `L1 1 + base 1 + 条件 rescue ≤1`（L2 1–2）；总 `15 L1 +15 base L2 + ≤15 rescue L2 = 总 30–45 硬帽45 (L2 ≤30)`（已删除此前 `60 L2+15 L1` 或每块≤4 L2 旧预算）；冻结 `paired Δexact = final_V52 - old` 描述性、McNemar `b/c` 仅描述。
- [ ] **A6** 冻结报告承诺 SHALL：`first_pass_success_count / incremental_rescue_count / final_exact_full / per_source (1M/1p5M/2M) / first_pass_success_leak(1064/1094/1104) / rescued_success_leak(1104/1134/1144) / final_failure_leak(leak_joint) / average_leakage(=leak_base+40×N_rescue_attempted/15) / failed_conditional_leakage(leak_joint) / joint_rank+nested+independence / real_tag_acceptance / paired_old_vs_incremental (fresh，去重：old 与 pass1 同一次译码)`；显式 `first_pass_success_leak=leak_base、rescued_success_leak=leak_joint、final_failure_leak=leak_joint、avg_leak=leak_base+40×N_rescue_attempted/15 = (N_first_success*leak_base + N_rescue_attempted*leak_joint)/N (N_rescue_attempted=N - N_first_success)、failed_conditional=leak_joint`；不得再用 `successful_conditional=leak_base` 单一口径。
...
- [ ] **B3** 泄漏公式校验（修正分类）：`leak_base 1064/1094/1104`；`leak_joint 1104/1134/1144`；`avg_leak = leak_base + 40×N_rescue_attempted/15 = leak_base + (1-p1)*40` 可重算（`N_rescue_attempted = N - N_first_success`）；`first_pass_success_leak==leak_base`、`rescued_success_leak==leak_joint`、`final_failure_leak==leak_joint`、`failed_conditional==leak_joint`；校验不得再用 `successful_conditional==leak_base` 旧口径。
...
- [ ] **B8** 预算与配对序校验（去重后）：fake 结构校验每块 `L1 1 + base_shared(兼 old/V52 pass1)1 + 条件 rescue ≤1` 冻结（L2 1–2，总 15 L1+15 base+≤15 rescue=30–45 硬帽45，L2 ≤30，old 与 pass1 同一次译码不重复）；源 1M/1p5M/2M、块升序、块内 `base_shared→(条件)rescue` 序。
- [ ] **B9** 写出合约（去重后）：文件集精确（≤30 L2 行：15 base_shared(兼 old/V52 pass1) + ≤15 rescue，含 `arm∈{base_shared,rescue}/pass_index/used_increment/matrix_id/joint` + `frame_ids/sampling_mode/ordinal` + `leak_total(1064/1094/1104 或 1104/1134/1144)/leak_joint/avg`；总调用 15 L1+15 base+≤15 rescue=30–45 硬帽45；summary 含 `first/rescued/final/old(derived)` 分层 + 每源 + `first_pass/rescued/failure/avg` 泄漏分类 + `f_*` + `joint_rank/nested/independence` + paired `old vs V52(去重)` + 四类/G3' + 终态）、CSV/JSON 行对等、已存在根 fail-closed、禁写 NPZ、provenance 含 `H_inc Δm=8 det1` + fresh held-out 溯源含每块 `frame_ids/ordinal`。
...
- [ ] **D2** 恰好一次：`python scripts/execute_v52_nested_rescue.py --execution-authorized --authorized-target-sha <sha>`；按 design §2.3 去重两遍协议执行：`15 块 × (base 1(兼 old/V52 pass1，同一次确定性译码) + 条件 rescue ≤1)`，每块 `L1 1 + base L2 1 + rescue ≤1`，总 `15 L1+15 base+≤15 rescue=30–45 硬帽45 (L2 ≤30)`；每条 L2 记录含 `arm∈{base_shared,rescue}/pass_index/used_increment/matrix_id/target_tag/candidate_tag/tag_ok/reclassified` (L2-only) 与 `exact_u1/exact_l2/exact_full/errors/syndrome/entropy/||q-p||1/leak_total(1064/1094/1104 或 1104/1134/1144)/leak_joint`；`old` 基准由 `base` 结果确定性复用，不另译码；保留 `run_01`；出错止、原样保留 raw partial 无聚合、返回 blocker；不 rerun/resume/tuning/加块/改增量/重实现 canonical；无论结果不做第二轮；同块 base 与 rescue 同 `bob` 同 `P_i(U2)`；采样恒 deterministic fresh held-out.
- [ ] **D3** 只读 postcheck：记账 `first_pass_success / rescued / final_V52 / old(derived from base)`、`15` 块去重记录与序每块 `base_shared→(条件)rescue` 正确（old 与 pass1 同记录不重复）、增量 `H_inc` provenance、fresh held-out `frame_ids/ordinal` 逐块比对 design §2.4、无 NPZ、V38–V51 输出 byte-identical、summary 完备（含 `first/rescued/final/old` 分层 + 每源 + `first_pass_success_leak/rescued_success_leak/final_failure_leak/avg_leak/failed_conditional` 泄漏分类 + `f_*` + `joint_rank/nested/independence` + paired(去重) + 四类/G3' + `tag acceptance` + 终态 + `leakage_formula(=leak_base+40×N_attempt/15)`）、独立重算.

## Phase E — 结果复核

- [ ] **E1** 写 `OPERATOR_RETURN.md` 与 `DEVELOPMENT_RESULT.md` 候选；lifecycle 保持 result-candidate；含 `first_pass_success / incremental_rescue / final_exact_full / old_exact_full(derived)` (overall & per-source)、`first_pass_success_leak/rescued_success_leak/final_failure_leak/avg_leak(=leak_base+40×N_attempt/15)/failed_conditional_leak`、`joint_rank/nested/independence` 实测、`paired old vs V52 Δexact(去重)` per block 描述性、McNemar `b/c` 仅描述、四类计数、`l1` 执行期诊断上下文真实 `q≠p` 严格在 claim boundary 内（含 L2-only 工程近似与 `leak_base 1064/1094/1104` / `rescued/failure 1104/1134/1144` / `avg_leak=leak_base+40×N_attempt/15` 且 first_pass 已成功帧不增泄漏说明，不宣称 FER/晋升）。
