...
- [ ] C-impl — **Minimal implementation (later gate, not started).** New
...
- [ ] D-tests — **Tests (later gate, not started).** At minimum cover the
  ...
  decoder calls only.
- [ ] E-reviews — **Independent reviews (later gates, not started).**
  Mathematical/implementation review verdict
  `D7_G_EXTRINSIC_CONTRACT_REVIEW_PASS` or `...FAIL`; then
  integration-readiness review verdict
  `D7_G_EXTRINSIC_INTEGRATION_READINESS_PASS` or `...FAIL`. Neither
  authorizes D7-H.
