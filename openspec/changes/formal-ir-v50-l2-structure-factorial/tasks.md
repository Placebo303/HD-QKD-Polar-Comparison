...
**HEAD**: `c38652de9e4bca3daccbf0f9c96d7897d9199b89`（实现冻结时重绑）

## Phase A — 语义冻结（plan ACCEPT 后）

- [ ] **A1** 冻结唯一等泄漏 protograph/MET 候选 `P0-MET-1`：`n=1024, m2=184/190/192` 按源、`GF32 poly37`、`无零列`、`满行秩`、`E=2048`、`dc_max=16`、`max_degree2_chain≤4`、`degree2_pure_ring(len≤12)==0`、`确定性 lifting/label`、`禁止 seed 搜索`、`support_cycles_4==0` 硬门、`6/8-cycles` 报告；与 Lane C 相同 `m2` 与泄漏 `1064/1094/1104` (920/950/960+80+64)、相同 decoder `90/1.0`；构造规则不接触 V48 outcomes。
...
- [ ] **A3** 冻结 15 未使用 held-out blocks：与 `FORBIDDEN 141 = 96(V36..V47)+45(V48)` 零重叠、无内部重复、每源均分 5、连续 IDs `391001..391005 / 391101..391105 / 391201..391205` 仅作 block ID；每块 `4 frames=1024 pairs (每帧256)`、`BLOCK_LENGTH=1024`、`sampling_mode=deterministic_four_consecutive_frames_heldout_unused`；增量根 `.../v50_l2_structure_factorial/run_01/` fail-closed；不接触 V48 outcomes。
- [ ] **A4** 冻结 2×2 workload（90 calls）：每块 `2×L1 (TRAIN/TRAIN+VAL) +4×L2 (A=TRAIN×LaneC, B=TRAIN+VAL×LaneC, C=TRAIN×P0, D=TRAIN+VAL×P0) =6`，共 `15×6=90` (`l1 30 / l2 60 / total 90`)；以内 block `A,B,C,D` 配对，主效应 `C−A` (结构) 与 `B−A` (先验) 与交互 `(D−C)−(B−A)`。
...
- [ ] **B1** Seed-registry 校验：拒重复/错形状/与 FORBIDDEN 141 重叠（`96+45`）；接受新区 `391001..` `391101..` `391201..` 15 IDs per source 连续均分零重叠。
- [ ] **B2** P0 重建匹配：`P0-MET-1 m2×1024` 注入 metric 漂移 → J 路径；重建一致；代表身份错 → J；校验 `rank==m2`、`col_degree_min≥1`、`dc_max≤16`、`E==2048`、`support_cycles_4==0`、`max_chain≤4`、`pure_ring(≤12)==0`。
...
- [ ] **B8** 写出合约：文件集精确（60 L2 行含 `prior_id/structure_id/tag_ok/reclassified/leak` + `frame_ids/sampling_mode`；summary 含分层记账 90 + 因子聚合 + `C−A/B−A/交互` + 四类/G3' + 终态）、CSV/JSON 行对等、已存在根 fail-closed、禁写 NPZ、provenance 含 `P0-MET-1` 确定性 ID + held-out 未使用溯源 + 双 prior 溯源。

## Phase C — preflight（decoder-free，授权前）

- [ ] **P1** 重建 `P0-MET-1 m2×1024` 三矩阵；与 committed v38 常量 `dc_max/capacity` 比对；校验 `compute_tag_64` 可 import 且 `empty+x2` 可用；校验 TRAIN 与 TRAIN+VAL counts 形态可达；校验未使用 held-out 池可达且与 FORBIDDEN 141 零重叠；校验 4 frames=1024/256 per frame；零求值器、零写盘。
- [ ] **P2** 在首块 `391001/391101/391201` 跑双 prior 绑定 preflight（`p_i→s1→BP_fake→q_fake` 双 prior + `chain_ring_ok` + `four_cycle_zero` + `tag_ok` + `leakage_accounted` + `sampling_mode/frame_ids`）；仅报 PASS/BLOCKED.
- [ ] **P3** 以拷贝 FORBIDDEN 141 机械校验 J2（新区 15 IDs 零重叠、无重复、每源均分连续）并确认输出根缺席；校验 4 frames/block 按 hold 帧序分散拼接可行；仅报 PASS/BLOCKED.

## Phase D — 授权 2×2 因子执行（需 EXECUTE_AUTH，90 calls @15 块）

- [ ] **D1** 主线程获独立 plan ACCEPT + 显式用户 `EXECUTE_AUTH`（绑定 repository、分支 `formal-ir-mainline`、完整实现 SHA、cycle V50P0、scope `v50_factorial_90_calls_2x2_exactly_once` 冻结，HEAD `c38652de`）。
...
- [ ] **D3** 只读 postcheck：记账 `planned 90 / l1 30 / l2 60 / total 90`、L2 60 记录与序每块 `A,B,C,D` 正确、L1 30 次、`prior_id` 双溯源、未使用 held-out 块、无 NPZ、V38–V48 输出 byte-identical、summary 完备（含分层记账与因子主效应 `C−A / B−A / 交互` + 四类/G3' + 终态 + `f_total` + provenance）、独立重算.

## Phase E — 结果复核

- [ ] **E1** 写 `OPERATOR_RETURN.md` 与 `DEVELOPMENT_RESULT.md` 候选；lifecycle 保持 result-candidate；含 terminal (`V50_EVIDENCE_INVALID / V50_FACTORIAL_COMPLETE`)、路由轨迹、因子聚合与主效应 `C−A/B−A/交互`、per-block 结果、四类计数、`l1` 执行期诊断上下文（真实 `q≠p`/entropy/iterations/||q-p||1/tag_ok）严格在 claim boundary 内（含 L2-only 工程近似与等泄漏 1064/1094/1104 与未使用 held-out 说明，不宣称 FER/晋升）。
