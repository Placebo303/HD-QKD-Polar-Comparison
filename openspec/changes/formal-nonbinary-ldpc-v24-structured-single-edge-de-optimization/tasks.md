...
Status: **ENGINEERING_IN_PROGRESS — P07 authorized 2026-08-18 by current objective; I01-I11 engineering DONE; M0-M2 gate launched**
...
- [ ] **M001** Create a fresh additive V24 run root and persist the frozen
  manifest before scientific evaluation.
- [ ] **M002** Read-only validate the accepted corrected V8 trace at
...
- [ ] **M003** Read-only verify the V8 rate identity, exact V17 channel values,
...
- [ ] **M101** For each attempt `k=0..8191`, use independent
...
- [ ] **M102** Screen every valid candidate at `n_samples=200`, `max_iter=50`,
  seeds 24001/24002; retain every result.
- [ ] **M103** Rank by `(converged_count desc, worst_final_entropy asc,
...
- [ ] **M104** Refine the selected set at `n_samples=1000`, `max_iter=150`,
  seeds 24003/24004/24005; retain every result.
- [ ] **M105** Apply the same ranking to refinement and persist
  `min(4,N_refined)` finalist distributions before holdout. `N_valid=0` sets
  `mechanism_unverified` and forbids M2.
- [ ] **M106** Read-only verify candidate count, attempt ceiling, complete seed
...
- [ ] **M201** Evaluate each pre-persisted finalist at `n_samples=2000`,
  `max_iter=200`, seeds 24101–24105, without profile replacement or mutation.
- [ ] **M202** Mark a finalist PASS only if it remains valid and final base-q
  entropy is `<=0.01` on all five holdout seeds.
- [ ] **M203** Set V24 `pass` iff at least one finalist passes M202; otherwise,
  after complete frozen execution, set `fail`.
- [ ] **M204** Independent read-only acceptance: reconstruct every candidate
...
- [ ] **M301** Record the exact terminal state and evidence index in
  `docs/decision-log.md`; correct any surviving statement that V23 tested true
  MET.
- [ ] **M302** Update CURRENT_TASK, AGENT_HANDOFF, and AGENT_PROJECT_MEMORY with
  commands, artifacts, failures, warnings, and the authorization boundary.
- [ ] **M303** Mark every task accurately: blocked/cancelled work is not
  completed; retain partial and invalid evidence with reasons.
- [ ] **M304** Run required memory triage and final dirty-worktree/frozen-root
  review.
- [ ] **M305** Archive V24 only after formal completion and user choice.
