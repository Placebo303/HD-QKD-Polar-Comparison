...
- [x] D3 Implement `--r1d` runner mode (dev script, default off): R1D arms at
  ...
  old roots unchanged).
- [x] D4 Focused tests (new file, fake-only, fresh task-owned basetemps):
...
- [x] D5 `py_compile` + focused D6 + seven-file non-perf suite
  (`pytest -p no:cacheprovider`, `workspace/<task>/<uuid>/` basetemps).
  Perf-v38 only if a scoped dep changed (record choice).
- [x] D6 Read-only independent implementation review (own file only; max one
...
- [x] D7 Scoped local commits: (1) freeze [this task]; (2) implementation +
