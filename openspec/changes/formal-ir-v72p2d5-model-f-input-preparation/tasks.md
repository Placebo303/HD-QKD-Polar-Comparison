...
- [ ] T1 — Builder `build_model_f_input(alice,bob,frame_ids)` (A1)
...
- [ ] T2 — Writer `write_model_f_input(out_dir, counts_ab, p_b)` (A1)
...
- [ ] T3 — Reader `load_model_f_input(in_dir)` (A1)
...
- [ ] T4 — Runner `scripts/v72p2d5_prepare_model_f_input.py` (A2)
...
- [ ] T6 — Lifecycle + focused suite (A3 + A6 + OpenSpec A7-A10)
...
- [ ] T-ISOL — Lifecycle-independent test containment (additive, no T1-T6/PX11 rewrite)
...
- [ ] PX11-R1 — Parquet path rework (registry-relative vs repo ROOT)
