...
**Lifecycle**: `DRAFT → PLAN_CANDIDATE / DECODER_FREE / EXECUTE_NOT_AUTHORIZED` — 仅四工件 + decoder-free scout，不改 V65/主候选/src，不启动正式执行
...
- **Lifecycle**: `DRAFT → PLAN_CANDIDATE / DECODER_FREE / EXECUTE_NOT_AUTHORIZED` — 本轮止于四工件 + decoder-free scout 脚本（`v65a_scout.py` `rg decode 0 hits`），**不实现 runner，不执行 decoder，不创建 `run_01`，不读 TEST 统计，不读旧 outcome，不批量物化**；正式 `1024/256` 执行需独立 `PLAN_ACCEPT` + `Pre-RESULT` 复核。
...
- `V65A 候选`: 固定三候选 `2026-01-13 162148` → `2026-01-07 2500K` → `2026-01-07 160254` 首个 `VERIFY_PASS` 即停。

### 2.2 处理点与物化单点冻结

- `84d62779` 语义：`dimension 1024, bin_width 200ps, pairing nearest double-pointer bin//1024消歧, rule legacy_v1, channels A1/B5, frame_len 256 pairs, BLOCK 1024, period 204800ps floor_div, threshold 40000ps, gate 200ps` — 单点，V65A 三候选复用同一处理点，仅 `delay/peak/sigma` 候选独立重算 `sign+50ps` 门禁。
...
- **合同项**：`dimension 1024, bin 200, pairing nearest bin//1024, legacy_v1, channels(cand-specific)⊇{1,5}, delay_used_ps, peak_center, sigma ∈[50,150], p2bg, gate 200, threshold 40000, frame_start_ps, period 204800, floor_div, mapping legacy_v1, 每帧256 A=32U1+U2 B=32V1+V2` 逐项显式落盘，与 V56 权威算法逐项一致已验（`delay` 仅 `sign+50ps` 门禁，不强制等于 V65 数值）。
...
- **G-verify**：`VERIFY_PASS_cand = (dimension==1024 && bin==200 && pairing==nearest && rule==legacy_v1 && channels⊇{1,5} && sign(delay)==sign(peak) && |delay-peak|<50ps && sigma∈[50,150] && gate==200 && threshold==40000 && period==204800 && 每帧256 A/B映射)`，否则 `VERIFY_FAIL` 进入下一候选。
...
- **Stage1 阈（放宽，仅淘汰）**：`G2 λ不触界, G3 ΔNLL≤0.75, G4 Val NLL≤H_cal+1.5, G5 unseen≤0.02, G6 m1≤16, G7 m2≤200, G7-aux m_total≤216, G8 provenance+CE链式<1e-9`，任一 FAIL → `V65A_COARSE_REJECTED` 终态，不进 Stage2；全过 → `V65A_ELIGIBLE_FOR_FORMAL` 仅放行规划。
