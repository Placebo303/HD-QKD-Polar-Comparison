...
**Status**: `DRAFT / DECODER_FREE / EXECUTE_NOT_AUTHORIZED` — 仅创建四工件至推送，不实现 decoder，不创建 run_01，不读旧 outcome
...
**Lifecycle**: `DRAFT → PLAN_CANDIDATE / DECODER_FREE / EXECUTE_NOT_AUTHORIZED` — 本轮止于四工件 + decoder-free scout 脚本 + 推送，不运行 decoder
...
- **Batch 禁止**：`8 frames/candidate` 逐候选物化，禁止一次性物化三候选全部帧或预加载三候选 `pairs.parquet` 全量。脚本内 `assert materialized_frames_total <= 8 + (selected? 256+64 :0) + (formal? 1024+256+32 :0)` 可机械校验（本轮 Stage 2 仅规划，实物化上限为 `8 + 320 =328 frames` 若 Stage1 执行）。
- **Decoder-free**：`rg "decode_" 0 hits` 且 `rg "import.*decoder" 0 hits` 在 scout 脚本内；`py_compile PASS`；不创建 `run_01`。
- **旧 outcome 禁读**：不得读取 `comparison_bench/outputs_comparison/**/v6*` `run_01` 的 `FER/阈值/SKR/outcome` 作候选排序或阈值调整；候选顺序由本 proposal 冻结，不以历史性能重排。
...
- 不读取旧 outcome（`V48..V65` 的 `FER/阈值/门禁结果/outcome.json`）作本次排序/阈值依据；V65A 候选顺序固定与历史无关。
...
- **只读依赖**：`src.reconciliation.run_nbldpc_demo_point._read_ttbin_timetags/_bin_indices_sorted_for_binwidth/_pairs_from_sorted_bins` + `legacy_v1` mapping 的 V56 权威算法只读复用；`PROJECT_DATA_ROOT` 下三候选 session 目录（`2026-01-13/162148` 等，路径由 manifest 记录，不硬编码旧 Windows 路径）；`V13 sidecars` 仅作 `dimension/bin/period` 算法对照，不读 outcome。
...
- [ ] **固定顺序首个通过即停可复现**：`candidates_ordered` 三候选按 `["2026-01-13 162148","2026-01-07 2500K","2026-01-07 160254"]` 冻结，脚本按序逐一 `materialize_exactly 8 frames (4+4)`，`materialized_candidates_count <=1 + (selected?1:0)` 且 `materialized_frames_total` 在 Stage0 阶段 `<=8×checked_candidates`（首个 PASS 时 `==8×(index+1)`），`selected` 为首个 `VERIFY_PASS`，后续候选未物化已验（`not_materialized_later_candidates`）。
- [ ] **4+4 materialization/provenance 逐项可复现**：每候选 8 帧均显式落盘 `dimension 1024 / bin_width 200ps / pairing nearest double-pointer bin//1024 / rule legacy_v1 / channels A1/B5(cand-specific) / delay_used_ps / peak_center / sigma / gate 200 / threshold 40000 / frame_start_ps / period 204800 floor_div / mapping legacy_v1 / 每帧256 A=32U1+U2 B=32V1+V2`，且 `sign(delay)==sign(peak) && |delay-peak|<50ps && sigma∈[50,150] && gate==200 && threshold==40000`，缺失/偏离则 `VERIFY_FAIL`，三候选 `VERIFY_PASS/FAIL` 明细已落盘，首个 PASS 即停。
...
- [ ] **256/64 粗筛仅淘汰已验**：对 selected 候选的 `256 CAL +64 VAL` 粗筛报告 `λ_at_boundary / ΔNLL / Val NLL / unseen / effective_contexts / CE1/CE2/CE_full chain_delta / m1/m2(CE) ceil`，粗筛 `FAIL → overall=REJECT` 不进入 Stage2，粗筛 `PASS → overall=ELIGIBLE_FOR_FORMAL` 绝不为 `READY`，`coarse_cannot_ready` 已验。
...
`V65` 保持 `DATA_NOT_READY`（三源 `4096+512+120` qualification 不动）；`V65A` 本轮 `DRAFT→PLAN_CANDIDATE / DECODER_FREE / EXECUTE_NOT_AUTHORIZED`（仅 decoder-free 勘探，不实现 runner，不执行 decoder，不创建 `run_01`，不读旧 outcome，首个通过即停）；`V65A` 正式 `1024/256` 执行需独立 `PLAN_ACCEPT`（`Pre-RESULT` 独立线程复核 `HEAD/ACCEPTED_PLAN_SHA/rg 0 hits/run_01不存在/py_compile+TEST未读/批量物化守卫/固定顺序`），双重 review 后方可进入 `FORMAL_READY`；`V65` 本身不因 V65A 改变终态。
