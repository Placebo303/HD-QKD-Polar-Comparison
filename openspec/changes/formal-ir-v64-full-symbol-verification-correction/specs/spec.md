...
- **Type**: `VERIFICATION_CORRECTION` — NB-LDPC verification 语义修正（L2-only `tag(U2)` → full-symbol `tag(32*U1+U2)`），Phase A 只读归因 0 calls + Phase B 45 fresh 15/source 一次 decode 双口径对照，非 formal qualification/promotion/安全证明。
- **Lifecycle**: `PLAN_CANDIDATE / EXECUTE_NOT_AUTHORIZED` — 本轮止于 plan 四工件，Phase A 未分流为 `L2_TAG_INSUFFICIENT` 前禁止任何 fresh decoder 实现/执行与正式 `run_01` 创建。
...
- **泄漏不变**：`leak=5*m_total+64` 双校验 `m_total 184/190/192 +8/+16` 且 `stage1-base=40, stage2-stage1=40`；单 64-bit tag 仍仅 `total` 计一次，不因输入从 5-bit 扩到 10-bit 而增。
- **双口径报告**：同一 `u1_hat/u2_hat` 解码输出并列计算 `tag_ok_l2` 与 `tag_ok_full`，`accepted_l2/accepted_full` 与 `undetected_l2/undetected_full` 对照，`Δ_accepted/Δ_undetected` 与 `被拦截U1-only wrong = count(!exact_u1&&exact_l2&&tag_ok_l2&&!tag_ok_full)` 诊断 L2-only 漏洞。
...
- **禁止**：第二 tag、重复计 tag、改 `m_total`、改 tag 长度、改 `m1/m2`、新增矩阵/阈值/decoder。
...
- 禁增泄漏（第二 tag / 重复计 tag / 改 `m_total`）；禁改 `src/experiments/tools` 任何文件（只读复用）；禁把 `L2-only` 冒充 `full-symbol`。
- 禁重估先验；禁跨域假同域；禁在 `ATTRIBUTION_INCOMPLETE` 时仍进 Phase B。
- 未创建正式 `run_01` 前禁 `decode_*` 新增在 plan 侧（Phase A 零 calls）。
...
### 3.3 三分支分流（first-match）

```
if fields_missing or inconsistent_definition or tag_scope != l2_only:
    branch = ATTRIBUTION_INCOMPLETE  # 无法拆解，不重跑，记录缺失字段
elif U1_wrong && U2_exact && syndrome_ok_l2 && tag_ok_l2:
    branch = L2_TAG_INSUFFICIENT  # U1 wrong + U2 exact → L2-only tag 不足 → 进入 Phase B
elif U2_wrong && tag_ok_l2:
    branch = PAUSE_INVESTIGATE_TAG_ENCODING  # U2 wrong + tag_ok → 暂停调查 tag/encoding 链路
else:
    branch = ATTRIBUTION_INCOMPLETE  # 无法拆解，不重跑
```

- `L2_TAG_INSUFFICIENT` 为进入 Phase B 的唯一前置；`PAUSE_INVESTIGATE` 停止不进 Phase B；`ATTRIBUTION_INCOMPLETE` 停止不重跑、不猜测。

## 4. Phase B — 45 fresh blocks 15/source 一次 decode 双口径报告（2–4 calls/block 总 90–180）
...
### 4.3 三阶段协议（硬帽180, verification-only, 仅 full 口径触发）

```
per block (45 blocks, 15/source):
  prior = TRAIN prior (shared)
  L1: H1 s1 → BP → q → P(U2)  // 45 次总计，per block 1 (L1)
  base: decode_L2(H_base(m2), P(U2), s_base) → syndrome_ok + tag_ok_l2 + tag_ok_full 双算 → verify_full_base = syndrome_ok && tag_ok_full ; verify_l2_base = syndrome_ok && tag_ok_l2
         if verify_full_base: final = base, leak = leak_base, stage=base
         else:
           s_inc1 = H_inc1*u2_true ; s_joint1=[s_base; s_inc1]
           stage1: decode_L2(H_joint1, P(U2), s_joint1) → syndrome_ok + 双 tag → verify_full_stage1
           if verify_full_stage1: final = stage1, leak = leak_stage1
           else:
             s_inc2 = H_inc2*u2_true ; s_total=[s_base; s_inc1; s_inc2]
             stage2: decode_L2(H_total, P(U2), s_total) → syndrome_ok + 双 tag → verify_full_stage2
             final = stage2, leak = leak_stage2
  record: exact_u1/l2/full, syndrome_ok, tag_ok_l2, tag_ok_full, accepted_l2/full, undetected_l2/full, delta_tag_ok, intercepted_u1_only
budget: L1 45 + base45 + stage1≤45 + stage2≤45 =90–180 硬帽180 (L2 45–135)
leak: leak_base 1064/1094/1104, stage1 1104/1134/1144, stage2 1144/1174/1184 (tag 已含，不重复)
```

- `rank_total==m2+16 nested/independence==8 row≤16` 已验；`leak_base/stage1/stage2` 三档已验；`base` 兼 old 不重复。
- 仅 `Phase A == L2_TAG_INSUFFICIENT + 独立 plan ACCEPT + EXECUTE_AUTH` 后才允许创建正式实现并执行。
...
- `exact_full = exact_u1&&exact_l2` 与 `accepted_full = syndrome_ok&&tag_ok_full` 分别计数（overall 45 + per-source 15）；`undetected_full = syndrome_ok&&tag_ok_full&&!exact_full` 单独表；`undetected_l2` 仅对照；`disclosure` 三档按实际 `stage_used` (以 full verification 触发计)；`calls/rescue` 按 `!verify_full_base` / `!verify_full_base&&!verify_full_stage1` 分母；`被拦截U1-only wrong` 单独计数。
- 各源各自 `base/stage1/final (accepted_full vs exact_full 分别 + L2-vs-full Δ)` 与 `leakage/rescue_rate/runtime/stage_used/被拦截U1-only` 分别报告；`base/stage1` 仅分层报告不作主判；门禁仅 `final` full 口径。

### 5.2 预注册终态（first-match，互斥）

```
if not attribution_complete or not tag_input_full_correct or rank/nested/verification/记账/域/重叠/预算/泄漏/all_exact_tag_ok_full 检查失败:
...
- `PASS` 需 `overall exact_full≥35/45 (77.78%)` 且 `per-source ≥10/15 (66.7%)` 且 `undetected_full_tag==0` 且 `所有 exact_full 帧 tag_ok_full==True` 且 `rank/nested/verification/记账/域/预算/泄漏` 通过；`exact_full` 与 `accepted_full` 分别达标（禁相等假定）。
...
- `ATTRIBUTION_INCOMPLETE`: Phase A 无法拆解/字段缺失/口径矛盾，不重跑。
- `EVIDENCE_INVALID` 优先于所有性能终态。

### 5.3 双口径对照与拦截数

- `L2-only vs full 差异表`: `Δ_accepted, Δ_undetected, Δ_tag_ok` per source & overall；`被拦截U1-only wrong` 数量为诊断 V63 漏洞的核心对照。
...
- 守卫：`git diff -- src/ ==0 && git diff -- experiments/ ==0 && git diff -- tools/ ==0 && git diff -- openspec/changes/formal-ir-v6[0-2,3]/ ==0` (除本变更外零改)，`py_compile PASS`，`HEAD==origin` 已验，`run_01` 不存在，Phase A 零 calls 已验，verification 单 tag 泄漏不增已验。
- 本轮仅 plan 四工件，不创建正式输出；任何 fresh decode 需 `Phase A == L2_TAG_INSUFFICIENT + 独立 plan ACCEPT + EXECUTE_AUTH`。
