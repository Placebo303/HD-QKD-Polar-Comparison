...
4. **Phase B 修正验证 / fresh instrumentation confirmation（45 fresh blocks，15/source，2–4 calls/block 总 90–180，不预设根因）**：
   ...
   - **预算与单 tag 不变性**：`L1 45 + base45 + stage1≤45 + stage2≤45 =90–180 硬帽180 (L2 45–135)`，`per block 2–4 calls`，`base` 兼 old 不重复，`verification-only` 触发（以 `full` 口径触发计），`leak` 三档不变（仍单 64-bit tag，不因同时算 `tag_ok_l2` 与 `tag_ok_full` 而增 calls/泄漏）
