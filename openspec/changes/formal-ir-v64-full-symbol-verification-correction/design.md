...
**Cycle**: `V64P1` (24-block contraction revision)
**Predecessor**: `formal-ir-v63-nbldpc-polar-shell-integration` (`5602f11c`, V63 二阶段 `H1-16+Lane C+Δ8+Δ8` 壳集成) — V64 继承其全部纠错参数，仅修正 verification 语义，fresh 收缩至严格 24 blocks 8/source 不换域不复用不放松 gap≥4
...
**Feasibility**: V54/V63 二阶段 `H_total 200/206/208 rank m2+16 nested True` 已在同域 43/45 可重放；`compute_tag_64` canonical 已在 V35/V63 验证仅改输入不增泄漏；唯一待验为 **Phase A 能否将 v63_dev_1M_0133 定位为 U1-only tag 缺口** 与 **Phase B 改 full-symbol tag 后是否拦截该类 undetected**（24-block 等比门禁已等比收缩）

## 1. 科学问题与关键判断

> 在**完全冻结 V63 纠错能力**（`H1-16+L1APP+Lane C+Δ8+Δ8, leak 1064→1144/1174 1184`）且**泄漏不增**的前提下，V63 的 `L2-only tag( U2 )` 是否导致 `U1 wrong + U2 exact` 的帧被误判为 `syndrome_ok && tag_ok` 而形成 `undetected/L2-only 假通过`？若是，则将 verification 改为 `tag(32*U1+U2)` 全符号单 64-bit tag 能否在 fresh 24 blocks 上使门禁 `exact_full≥19/24 & per-source≥6/8 & undetected_full_tag==0` 成立？
...
- **新增分流（最小修订）**：若字段缺失/口径矛盾 → `ATTRIBUTION_INCOMPLETE` 不重跑 `v63_dev_1M_0133`，但**允许**进入 Phase B **fresh instrumentation confirmation**；该路径**不预设根因**，仅在 24 fresh blocks 上做仪器化对照，需同次 decode 同时保存 `exact_u1/l2/full、syndrome_ok_l1/l2、tag_ok_l2/tag_ok_full、errors_u1/u2、old/new accepted、stage/calls/leak` 且双算 `tag_ok_l2/tag_ok_full` 仍只计一个 64-bit tag 不增 calls/泄漏；直接继续沿用 `U1 wrong+U2 exact → verification-scope` 的旧假设而不经 fresh 仪器化则违冻结分流。

### 2.4 Phase B fresh 24 blocks 数据就绪（同域 84d62779，不换域不复用不放松 gap≥4，固化 24-block registry）

- **目标运行域声明（冻结）**：`D_target = { stratum_1M, stratum_1p5M, stratum_2M }`，处理点 `dimension 1024, bin_width 200ps, pairing nearest, rule legacy_v1` (**单点**，`84d62779`)，`source` 三源各一 held-out 区间（与 `V54/V63` 已用区间零重叠可校验，逐帧零重叠证明）。
- **规模**：`24 blocks = 8/source ×3`，每块 `1024 symbols (4×256 frames)`，枚举剩余非重叠四连续窗口（`start 0..H-4 per source, H=400/554/729, base=1600/2213/2916` 与 V63 一致的 held-out 超集，gap≥4 不放松），过滤与已用 `V48/V50/V51/V52/V53/V54/V63`（含 `v63_smoke 9 + v63_dev 90`）重叠者得 `K2`，按 `ordinal` 排序后以 `index_j=floor(j*(K2-1)/7) j=0..7` 分散选 `8/源`；`v64_fresh_registry.json` 为 fresh authoritative 固化实际 24-block registry，禁换块/禁重采样，逐帧零重叠证明。
...
- **预算冻结**：`24 blocks: L1 24 + base24 + stage1≤24 + stage2≤24 =48–96 硬帽96 (L2 24–72)`，`per block 2–4 calls`，第97次拒绝。
- **不换域不复用约束**：若 `K2<24` 或 `per-source<8` 则 `EVIDENCE_INVALID` 停止，不复用 V63 块、不回退小规模、不以 SER/vis 代理冒充；gap≥4 保持。

### 2.5 数据裁决（同域 vs 新 session）

- **同域可直接运行**：`comparison_bench/outputs_comparison/nonbinary_diagnostics/v13r3fresh_pairs_20260816` 的 held-out 池，与 V63 同处理点 `84d62779`，Phase B 所需 `K2≥24` 且 `per-source≥8` 才合法；Phase A 对 `v63_dev_1M_0133` 不做域检查（已落盘）。
- **新 session 准入**：本变更不新增新 session 采集；若未来 fresh 池 `K2<24` 则 `EVIDENCE_INVALID` 停止，不伪造，不以 `SER/vis` 代理冒充。
- **未对齐停止**：若 `K_available<24` 或 `pairing/dimension/bw` 不一致或 `H 矩阵校验失败`，则 `EVIDENCE_INVALID` 停止，不伪造。

## 3. 指标冻结（主/次分层，undetected 隔离，双口径对照）

### 3.1 主指标（门禁与价值判定，per-source 分层，24-block 等比）

| 指标 | 定义 | 分层 | 门禁 (Phase B 24) |
|---|---|---|---|
| `exact_u1 / exact_l2 / exact_full` | `exact_u1 = u1_hat==u1_true`；`exact_l2 = u2_hat==u2_true`；`exact_full = exact_u1&&exact_l2` oracle；分别计数 | overall + per source (8) | `exact_full ≥19/24 overall ∧ 每源 ≥6/8` |
...
| `disclosure bits/block` | `leak_base 1064/1094/1104, stage1 1104/1134/1144, stage2 1144/1174/1184` 三档 + `per_source_avg[s]=leak_base[s]+40*N_stage1[s]/8+40*N_stage2[s]/8` + `overall_avg=(Σ leak_base+40*N_stage1+40*N_stage2)/24` + `per accepted = total_disclosed / accepted_full_count` | per source + overall | 仅报告，但需 `leak=5*m_total+64` 双校验 |
...
| `Wilson 95%` | `overall 24 与 per-source 8` 的 Wilson 下界按 `exact_full`（主）与 `accepted_full`（审计）分别报告 | per source + overall | 仅报告 |
...
- Phase B 24 blocks 为 fresh development，不作独立 TEST qualification。
- `McNemar / binomial paired` 的 `p` 值可作为 `L2-vs-full` 差异的补充描述（`p>0.05` 视为不劣），但**不替代** `19/24 & 6/8` 硬门禁。
...
| G5 | 域一致性 | fresh `K2≥24` 且 `per-source≥8` 且与 `V48–V63` `frame_ids` 逐帧零重叠（逐帧零重叠证明） |

- Phase A 仅需 G1 PASS 即可归因；Phase B 需 G1-G5 全 PASS 且 `K2≥24`，否则 `EVIDENCE_INVALID` 停止；**`ATTRIBUTION_INCOMPLETE` 不再直接终止 Phase B fresh instrumentation** — 已记录缺失字段清单后允许进入 Phase B 仪器化对照（不预设根因，claim 受限）。
- 本轮 plan 仅冻结 G1-G5 定义，不执行 decoder（`rg "decode_"` 仅在冻结模块内）；`DECODE_FORBIDDEN` 直至实现后授权前保持。

## 5. 预算（已冻结，24-block 硬帽96）

- **Phase A**：`0 calls` (decoder-free 只读)。
- **Phase B 24 blocks**：`L1 24` 固定 + `base 24` 固定 + `stage1 0-24` 条件 + `stage2 0-24` 条件 → 总 `48–96` 硬帽96，第97次拒绝，`L2 24–72`，`per block 2–4 calls`。
- `L1 共享`（同一 `q_i` 与 `P_i(U2)` 在三阶段间共享），`base` 兼 old 不重复。

## 6. 门禁与终态（已冻结，ATTRIBUTION_INCOMPLETE fresh 仪器化修订，24-block 等比）

对 `24-block` fresh `8/source` 判定：
...
- **预注册终态（冻结，24-block，first-match，最小修订）**：
...
elif not domain_possible (K2<24 or per_source<8 or not registry_zero_overlap_frame_level):
    V64 = EVIDENCE_INVALID  # 域不足，不伪造，不换域不复用不放松 gap≥4
...
# elif exact_full >0 but (exact_full<19/24 or per_source<6/8 or undetected_full_tag!=0) and corrected_has_signal:
#        → V64_CORRECTION_WORKS_VERIFICATION_STILL_FAILS  # 纠错有信号但 full verification 未全过
# elif exact_full <19/24 or per_source<6/8 and no_corrected_signal:
#        → V64_CORRECTION_PERFORMANCE_FAIL
# else → V64_EVIDENCE_INVALID
```

- **PASS 容差冻结**：`overall exact_full≥19/24 (79.17%)` 且 `per-source ≥6/8 (75%)` 且 `undetected_full_tag==0` 且 `所有 exact_full 帧 tag_ok_full==True` 且 `rank/nested/verification/记账/预算/泄漏` 通过；`exact_full` 需单独达标（`accepted_full` 与 `exact_full` 分别计数，禁相等假定）；`leakage` 仅报告。
...
1. **拒绝类最先**：默认拒绝；必带 `--execution-authorized`；`git rev-parse HEAD` 与 `origin/formal-ir-mainline` 与 `--authorized-target-sha` 精确等值绑定未来实现 SHA（plan 引用 `84d62779` data SHA）；`v64_fresh_registry.json` 的 `frame_ids` 与 `V48–V63` 逐帧零重叠已验（逐帧证明）；G1-G5 未过即拒；输出根已存在即拒；任一拒绝零 calls 不建文件；预算硬帽96第97次拒绝。
2. **科学 preflights（decoder-free, write-free，未来执行前）**：
   ...
   - Phase B: G1-G5 全检 + `H_base` 三矩阵与 committed `v63` 常量 `rank/support` 比对 + `H_inc1/H_inc2` 确定性重建与 `rank_total==m2+16 / nested / independence==8 / row≤16 / col≤1 / E≈96 / leak+40+40` 校验 + TRAIN counts 形态校验 + `FrameBatch` 可物化校验 + fresh registry `frame_ids/ordinal` 每源分散校验 + `K2≥24 per-source≥8` 且逐帧零重叠证明
3. **执行偏差防复发（冻结）**：
...
comparison_bench/outputs_comparison/formal_ir_methods/v64_full_symbol_verification/run_fresh_24/    # Phase B 24-block
```

文件：`v64_attribution.json` (Phase A 归因报告, 0 decoder calls) + `v64_records.json/.csv` (Phase B `24–72` L2行；总 calls `48–96` 硬帽96)、`v64_summary.json`（分层，含双口径对照）、`v64_fresh_registry.json` (authoritative 24, 固化实表逐帧零重叠证明)、`v64_invalid_notice.json`（失败时）、`v64_data_readiness.json`（G1-G5）。CSV/JSON 行对等；禁写 NPZ. **本轮 P0 不创建上述输出**，仅冻结计划。
...
- `comparison_bench/src/comparison_bench/methods/nbldpc_shell_adapter.py` 补丁：`tag_input` 从 `x2` 改为 `32*u1_hat+u2_hat`，输出 `tag_ok_full` 与 `tag_ok_l2` 双字段同次保存，`actual_disclosure_bits` 三档不变，`single_tag_instrumentation` 断言，预算硬帽96第97次拒绝。
...
- `scripts/execute_v64_fresh_verify.py`：默认拒绝；`--execution-authorized --authorized-target-sha <sha>`；HEAD/origin 精确绑定未来实现 SHA（plan data SHA `84d62779` + fresh registry `hash` 24-block 实表）；SCOPED dirty；G1-G5 全 PASS 已验（含 K2≥24 per-source≥8 逐帧零重叠）；budget 硬帽96执行；同次双算单 tag 断言；执行偏差防复发；任一 gate 失败非零退出；fresh 解释按三规则（`U1-intercept→confirm` / `U2+tag_ok→investigate` / `无 discordance→仅 performance signal`）落盘。
- 仅 fake-runner 测试通过后才可进入 Phase B fresh verification；不以 outcomes 定增量或调 `Δm`；不改 V63 工件；`DECODE_FORBIDDEN` 保持至授权。

## 12. 自由裁量 D1–D8（修订至 PLAN_CANDIDATE，24-block contraction）
...
- D4 Phase A 零 decoder 只读归因三分支分流**新增 ATTRIBUTION_INCOMPLETE → Phase B fresh instrumentation confirmation（不预设根因）**，Phase B `8/source=24` 注册算法：若超集更大则分散选 `index_j=floor(j*(K2-1)/7) j=0..7`，否则判 `EVIDENCE_INVALID`，frame_ids 已冻固化实表，禁换块，逐帧零重叠证明，gap≥4 不放松；同次 decode 同时保存 `exact_u1/l2/full、syndrome_ok_l1/l2、tag_ok_l2/tag_ok_full、errors_u1/u2、old/new accepted、stage/calls/leak` 且双算仍单 tag。
- D5 预算 `Phase A 0 / Phase B 48–96` 硬帽96第97次拒绝，`L2 24–72`，`base` 兼 old 不重复，**双口径不增 calls/泄漏**。
...
- D7 fresh 24 需 `K2≥24` 且 `frame_ids zero overlap` 逐帧 with `V48–V63`，否则 `EVIDENCE_INVALID` 停止；Phase A `U2 wrong+tag_ok` 暂停调查 tag/encoding，fresh 中再现 `U2 wrong+tag_ok` 亦转 `tag/canonical investigation`；不换域不复用不放松 gap≥4。
