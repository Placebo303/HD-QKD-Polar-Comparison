...
## Phase B — 45 fresh blocks 15/source 2–4 calls 总 90–180，一次 decode 双口径仪器化报告（冻结 V63 全部纠错参数，仅 verification 改 full-symbol，不预设根因，单 tag 不增开销）
...
- [ ] **B8 未对齐/仪器化 claim 停止**：若 `K2<45` 或 `pairing/dimension/bw` 不一致或 `H 矩阵校验失败` 则 `EVIDENCE_INVALID` 停止；若 `Phase A==PAUSE_INVESTIGATE` 则停止不进 Phase B；若 `Phase A==ATTRIBUTION_INCOMPLETE` 允许进 Phase B fresh instrumentation 但 **fresh 无 discordance 时不得宣称已解释 V63 那例**，且 `U2 wrong+tag_ok` 再现时转 `tag/canonical investigation`；不伪造，不以旧假设直接继续
...
- [ ] **C2 门禁与 fresh 仪器化解释冻结（V64 PASS 四条件 + EVIDENCE_INVALID 优先 + ATTRIBUTION_INCOMPLETE fresh 仪器化 claim 边界）**：
  ...
  ```
  `G1 exact_full≥35/45 overall, G2 每源≥10/15, G3 undetected_full_tag==0, G4 所有 exact 帧 tag_ok_full==True` 已显式，不因 V63 差值改阈值，`rank/nested/记账/预算` 优先于性能门禁已验；**fresh 解释三规则已显式**：`U1 wrong+U2 exact+old accept+full reject→确认 verification-scope；U2 wrong+tag_ok→转 tag/canonical investigation；无 discordance→仅 performance signal`
- [ ] **C3 双口径仪器化对照与 U1-only 拦截冻结**：冻结**同次 decode 同时保存**的 `L2-only vs full 差异表 (Δ_accepted, Δ_undetected, Δ_tag_ok per source & overall)` + `被拦截 U1-only wrong 数量 = count(!exact_u1&&exact_l2&&tag_ok_l2&&!tag_ok_full)（确认为 U1 wrong+U2 exact+old accept+full reject）` + `若现 U2 wrong+tag_ok 则转 tag/canonical investigation` + `L1/L2 exact 提升对照` + `paired delta per block base vs stage1 vs final (以 full verification 为准)`，`U1-only 拦截` 为确认 verification-scope 的 fresh 仪器化关键对照，若 fresh 45 无 discordance 则仅 `full-tag performance signal` 已验
