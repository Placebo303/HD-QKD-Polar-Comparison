# D5 计划 R2：F 模型两层 GF32 码率与嵌套母矩阵设计（只冻结计划，不实现）
...
- Lifecycle：`PLAN_REVISE_REQUIRED`（R2；R1 `PLAN_ACCEPTED` 已被 corrigendum 宣告 superseded，仅作历史保留）
- R2 corrigendum：`docs/research_cycles/V72P2D5-GF32-RATE-MOTHER/PLAN_CORRIGENDUM_R2.md`（CAUSE: `DEGREE2_PREFIX_CONNECTIVITY_IMPOSSIBLE`；decoder 未执行，full mother 未构建；当前 3 `.py` 候选为 `UNCOMMITTED_NOT_ACCEPTED` 部分候选）
- R2 冻结单新 mother：`G2_MINIMAL_NESTED_DV3_GF32`（每层 `1000x1024`，列重 3，GF32 非零系数；无并行家族；仅必要边数条件可满足，不预断结构 PASS）
- R2 删除 live 路径：V31-degree-2 作为 D5 嵌套 mother（状态 `EXIT_PREFIX_CONNECTIVITY_IMPOSSIBLE`）、natural-prefix 合格假设、row-ordering 修复路径（自然序 = 构造序，无 `order_rows_for_prefix_coverage`，无后构造重排）
...
3. 仓库哪个现有构造器最适合扩展到约 700–1000 行？→ design §3 + `PLAN_FREEZE.md` 能力表（R2 结论：V31 degree-2 路径状态为 `EXIT_PREFIX_CONNECTIVITY_IMPOSSIBLE`，仅保留 field/rank/decoder 复用，degree-2 support 不再作为 D5 嵌套 mother；冻结单新 mother `G2_MINIMAL_NESTED_DV3_GF32`，见 design §3；Lane-C 仅备份，不并行）。
4. 如何构造嵌套、满秩、稀疏且可增量披露的 GF32 mother？→ design §4 + spec S-MOTHER（M0-STRUCTURE + 13 项 prefix 结构门禁 + 新增 `variable_degree_min>=2` 硬门；自然序 = 构造序，无 row ordering；4-cycle 只计数报告，无绝对 PASS 阈值，非零则 `STRUCTURE_PASS_WITH_CYCLE_RISK`）。
...
- 不实现任何构造器/decoder/prior 适配代码；不修改 `src/ experiments/ tools/ comparison_bench/src/` 下任何 `.py`（R2 plan-only 本轮尤其不碰 3 个未提交 `.py` 候选；未来 R2 实现 delta 在新 packet 接受后才动代码）。
- 不运行 decoder（未来测试的显式注入 fake 也不在本轮），不读 VAL，不创建正式结果，不授权真实执行（`run_01` / `EXECUTE_AUTH` 禁止）；不做 full-mother 构造（R2 仅冻结 dv3 构造合同，不执行构造）。
...
- R2 明确禁止的 live 路径：degree-2 V31 adapter 作为生产候选、row-ordering 实现（含 `order_rows_for_prefix_coverage`、后构造重排、decoder 驱动重排）、per-`k` 独立矩阵冒充 nested、support 追 rank 重抽、decoder 引导调图。
...
- V31 `build_layer(1000,1024)` degree-2 前缀视为 nested mother 的路线已关闭（R2 不可能性证明见 corrigendum §2：full mother `E=2N=2048, V=2024, need 2023` 可行；L2 `k=686: 1420<1709`、L1 `k=782: 1612<1805`，任意排序不可修复）。本计划改为冻结单新 dv3 mother（design §3–§4），不假设自然行序合格，不设 row-ordering 修复。
...
- [ ] prefix 门全：L1/L2 每个 `k` 13 项报告 + 最低 PASS（含新增 `variable_degree_min>=2` 硬门，非递减）；natural-prefix 非假设；无 row-ordering live 路径（自然序 = 构造序）；4-cycle 只计数报告（非零则 `STRUCTURE_PASS_WITH_CYCLE_RISK`，不断言绝对阈值）；M0 全 PASS 才进 G0；任一冻结 prefix 秩不足则 `G2_PREFIX_RANK_BLOCKED` 返回 planner，不自动重抽。
- [ ] dv3 必要条件：`E=3072`；L2 `k=686: 2444>=1709`、L1 `k=782: 2636>=1805` 仅记“必要条件可满足”，不预断结构 PASS。
