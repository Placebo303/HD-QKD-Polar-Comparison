...
- A-X1-20260921 (planning only, NOT granted): per-source rescue extension per the user generality directive 2026-09-21 ("test the WORST source, not the best"). §§1–8 UNCHANGED (2M frozen arms P1-R1/R2 intact, NOT re-run). New §9 adds 1M + 1.5M rescue arms consuming `X1_CROSS_SOURCE_PACKET.md` evidence. This amendment authorizes NOTHING; §9 arms need X1 evidence + Pre-EXECUTE + fresh explicit grant each.

## 9. Per-source rescue extension (A-X1-20260921; FROZEN, NOT GRANTED)

- Generality anchor: the test is anchored on the WORST source (**1M**, lowest corrected H_full 0.80361, `workspace/p3_census_3954637c/DESIGN_POINT_ARITHMETIC.md`). A PASS on 1M is the anti-cherry-picking evidence. **Reporting only 2M as headline is cherry-picking and FORBIDDEN** — any headline comparison MUST lead with the 1M verdict.
- Machinery (identical to §§3/5, per source): same prior family (b2f soft-marginal verbatim), same decoder (v28 max_iter 300/streak 3, exact_match), same §3A alignment procedure (per-source derived offsets: 1M −50 ps / 1.5M +50 ps, `P3_A1_REVIEW.md` findings 1–3), same Δm=8 single-segment nested rescue rule (base = leading-m_base rows of the frozen same-instance A208 matrix, rescue rows [m_base,m_base+8), COLD full-matrix re-decode). Per-source channel bundle (no cross-source reuse); 240 paired blocks `2026095601+idx`, stream `o1_blk:{seed}`; instances 2026092001 (R1) + 2026092011 (R2), reported separately, NO pooling. Base rate set FROM X1 cliff evidence, never from 2M assumption — §9 arms are BLOCKED until X1 lands for their source.
- **P1-1M-R1/R2 (worst-source arms): m_base = 193, rescue +8, total 201.** Arithmetic on 1M own corrected basis (H=0.80361, content=1024·H=822.897, cap=1.3·content=1069.77): binding constraint is m_max=201 (frozen cap 208 does NOT bind 1M), so m_base+8 ≤ 201 ⇒ m_base ≤ 193; 193 frozen as the maximum admissible base. Base leak 5·193+64=1029 (f_base≈1.25046); total leak 1069 (f≈1.29907 ≤ 1.3); E[leak]=1029+40·r; headroom=1069.77−E[leak] (40.77 b at r=0); gate (c) ⇒ r ≤ 48.2%. N-rule worst case ≈15438 (operator verifies; ≫911 — uncertifiable at zero-fail scope, stated plainly).
- **P1-1.5M-R1/R2: m_base = 199, rescue +8, total 207 — status INDETERMINATE-pending-X1.** Arithmetic on 1.5M own corrected basis (H=0.82896, content=848.855, cap=1103.51): m_max=207 ⇒ m_base ≤ 199; 199 frozen as the maximum admissible base. Base leak 1059 (f_base=1.24756, N=274); total leak 1099 (f≈1.29470 ≤ 1.3); E[leak]=1059+40·r; headroom 44.51 b at r=0; gate (c) ⇒ r ≤ 57.5%. INDETERMINATE because 1.5M's @208 status sits inside its CI (0.82896 vs 0.829327 threshold, Δ=−0.00037 within half-width 0.00221) and m_min is unmeasured — X1 may move the base DOWN; execution/grant FORBIDDEN until X1 lands and the main thread confirms the base.
