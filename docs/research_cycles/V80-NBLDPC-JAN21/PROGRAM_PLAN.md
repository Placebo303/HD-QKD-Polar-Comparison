...
- NB 目标行是 f=1.3 精确预算（非整数 m）；整数 m 实现必须走超帧：q=1024 取 m=25/超帧（泄漏 1064，净 +4056）或 DE 可达点 m=24（泄漏 1024，净 +4096，f≈1.20）；分层臂 m_total=49（m₂=47+m₁≈2，泄漏 1044，净 +4076，f≈1.22）。n=256 单帧无整数 m 达标。
  [2026-09-20 注释，历史保留：(a) GF(32)² 的“1−49/256≈0.8086”不得称为总码率——系帧-层口径；
  每层 256 GF(32) 符号（共 512）、49 个独立校验下总名义码率为 1−49/512≈0.9043，引用时必须显式标注口径。
  (b) “n=256 单帧无整数 m 达标”收窄为“尚无这些点可纠错的证据”——本计划 §1.3 自算直接臂 m≤21 预算可过（含 tag 单帧 syndrome≤213.08）。
  (c) “m=25/超帧”读作每子帧 25 行、四帧共 100 行。]
...
- 门：FER ≤5%（blind reconciliation 速率自适应后）；效率 f≤1.3（冻结口径：整帧含 tag，超帧 n=1024 上度量，见 §1.3）⇒ 构造码率目标：q=1024 直接臂 m≤25（H=0.8，rate≥0.902）/ m≤26（H=0.83，rate≥0.898），分层臂 m_total≤50（H=0.8）/ ≤52（H=0.83）。S2 未授权（维持现状，无新增授权语句）。
  [2026-09-20 注释：n=1024 是验证分组长度，尚非 LDPC 码长；当前设计仍是四个 n=256 帧；
  S2 必须冻结整组接受/丢弃规则并直接度量超帧 FER；若各帧独立且任一失败即整组失败，
  超帧 FER≤5% 要求单帧 FER≤1.274%（1−(1−0.05)^(1/4)）；不得复用单帧 5% 门。]
