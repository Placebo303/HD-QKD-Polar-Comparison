...
**Implementation SHA**: `git rev-parse HEAD == origin/formal-ir-mainline` (to be verified at push)
...
|------|-------|----------|--------|
| G0 HEAD/origin binding | `git rev-parse HEAD == origin/formal-ir-mainline == --authorized-target-sha` | 40-char match | TODO verify after push |
...
| G2 pytest | `pytest test_v64_instrumentation.py -p no:cacheprovider` 8 passed (48-96, 24/8, gap≥4, SHA binding, no fallback) | 8 passed | PASS |
...
| G5 run_01 absent | `Test-Path v64_full_symbol_verification/run_01` == False | not exists | PASS |
| G6 budget | `48-96 hard cap96 97th reject, L2 24-72, per-block 2-4` | code enforces | PASS |
...
- Gate: `exact_full ≥19/24 overall (79.17%) ∧ per-source ≥6/8 (75%) ∧ undetected_full==0 ∧ all exact tag_ok_full==True`
...
- Decoder frozen `90/1.0 poly37`, `TRAIN prior`, `H1-16 rank16`, `Lane C m2 184/190/192`, `H_inc1/2 8×1024 det`, `H_total 200/206/208 nested`, `independence 8`, `row≤16 col≤1 E≈96`
...
python -m pytest comparison_bench/tests/test_v64_instrumentation.py -v -p no:cacheprovider --basetemp workspace/v64_preexec
