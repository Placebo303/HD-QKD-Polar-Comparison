...
计划阶段只允许保留本入口和 OpenSpec 四工件共 5 个新增文件。禁止实现代码、
decoder、真实数据或 parquet 读取、结果目录创建，以及对 V72P1、V72P2、D1
已接受文件、`cycle_state.yaml`、memory、`src/`、`experiments/`、`tools/`、
`results/`和既有 comparison 输出的修改。
...
720 次完整更新、`float64`、`llr_clip=20.0`、`convergence_tol=1e-6`、
验证规则和公开计费规则。每臂 c2v 状态独立。L 的 factor work 单独计量，
不得把相同更新次数描述为相同总计算量。
...
- 各 checkpoint 上限 10，单臂总上限 720；剩余预算不足一个完整 sweep 时
  不启动 partial sweep。
...
f2b，以及 APP_raw。APP 只在诊断、输出和 hard sign 处裁剪，裁剪不改变 sign。

每 checkpoint 只携带 active c2v；新增 edge 置零，并从完整 active c2v 重建
b2f、f2b、APP_raw。每个完整 sweep 的 residual 是 sweep 前后 active c2v
snapshot 的最大绝对差。syndrome-aware sign、目标 bit self-exclusion、
收敛与 syndrome/tag 验证必须与 V72P1 语义一致。

本计划固定接口方案 A：adapter 零改动；新模块提供
...
OpenSpec `design.md` §4。实现必须对全部 10240 列使用
`old_to_phys`，并验证 `phys_to_old` 互逆；参考的 `41/47/65` 与 `95/95`
不是门槛。

P 固定使用 V70R1 1M CAL-only 选择的 M2 Laplace 参数：
`mu=0.0`、`scale=0.2714417616594907`、`eps=0.562251256281407`、
`Q=1024`。M2 的 kernel、floor 阶段、自然 log 和 Bob-only builder 见
`design.md` §5。历史 VAL CE `6.787126437359054` 只作背景，不是门槛或优胜
证明。
...
factor target updates/state evaluations、finite、syndrome/tag/oracle 状态。
不保存秘密数组、Alice/Bob 符号、syndrome bytes、完整 prior 或消息数组。

公开计费按每臂 counterfactual 独立计算：每 checkpoint 发布新增 syndrome rows，
首次进入验证前发布 64 tag bits，进入下一 checkpoint 才增加 1 CONTINUE bit；
成功、ladder exhausted、异常和 timeout 都保留已发布计数。不得将三臂计数相加，
不得把所有臂硬写成 `9100`。
...
真实执行只允许 L/I/P 各一次；A 只读。准备失败使三臂为
`NOT_ATTEMPTED` 并返回非零；任一新臂 exception、nonfinite、RSS 超限或
600 秒 soft wall 超时使该臂 `BLOCKED`，停止本次 invocation，其余臂为
`NOT_ATTEMPTED`，四个 arm artifact 均保留。普通 `LADDER_EXHAUSTED` 继续
下一臂。单臂 soft wall 为 600 秒，invocation 为 2400 秒
（prep 600 + 三臂各 600），peak RSS 硬上限 2 GiB。执行前必须通过独立
Plan Review、Implementation Review、Pre-EXECUTE，发布前必须通过 Pre-RESULT；
本计划不授任何执行权限。

## 预注册判别与后继

- L 只有出现 candidate escape、violation 下降或验证成功，才支持继续调度路线；
  仅 runtime 变快不算纠错改善。
- I 只有出现 escape 或 violation 下降，才支持冻结该列映射；单块不证明普遍
  图因果。
- P 若改变消息或翻转但不验证，只能说明 prior 影响；完全不变则削弱 prior-only
  解释。
- L/I/P 都无 hard-bit escape 时，停止 binary edge-level flooding/layered/
  ...
  GF32 对照。
- 任一臂成功只允许进入同路线的小样本 confirmation plan，不直接进入 V73。
...
代数等价、M2 参数与 floor、完整 metrics、动态计费、异常停机、schema、测试
和禁止文件范围。任一合同无法证明即 `BLOCKED`，不得用默认值或未冻结文字绕过。
