...
**Predecessor Result Review SHA**: cc1483cd4d7d3d2dc90bc02c4cf2db44d5ba73bf
...
6. Maximum check degree dc_max <= 16 across all rows.
...
### 3.1 Lane A: Near-dv=2 + GF(32) Cycle-Aware Edge Labeling (Label-Only Isolation Experiment)
- **Purpose**: Causal isolation test to determine whether changing GF(32) coefficients alone on the existing low-degree V31 binary support provides finite-block benefit.
...
- **Coefficient Initialization**: For a given construction seed, assign every existing edge a nonzero GF(32) entry from a deterministic PRNG seeded solely by that construction seed.
- **Deterministic Local Search Algorithm**:
  - **Lexicographic Objective**: (degenerate_4_cycles, degenerate_6_cycles, degenerate_8_cycles, -rank_GF32(H), tie_break).
  ...
  - Stop early if a full sweep results in 0 coefficient updates.
...
- **Purpose**: Test whether structural partitioning of variable classes (information vs. parity chain) provides low effective check degrees and convergence without random degree-2 cycle explosion.
- **Matrix Partition**: H_B = [H_info | H_parity] where H_info is m x (1024 - m) and H_parity is m x m.
- **Parity Structure**: H_parity is fixed as a deterministic lower-bidiagonal matrix (diagonal entries = 1, first subdiagonal entries = 1, all others = 0). This guarantees rank_GF32(H_parity) == m. (All existing parity entries = GF(32) element 1).
- **Information Support Construction**: Every column in H_info has weight exactly 2. For column j in ascending order:
  1. Select first check from checks with minimum current degree (ties ordered by seed-derived PRNG permutation).
  2. Select second check from eligible minimum-degree checks that does not create a duplicate edge.
  3. Among candidates, prefer those that do not create a new 4-cycle.
  4. Ties use the deterministic seed-derived check ordering.
- **Information Coefficients**: Assign independent deterministic nonzero GF(32) entries from the construction seed coefficient substream. (No label optimization in Lane B).
- **Lane-Specific Validity**: All information columns degree 2, dual-diagonal parity pattern (note: the parity endpoint has degree 1 as an explicit structural feature of eIRA), full GF(32) row rank, dc_max <= 16.

### 3.3 Lane C: SC-Inspired Finite Spatially Banded Prototype
- **Purpose**: Test whether spatial localization / band-diagonal coupling provides a useful finite-block signal on the empirical channel. (Explicitly designated as an SC-inspired finite prototype, not optimized SC-NB-LDPC).
- **Spatial Configuration**: L = 8 spatial positions (p = 0..7), coupling width w = 2.
- **Variable Allocation**: Exactly 128 variables per position (pos_v(j) = floor(8 * j / 1024)).
- **Check Allocation across 8 positions (Largest-Remainder)**:
  - 1M (m=184): [23, 23, 23, 23, 23, 23, 23, 23] (23 checks/pos).
  - 1.5M (m=190): [24, 24, 24, 24, 24, 24, 23, 23] (24 in pos 0..5, 23 in pos 6..7).
  - 2M (m=192): [24, 24, 24, 24, 24, 24, 24, 24] (24 checks/pos).
- **Support Construction**: Every variable column has weight exactly 2. For variable at position p:
  - Choose first check in position p from lowest-degree checks.
  - Choose second check in position min(p+1, 7) (for p=7, choose a distinct check within position 7).
  - Prefer choices that do not introduce a 4-cycle; ties ordered by seed PRNG.
- **Coefficients**: Assign deterministic nonzero GF(32) entries from construction seed substream. (No label optimization in Lane C).
...
- **Fallback Status**: If all 3 seeds fail full-rank/structural requirements, the lane status is LANE_STRUCTURAL_NOT_READY (does NOT invalidate other lanes).
...
3. **Error Reduction (At Least One)**:
   ...
   - **Criterion B**: overall_median_improvement >= 0.15 (median errors <= 150.45 vs. baseline 177.0).
...
The future implementation phase must implement and pass the following 20 focused tests (T1-T20):
