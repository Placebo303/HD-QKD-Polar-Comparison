...
2026-08-14 条目待办 #1（码率搜索修复后的全链重跑）获授权执行：Stage 0 主进程
PID 17828 于 2026-08-22T22:00:19+08:00 启动（workers=2 + metric-jobs=4），
输出根 `results/paper_grade_v4_rate_search_fix/four_loss_parts_frames300`，
冻结命令与日志同目录（`stage0_launch_record_20260822.txt` 等）。

### 发现与取证

详细记录：`results/paper_grade_v4_rate_search_fix/DATA_PROVENANCE_INCIDENT_20260825.md`。

- 2026-08-24 发现不同损失档之间存在字节级相同的候选序列；2026-08-25 完成取证。
- 根因：候选物化时 56 个 (d,bw) 点的序列取自**无损失命名空间的共享序列池**
  （`results/real_sequences/d{d}_bw{bw}/blk0`，2026-03-18 物化；以及已删除的
  `workspace/override_points`）。16/10/6dB 三档共用同一物理池 ⇒ 同 seed 的
  确定性重放输出逐位相同，跨损失候选输入并非相互独立样本。
- 影响清单：10∩16dB = 56 格、6∩16 = 56 格、6∩10 = 40 格字节级相同；
  **20dB 干净**（独立 t15 物化，与其他档 0 共享）；16dB 档内 56 个共享格的
  来源不可考。

### 终止决策（用户授权）

- 授权依据（用户 2026-08-25）：**"确认 6dB 波及即无必要继续"**。
- 取证确认 6dB 波及后，于 2026-08-25 20:58:53.870 (+08:00) 执行
  `taskkill /F /T /PID 17828`（树内 8/8 进程全部终止）；20:59:08 复核无残留、
  无误伤。
- 处置：20/16/10 三档产物共 **267 文件就地保留未动**（各档最后写入时刻均早于
  终止时刻，终止操作未触碰既有产物）；6dB 目录仅空骨架，按要求保留。该次运行
  **不产生任何论文口径结论**。

### 结论边界（科学有效性）

- 20dB 档内结论可用；16dB 档内可用但其 56 个共享格禁止用于跨损失对比；
  **10dB 与 6dB 的任何跨损失对比一律不成立**。

### 决定（持久）

1. 本次终止为最终状态：不重跑、不打补丁续用；三档产物作为取证证据就地冻结，
   不得覆盖或删除。
2. **修复对象是候选数据物化层**，不是本次运行的参数或流程逻辑。
3. **持久教训**：今后任何候选物化必须带损失命名空间，且在下游 replay 消费前
   通过跨档字节唯一性校验——同 seed 确定性重放会把输入相同静默传播为输出相同。

### 替代方案（已拒绝）

- 仅补跑 6dB、保留 10/16dB 结果：拒绝——10∩16 = 56 且 6∩10 = 40，跨档独立性
  已被破坏，部分保留会制造"局部干净"的假象。
- 对现有产物打降级标签继续使用：拒绝——命名空间独立性不可事后恢复，标签无法
  阻止下游误用。
- 立即自动重启重建：拒绝——旧共享池已删除，须从原始 ttbin 按损失命名空间重建
  每点序列，属新任务范围，超出本次变更授权。

### 待决策（新任务，需用户授权 + planner 规划，当前无人启动）

- 修复对象：候选数据物化层——从原始 ttbin 带损失命名空间重建每点序列。
- 重跑范围：至少 10dB、6dB 全档；16dB 的 56 个共享格是否复验待定。
...
## HD-QKD_Polar_Comparison 线（formal-ir，2026-06~08，融合自 origin/main 的合并）
