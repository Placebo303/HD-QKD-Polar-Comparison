#!/usr/bin/env python3
"""V72P2D4R2 CAL GF32 model rate audit runner — CAL-only frame-blocked nested U/G/F/L.

Reads a registry JSON, runs the compliant nested audit, writes the
four-file output to a fresh directory. Test runs use workspace dirs.
"""
...
MODULE_PATH = (
    ...
    / "comparison_bench"
    / "formal_ir"
    / "v72p2d4r2_cal_gf32_model_rate_audit.py"
)
...
def load_audit_module() -> Any:
    spec = importlib.util.spec_from_file_location(
        "v72p2d4r2_cal_gf32_model_rate_audit", str(MODULE_PATH)
    )
...
def run_audit(*, out_dir: str | Path, registry_path: str | Path) -> dict[str, Any]:
    ...
    report = mod.run_cal_audit(registry_path=registry_path, out_dir=out_dir)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.registry:
        ...
        return 2
    try:
        report = run_audit(out_dir=args.out_dir, registry_path=args.registry)
