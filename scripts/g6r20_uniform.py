"""R20 UNIFORM m100 thin runner — prior-isolation diagnostic (DECIDE-diagnostic).

Frozen scope (main-thread, do not redesign): single arm uniform-prior /
90-iters / m100 on R9's EXACT 128 blocks (session 20260107_PPLN_1p5M frames
2123..2186, graphs 4720/4721 even-odd reused, damping 1.0, T=64, disclosure
564, C/I/A=0, H-pair reported only). Baselines imported read-only (R9
modelF/90/m100 trajectories — byte-identical check, zero new calls).

- P-pop: registry v71_data_registry.json, session 20260107_PPLN_1p5M,
  frames 2123..2186 (64 frames, K=2 even-odd -> 128 calls). Same window as
  R9 (paired blocks).
- P-op: n128/m100/L020 lam_d2_0.20_d3_0.80, verbatim S7-arith sheet:
  ...
  4720/4721 as R9 (reuse by import, no new construction).
- P-dec: UNIFORM prior 1/32 per symbol (uniform-only single mode; NO
  prior-mode flag to prevent misuse), cold row-layered 90/1.0
  single-pass, disclosure_i=5*rows+64=564, control=interaction=auth=0.
  Model-F loader NEVER invoked on the uniform path: production bind drops
  the loader key structurally, and any injected loader is wrapped in a
  fail-closed guard (raises on call; loader_calls recorded, must be 0).
- P-acct: attempted/exact/accepted/undetected isolated; disclosure sums
  include failures; k_sym nominal 28 (= n-m, derived-only); beta
  derived-only (primary + L2 sensitivity, reported only, never gated).
...
- P-cmd: flags below; --execute-real refuses without --execution-authorized
  (rc=2) BEFORE any root/bind/load.
- P-tests: delta FAKE-fixture tests only (see tests/test_g6r20_uniform.py).
...
Reuse: r9 binds by import (no copy) — parse/graph/plan/tag/decode
adapters/verifier helpers; D10-R2 builder/admission/refusal; D16/v35
decoder kernel (lazy bind, no duplicate); R9 Model-F npz NEVER read here
(prior-root is identity-only).

Forbidden: any Model-F load on uniform path; prior-mode flag; create the
UUID root before grant; edit frozen runners/tests; touch
src/experiments/tools/results/outputs_comparison/prior roots/R9 root;
commit/push.
"""
...
# Frozen identifiers. Graph/op/dec/pop values aliased from R9 (same m100
# operating point and same 128 blocks; reuse by import).
CHANGE_ID = "g6r20-uniform"
CYCLE_ID = "G6R20-UNIFORM"
N = r9.N
M = r9.M
Q = r9.Q
POLY = r9.POLY
...
FROZEN_PRIOR_ROOT = r9.FROZEN_PRIOR_ROOT  # identity only; never loaded here
PRIOR_MODE = "uniform-1/32"
FUTURE_ROOT_UUID = "05100de1-e48c-4eea-a8a7-d5390f87cfc8"
FUTURE_ROOT = "workspace/g6r20_uni_" + FUTURE_ROOT_UUID
SCI_CEILING = r9.SCI_CEILING
SETUP_CEILING = 8
EVIDENCE_FILES = tuple(r9.EVIDENCE_FILES)
BLOCK_COLUMNS = tuple(r9.BLOCK_COLUMNS)
FROZEN_COMMAND = (
    ".venv/bin/python scripts/g6r20_uniform.py --execute-real "
...
_pre_execute_check = r9._pre_execute_check  # deep key-gate incl parquet frame presence


def uniform_l2_prior(n: int = N, q: int = Q) -> np.ndarray:
    """Uniform prior 1/q per symbol (uniform-only path; no Model-F input)."""
    prior = np.full((int(n), int(q)), 1.0 / float(q), dtype=np.float64)
    return prior


def build_graph(graph_seed: int) -> dict:
    """Rebuild one frozen R20 graph (same pair/builder/namespaces as R9)."""
    if int(graph_seed) not in GRAPH_SEEDS:
        raise ValueError("PRE_EXECUTION_BLOCKED: graph seed %r outside "
                         "frozen R20 pair" % (graph_seed,))
    return r9.build_graph(int(graph_seed))


def bind_production_adapters():
    """Lazy production bind (decoder + syndrome ONLY; loader never bound)."""
    ads = r9.bind_production_adapters()
    ads.pop("load_prior_fn", None)  # uniform path: loader never bound
    return ads


def _guard_loader(adapters: dict) -> dict:
    """Fail-closed guard: any injected loader raises on call (R6d2-F3 pattern).

    Returns call-state dict; successful completion with calls==0 proves the
    uniform path never invoked the Model-F loader.
    """
    state = {"loader_calls": 0}
    loader = adapters.get("load_prior_fn")
    if loader is not None:
        def _forbidden(root, _inner=loader, _st=state):
            _st["loader_calls"] += 1
            raise AssertionError(
                "Model-F loader forbidden on uniform path")
        adapters["load_prior_fn"] = _forbidden
    return state
...
def run_authorized_batch(out_root: str, registry: str, session_id: str,
    ...
    """Execute frozen 128-call uniform matrix; returns bundle (writes separately)."""
    ...
    for key in ("decode_fn", "syndrome_fn"):
        if adapters.get(key) is None:
            raise ValueError("adapters %s must be injected" % key)
    loader_state = _guard_loader(adapters)  # uniform: loader never invoked
    ...
    # Uniform path ignores Bob payloads (prior is 1/32); truth post-decision.
    import pandas as pd
    ...
    if loader_state["loader_calls"] != 0:
        raise AssertionError("uniform path invoked Model-F loader")
    ...
    manifest = {
        "schema": "g6r20_uniform_manifest_v1", "change_id": CHANGE_ID,
        ...
        "prior": {"mode": PRIOR_MODE,
                  "model_f_loader": "never-invoked (prior-root identity only)"},
    ...
    summary = {
        "schema": "g6r20_uniform_summary_v1", "terminal": terminal,
        "prior_mode": PRIOR_MODE, "loader_calls": 0,
...
def write_batch_root(bundle: dict) -> dict:
    ...
    lines = ["# R20 UNIFORM m100 report", "",
             "- terminal: `%s`" % summary["terminal"],
             "- prior: `%s` (loader_calls=%d)"
             % (summary["prior_mode"], summary["loader_calls"]),
...
def verify_root(out_root: str) -> bool:
    ...
    if (manifest.get("prior") or {}).get("mode") != PRIOR_MODE:
        print("VERIFY prior mode != uniform-1/32")
        return False
    if summary.get("prior_mode") != PRIOR_MODE:
        print("VERIFY summary prior_mode != uniform-1/32")
        return False
    if int(summary.get("loader_calls", -1)) != 0:
        print("VERIFY loader_calls != 0")
        return False
    ...
    if len(rows) not in (128,):
        ...
        # because exact/syndrome/tag isolation requires full 128 for R20).
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="R20 UNIFORM m100 thin runner")
...
def main(argv=None, *, adapters_override=None) -> int:
    ...
    print("R20 terminal=%s attempted=%d exact=%d accepted=%d undetected=%d"
