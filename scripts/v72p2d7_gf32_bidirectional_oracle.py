"""D7-C bidirectional-oracle runner (frozen R1+A1; gate only, no execution).
...
module by file path. Reads only the D7-C authorization state file; refuses
before any work/Model-F read/decoder bind/root creation while
``d7c_execution_authorized`` is false. ``--dry-run`` prints the frozen 128-call
matrix; ``--verify <root>`` performs the read-only six-file recomputation.
The frozen future command is not run in this task.
"""
...
CORE_PATH = (ROOT / "comparison_bench" / "src" / "comparison_bench"
             / "formal_ir" / "v72p2d7_gf32_bidirectional_oracle.py")
STATE_PATH = (ROOT / "docs" / "research_cycles"
              / "V72P2D7-GF32-BIDIRECTIONAL-ORACLE" / "cycle_state.yaml")

_SPEC = importlib.util.spec_from_file_location("d7c_bidirectional_oracle_core",
                                               str(CORE_PATH))
...
def build_parser():
    ap = argparse.ArgumentParser(
        description="D7-C bidirectional cross-layer oracle (authorization-gated).")
    ...
    ap.add_argument("--dry-run", action="store_true",
                    help="print the frozen 128-call matrix only")
    ap.add_argument("--verify", default=None,
                    help="read-only verify a six-file root")
    return ap
...
def main(argv=None):
    ...
    if args.verify:
        ...
        print(("VERIFY_OK" if report.get("ok") else "VERIFY_FAIL") + " %r" % (report,))
        return 0 if report.get("ok") else 2
    if args.dry_run:
        identities = _mod.frozen_identities()
        print("calls=%d conditions=%r budget=%d" % (
            len(identities), list(_mod.CONDITIONS), _mod.MAX_CALLS))
        for identity in identities:
            print("%d %s %d %s %s %d" % (
                identity["call_idx"], identity["condition"], identity["seed"],
                identity["f"], identity["layer"], identity["rows"]))
        return 0
    ...
    if not _mod.is_authorized(state):
        print("D7-C execution is not authorized; refusing before any work")
        return 3
    ...
    if not _mod.model_f_root_matches(args.model_f_root, repo_root=ROOT):
        print("D7-C refused: --model-f-root must equal %s" % (_mod.MODEL_F_ROOT,))
        return 3
    try:
        result = _mod.run_bidirectional_oracle(
            out_root=args.out_root, model_f_root=args.model_f_root,
            decode_fn=None, state=state, command_str=" ".join(sys.argv),
            repo_root=ROOT)
    except Exception as exc:
        print("D7-C refused: %r" % (exc,))
        return 3
