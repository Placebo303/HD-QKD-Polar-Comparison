"""V42P0 guarded conditional-realism diagnostic runner: exactly 18 real decoder calls.

Formal CLI for change ``formal-ir-v42-conditional-realism-diagnostic`` (cycle V42P0).
The mandatory ``--execution-authorized`` flag plus an explicit
``--authorized-target-sha`` (bound by user EXECUTE_AUTH to scope
``v42_diagnostic_18_calls_exactly_once``) are required. There is no
...
from comparison_bench.formal_ir.v42_conditional_realism_diagnostic import (  # noqa: E402
    OUTPUT_ROOT,
    IntegrityFailure,
    run_v42_diagnostic,
)
...
def _parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the guarded V42P0 conditional-realism diagnostic (hard cap 18 real decoder calls).")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not args.execution_authorized:
        print("BLOCKED: EXECUTE_NOT_AUTHORIZED — pass --execution-authorized with --authorized-target-sha per the accepted plan and explicit user EXECUTE_AUTH (scope v42_diagnostic_18_calls_exactly_once).", file=sys.stderr)
        return 2
    if not args.authorized_target_sha:
        ...
        return 2
    try:
        results = run_v42_diagnostic(execution_authorized=True, authorized_target_sha=args.authorized_target_sha, fake_runner=False, output_root=OUTPUT_ROOT)
    except IntegrityFailure as exc:
        print(f"BLOCKED: integrity failure {exc}", file=sys.stderr)
        return 2
    print(f"V42P0 additive evidence written to {results['output_root']}")
