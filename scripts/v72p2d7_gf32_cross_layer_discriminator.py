"""D7-E provenance-safe cross-layer discriminator runner (frozen R1; gate only).
...
module by file path.  Reads only the D7-E authorization state file; refuses
before any work/Model-F read/decoder bind/root creation while
``d7e_execution_authorized`` is false.  ``--dry-run`` prints the frozen
192-slot matrix; ``--verify <root>`` performs the read-only seven-file
...
CORE_PATH = (ROOT / "comparison_bench" / "src" / "comparison_bench"
             / "formal_ir" / "v72p2d7_gf32_cross_layer_discriminator.py")
STATE_PATH = (ROOT / "docs" / "research_cycles"
              / "V72P2D7-GF32-CROSS-LAYER-DISCRIMINATOR" / "cycle_state.yaml")

_SPEC = importlib.util.spec_from_file_location("d7e_cross_layer_discriminator_core",
                                               str(CORE_PATH))
...
def build_parser():
    ap = argparse.ArgumentParser(
        description="D7-E provenance-safe cross-layer discriminator "
                    "(authorization-gated).")
    ...
    ap.add_argument("--dry-run", action="store_true",
                    help="print the frozen 192-slot matrix only")
...
def main(argv=None):
    ...
    if args.dry_run:
        ...
        for slot in slots:
            print("%d %s %d %s %s %s %s %d" % (
                ...
                slot["direction"], slot["role"], slot["condition"],
    ...
    if not _mod.is_authorized(state):
        print("D7-E execution is not authorized; refusing before any work")
        return 3
    ...
    if not _mod.model_f_root_matches(args.model_f_root, repo_root=ROOT):
        print("D7-E refused: --model-f-root must equal %s" % (_mod.MODEL_F_ROOT,))
        return 3
    try:
        result = _mod.run_cross_layer_discriminator(
