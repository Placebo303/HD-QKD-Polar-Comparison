"""Authoritative V64 fresh 36-block registry generator — 12/source strict, zero overlap V48-V63, K2>=36 hard, no fallback."""
...
def main(argv=None) -> int:
    import argparse
    p = argparse.ArgumentParser(description="Generate authoritative v64_fresh_registry.json 36 blocks 12/source")
    p.add_argument("--output", default=str(DEFAULT_OUT), help="Output path for registry json")
    p.add_argument("--check-only", action="store_true", help="Only validate K2>=36 and zero overlap, do not write")
    ...
    try:
        ...
        return 2
    # verify per-source 12
    from collections import Counter
    c = Counter(r["source"] for r in reg)
    if c["1M"] != 12 or c["1p5M"] != 12 or c["2M"] != 12 or len(reg) != 36:
        print(f"EVIDENCE_INVALID: per-source {dict(c)} len {len(reg)} !=36", file=sys.stderr)
        return 2
    if args.check_only:
        print(f"V64 registry check PASS 36 blocks K2 per-source 12 provenance {[r['H_provenance']['K2'] for r in reg[:3]]}")
        return 0
