"""R12 FRESH nested-prefix runner — DECIDE-readiness (NOT execution).

R12b scope (frozen R12 packet, this session): readiness for the frozen R12
packet — M-mother m120, S0-S2 = 100/108/120 nested prefixes, cumulative
final-prefix accounting, no-retry-rewrite, fresh-window diagnostic on the
T1 window 2187..2250 (64 frames -> 128 blocks) with >=4 lift gate over the
R11 paired baseline, sci<=384. R12b makes ZERO real calls:
no construction, no decoder calls, no symbol reads, no UUID root.

- P-pop: fresh pool (T1 window, lowest admissible start >= 2187): session
  `20260107_PPLN_1p5M`, frames 2187..2250 (64 frames -> 128 blocks,
  frame->block order, even/odd mother). Allowlist ONLY that window.
  Disjointness proven in R12b readiness: adjacent above R9/R11
  2123..2186, below V55 2295..2298 block, above V55 2119..2122 block,
  clear of CAL 702..1725 + VAL 1726..1981, inside frames_total 5125.
...
- P-pair: fixed baseline F imported READ-ONLY from R11 artifacts
  (zero fixed decode calls asserted; byte-identical post-import
  asserted); paired lift = fresh_exact - F_R11; lift gate >= 4
  (diagnostic-first, stated for R12c routing).
...
- Construction = R12c START pre-phase as setup<=8 with admission gate:
  non-admitted mother -> ENGINEERING_BLOCKED, zero decoder calls.
  Mothers are the R11c-admitted REUSED pair (seeds 4722/4723, g6r11
  namespaces; deterministic rebuild = identical graphs; R11c admission
  artifacts valid by reference). R12b constructs NOTHING (admission
  predicate STATED below).

Lineage: R11 import-delta (F-method/F-cmd only: fresh-window allowlist
2187..2250 + new root; all frozen values identical). R9 binds reused by
import (no copied kernels): parse_frames, _candidate_tag,
marginal_l2_prior, bind_production_adapters, _hard_fail_oracle_app,
disclosed_bits formula. _pre_execute_check is LOCAL (fresh-window
allowlist; mirrors R9 structure with R12 window). Decoder/syndrome/PEG/
v10 kernels reused by import (v35, r2, common).
...
CHANGE_ID = "g6r12-fresh"
CYCLE_ID = "G6R12-FRESH"
N = 128
M_MOTHER = 120
Q = 32
MAX_ITER = 90
...
# Fresh-window allowlist EXACTLY the T1 window (lowest admissible start
# >= 2187; proven disjoint from R9 2123..2186, all V55-1p5M blocks,
# CAL 702..1725 + VAL 1726..1981, inside frames_total 5125).
FROZEN_FRAMES = "2187..2250"
FROZEN_PRIOR_ROOT = r9.FROZEN_PRIOR_ROOT
# Paired baseline: R11 diagnostic root (reference pattern; F_R11 = 87).
BASELINE_ROOT = "workspace/g6r11_adaptive_3c2b5b2e-897b-467e-a3e6-0cba005914ae"
FUTURE_ROOT_UUID = "c97777aa-620d-448f-8a6c-8abda5fb3b48"
FUTURE_ROOT = "workspace/g6r12_fresh_" + FUTURE_ROOT_UUID
SCI_CEILING = 384  # 128 blocks x <=3 stages
SETUP_CEILING = 8
PAIRED_LIFT_GATE = 4  # paired-diagnostic-first: fresh_exact - F_R11 >= 4
...
STAGE_COLUMNS = (
    ...
    "provenance")
ADMISSION_PREDICATE = (
    "STATED, NOT EXECUTED (R12b): at R12c START as setup<=8, rebuild one "
    ...
    "degrees recorded, never gating); the pair is the R11c-admitted "
    "REUSED pair (same seeds/namespaces; deterministic rebuild = "
    "identical graphs; R11c admission artifacts valid by reference); "
    "any non-admitted mother -> "
...
def coefficient_seed(graph_seed: int) -> int:
    """Fresh coefficient stream (R11 reused namespace, mother m120)."""
...
def build_mother(graph_seed: int) -> dict:
    """Build one frozen m120 mother + nested prefix slices (R12c setup)."""
    if int(graph_seed) not in MOTHER_SEEDS:
        raise ValueError("mother seed %r outside frozen R12 pair" % graph_seed)
    var = dict(VAR_COUNTS)
...
def build_call_plan(frame_ids: list[int]) -> list[dict]:
    ...
    if len(frames) != 64 or frames[0] != 2187 or frames[-1] != 2250:
        raise ValueError("R12 plan requires exactly frames 2187..2250")
    plan = []
...
def _pre_execute_check(registry_path: str, session_id: str,
    ...
    Local fresh-window allowlist (mirrors R9 structure with the R12
    window): frozen registry/session/frames/arm/prior-root identity, CAL
    702..1725 and VAL start, window disjointness from CAL/VAL, and parquet
    frame_id presence (frame_id column ONLY, never alice/bob). The exact
    window 2187..2250 was proven disjoint from R9 2123..2186, all
    V55-1p5M blocks, and CAL/VAL at R12b readiness, so exact-equality on
    the spec enforces the full F-pool.
    """
    ...
    frames = r9.parse_frames(frames_spec)
    if len(frames) != 64 or frames[0] != 2187 or frames[-1] != 2250:
        raise ValueError("PRE_EXECUTION_BLOCKED: window != 2187..2250")
...
def run_one_block_staged(mother: dict, prior: np.ndarray,
    ...
    if mother.get("dense") is None or not bool(mother.get("admitted")):
        ...
        raise _r2.StructureNotAdmitted("non-admitted R12 mother")
    nested_ok, nested_reason = check_nested_prefixes(mother)
    if not nested_ok:
        raise ValueError("non-nested R12 mother: %s" % nested_reason)
    truth = np.asarray(truth_u2, dtype=np.int64)
...
def run_authorized_batch(out_root: str, registry: str, session_id: str,
    ...
    frames = (pre_execute or _pre_execute_check)(
    ...
    manifest = {
        "schema": "g6r12_fresh_manifest_v1", "change_id": CHANGE_ID,
        ...
        "population_split": {"CAL": "702..1725 consumed", "selection": "empty",
                               "confirmation": "2187..2250 key-disjoint "
                                               "(fresh; paired-baseline R11)"},
    ...
    summary = {
        "schema": "g6r12_fresh_summary_v1", "terminal": terminal,
...
def write_batch_root(bundle: dict) -> dict:
    ...
    lines = ["# R12 FRESH nested-prefix report", "",
...
def profile_only() -> dict:
    ...
    return {
        ...
        "baseline": {"F": baseline["F"], "sha256": baseline["sha256"],
                     "successes": baseline["successes"]},
        "admission_predicate": "STATED, NOT EXECUTED (R12c setup<=8 gate)",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="R12 fresh runner")
...
def main(argv=None, *, adapters_override=None) -> int:
    ...
    print("R12 terminal=%s attempted=%d exact=%d accepted=%d undetected=%d "
