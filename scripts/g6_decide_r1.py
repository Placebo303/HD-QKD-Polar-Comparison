"""G6 DECIDE R1 thin runner — first real-data confirmation readiness.

Frozen R1 scope (planner return this session: F-pop/F-op/F-dec/F-acct/
F-gate/F-bud/F-cmd/F-tests/F-stop/F-auth). Track DECIDE-readiness
(NOT execution). Branch formal-ir-v72p1-addendum-clean.

- F-pop: registry v71_data_registry.json, session 20260123_2M_1p2M_0dB,
  frames 2093..2156 (64 frames, key-disjoint from CAL 702..1725,
  VAL 1726..1981 and V55 2M blocks; 1M/1p5M windows overlap V55).
- F-op: n128/m94/L020 lam_d2_0.20_d3_0.80 (2^35+3^93/E349/3^27+4^67).
- F-dec: Model-F marginal prior (read-only npz), cold row-layered 90/1.0
  single-pass, disclosure_i=5*rows+64, control=interaction=auth=0.
- F-acct: attempted/exact/accepted/undetected isolated; disclosure sums
  include failures; k_sym nominal 34; beta derived-only.
- F-gate: UNDETECTED_STOP absolute; ENGINEERING_BLOCKED for infra.
- F-bud: sci<=128/setup<=16.
- F-cmd: flags below; --execute-real refuses without --execution-authorized.
- F-tests: F1-F7 on FAKE fixtures only (see tests).
- F-stop: any violation STOP + retain + single decision needed.
- F-auth: standing grants cover THIS R2 scope only (runner+F-tests+graphs
  +proofs, zero real-payload decoder calls); R3 execution NOT consumed.

Reuse: D10-R2 builder/admission/refusal (import, no copy); D16/v35
decoder kernel (lazy bind, no duplicate); Model-F npz read-only.
Structural reuse ONLY from v72p2d19_l2_finite pattern; scripts/
v72p2_real_smoke.py reuse REJECTED per F-cmd (different ladder/syndrome).

Forbidden: decode/analyze real symbols here except the Pre-EXECUTE
frame_id key-check (frame_id column ONLY); create the UUID root;
touch src/experiments/tools/results/outputs_comparison; commit/push.
"""
...
# Frozen identifiers (F-pop/F-op/F-dec/F-bud/F-cmd).
CHANGE_ID = "g6-decide-r1"
CYCLE_ID = "G6-DECIDE-R1"
N = 128
M = 94
Q = 32
POLY = 37
...
K_SYM_NOMINAL = 34  # SUPERSEDED by net_secret_bits (R16 correction): k-based retained double-charges parity; see V72P3R16-CORRECTION/
VAR_COUNTS = {2: 35, 3: 93}
CHECK_COUNTS = {3: 27, 4: 67}
EDGE_TOTAL = 349
ARM = "L020"
CANDIDATE_ID = "lam_d2_0.20_d3_0.80"
GRAPH_SEEDS = (2026094601, 2026094602)
# Fresh namespaces (frozen exact strings; proven absent before use).
DOMAIN_NAMESPACE = "g6r1-domain"
COEFF_NAMESPACE_TMPL = "g6r1:coeff:n128:L020:%d"
FROZEN_REGISTRY = "v71_data_registry.json"
FROZEN_SESSION = "20260123_2M_1p2M_0dB"
FROZEN_FRAMES = "2093..2156"
FROZEN_PRIOR_ROOT = "workspace/v72p2d5_model_f_input/20260907_r1"
FUTURE_ROOT_UUID = "8e7c2a1f-4b6d-4e9a-9c3f-2a5b7d8e0f1a"
FUTURE_ROOT = "workspace/g6_decide_r1_" + FUTURE_ROOT_UUID
SCI_CEILING = 128
SETUP_CEILING = 16
...
FROZEN_COMMAND = (
    ".venv/bin/python scripts/g6_decide_r1.py --execute-real "
...
def parse_frames(spec: str) -> list[int]:
    """Parse frozen frame spec '2093..2156' (inclusive) to ID list."""
    text = str(spec).strip()
    if ".." in text:
...
def coefficient_seed(graph_seed: int) -> int:
    """Fresh coefficient stream (single G6 namespace, arm/width frozen)."""
...
def build_graph(graph_seed: int) -> dict:
    """Build one frozen G6 graph via D10-R2 builder + fresh coefficients."""
    if int(graph_seed) not in GRAPH_SEEDS:
        raise ValueError("graph seed %r outside frozen G6 pair" % graph_seed)
...
def build_call_plan(frame_ids: list[int]) -> list[dict]:
    ...
    index even/odd selects graph 4601/4602. One call per block (no pooling).
    """
    frames = [int(f) for f in frame_ids]
    if len(frames) != 64:
        raise ValueError("G6 plan requires exactly 64 frames, got %d" % len(frames))
    plan = []
...
def _hard_fail_oracle_app(adapters: dict) -> None:
    for key in adapters:
        ...
        if "oracle" in low or low == "app" or "app_" in low or "transfer" in low:
            raise ValueError("ORACLE/APP/transfer forbidden on G6 real path: %r" % key)
    for bad in ("oracle_prior_fn", "oracle_parts", "app_prior_fn", "transfer_fn"):
        if adapters.get(bad) is not None:
            raise ValueError("ORACLE/APP/transfer forbidden on G6 real path: %s" % bad)
...
def run_one_block(graph: dict, prior: np.ndarray, syndrome: np.ndarray,
    ...
    if graph.get("dense") is None or not bool(graph.get("admitted")):
        raise r2.StructureNotAdmitted("non-admitted G6 graph")
...
def _pre_execute_check(registry_path: str, session_id: str,
    ...
    if len(frames) != 64 or frames[0] != 2093 or frames[-1] != 2156:
        raise ValueError("PRE_EXECUTION_BLOCKED: window != 2093..2156")
...
def run_authorized_batch(out_root: str, registry: str, session_id: str,
    ...
    manifest = {
        "schema": "g6_decide_r1_manifest_v1", "change_id": CHANGE_ID,
        ...
        "coefficient_namespace": COEFF_NAMESPACE_TMPL % 2026094601,
    ...
    summary = {
        "schema": "g6_decide_r1_summary_v1", "terminal": terminal,
...
def verify_root(out_root: str) -> bool:
    ...
    if len(rows) not in (128,):
        ...
        # because exact/syndrome/tag isolation requires full 128 for R1).
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="G6 DECIDE R1 thin runner")
...
def main(argv=None, *, adapters_override=None) -> int:
    ...
    print("G6 terminal=%s attempted=%d exact=%d accepted=%d undetected=%d"
