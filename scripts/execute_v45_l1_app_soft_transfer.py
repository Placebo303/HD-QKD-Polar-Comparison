...
Mandatory ``--execution-authorized`` plus explicit ``--authorized-target-sha`` (bound by user EXECUTE_AUTH to scope v45_diagnostic_27_invocations_exactly_once) required. No fake-runner option; production path always binds fake_runner=False.
"""
...
def _parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the guarded V45P0 L1-APP soft-transfer diagnostic (hard cap 27 real decoder calls).")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not args.execution_authorized:
        print("BLOCKED: EXECUTE_NOT_AUTHORIZED — pass --execution-authorized with --authorized-target-sha per the accepted plan and explicit user EXECUTE_AUTH (scope v45_diagnostic_27_invocations_exactly_once).", file=sys.stderr)
        return 2
