...
def _check_git(authorized: str) -> None:
    # SCOPED fail-closed: ACCEPTED_PLAN_SHA mismatch must BLOCKED, not warning
    ...
    if ACCEPTED_PLAN_SHA != "760cb2967c7f5d5548a68f056458ef89398de3f2":
        print(f"BLOCKED: ACCEPTED_PLAN_SHA drift {ACCEPTED_PLAN_SHA} != 760cb2967c7f5d5548a68f056458ef89398de3f2 (SCOPED fail-closed)", file=sys.stderr)
        sys.exit(2)
    try:
...
def _preflight(output_root: Path) -> list[dict]:
    ...
    # held-out parquet existence (no synthetic fallback) — production always checks
...
# Test-only interface (not exposed via CLI) — production CLI has no --fake-runner
def build_fake_v64_records_for_tests(registry: list[dict], acct: V64CallAccounting) -> list[dict]:
    import numpy as np
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    try:
        registry = _preflight(output_root)
    ...
    try:
        ...
        if records:
            with (output_root / "v64_records.csv").open("w", newline="", encoding="utf-8") as f:
                ...
                w.writeheader()
                w.writerows(records)
        # build frozen summary (exact_u1/l2/full overall+per-source, dual accepted/undetected, 19/24 6/8 gates, 5-state terminal, stage/calls/leak/runtime, L2 vs full diff, intercepted)
        from comparison_bench.formal_ir.v64_full_symbol_verification import build_v64_summary_payload
        summary = build_v64_summary_payload(records, registry, acct, t0, args.authorized_target_sha)
        (output_root / "v64_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    except BaseException as exc:
        # retain partial records on interrupt, no summary
        try:
            (output_root / "v64_records.json").write_text(json.dumps(records, indent=2), encoding="utf-8")
        except Exception:
            pass
        (output_root / "v64_interrupted.json").write_text(json.dumps({"interrupted": True, "error": f"{type(exc).__name__}: {exc}", "total_calls": acct.completed, "records_written": len(records)} , indent=2), encoding="utf-8")
