"""S1 SHIFT m100 thin runner — paired shift-prior diagnostic (DECIDE-diagnostic).

Frozen scope (main-thread, do not redesign): single arm shift-prior /
...
modelF/90/m100 + R20 uniform/90/m100 trajectories — byte-identical check,
zero new calls).
...
- P-dec: SHIFT prior from CAL TRAIN counts_ab
  (workspace/v72p2d5_model_f_input/20260907_r1/model_f_input.npz,
  counts_ab only) via build_shift_prior_concentration with frozen eps
  1e-4 (GATE A CV: best within 0.05 bits of 1e-4 -> freeze 1e-4);
  cold row-layered 90/1.0 single-pass, disclosure_i=5*rows+64=564,
  control=interaction=auth=0. Shift-only single mode (no prior-mode flag).
...
- P-tests: delta FAKE-fixture tests only (see tests/test_g6s1_shift.py).
...
adapters/verifier helpers/marginal prior; D10-R2 builder/admission/refusal;
D16/v35 decoder kernel (lazy bind, no duplicate); shift build by import
from the additive shift module (no copied kernels, no legacy-constant
reuse on this path).

Forbidden: any prior-mode flag; Model-F table reuse on the shift path;
create the UUID root before grant; edit frozen runners/tests; touch
src/experiments/tools/results/outputs_comparison/prior roots/R9/R20 roots;
commit/push.
"""
...
FROZEN_PRIOR_ROOT = r9.FROZEN_PRIOR_ROOT  # counts_ab source (CAL TRAIN)
FROZEN_EPS = float(DEFAULT_EPS)  # GATE A freeze: 1e-4 (best within 0.05 bits)
PRIOR_MODE = "shift-delta-eps1e-04"
FUTURE_ROOT_UUID = "4105cfc2-3bbb-444a-9283-31403d43b7ff"
FUTURE_ROOT = "workspace/g6s1_shift_" + FUTURE_ROOT_UUID
SCI_CEILING = r9.SCI_CEILING
SETUP_CEILING = 8
EVIDENCE_FILES = tuple(r9.EVIDENCE_FILES)
BLOCK_COLUMNS = tuple(r9.BLOCK_COLUMNS)
FROZEN_COMMAND = (
    ".venv/bin/python scripts/g6s1_shift.py --execute-real "
...
def run_authorized_batch(out_root: str, registry: str, session_id: str,
    ...
    """Execute frozen 128-call shift matrix; returns bundle (writes separately)."""
    ...
    manifest = {
        "schema": "g6s1_shift_manifest_v1", "change_id": CHANGE_ID,
    ...
    summary = {
        "schema": "g6s1_shift_summary_v1", "terminal": terminal,
...
def verify_root(out_root: str) -> bool:
    ...
    if len(rows) not in (128,):
        ...
        # because exact/syndrome/tag isolation requires full 128 for S1).
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="S1 SHIFT m100 thin runner")
...
def main(argv=None, *, adapters_override=None) -> int:
    ...
    print("S1 terminal=%s attempted=%d exact=%d accepted=%d undetected=%d"
