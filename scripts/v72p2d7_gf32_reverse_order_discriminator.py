"""D7-F paired reverse-order discriminator runner (frozen R1; gate only).
...
module by file path.  Reads only the D7-F authorization state file; refuses
before any work/Model-F read/decoder bind/root creation while
``d7f_execution_authorized`` is false.  ``--dry-run`` prints the frozen
128-slot matrix; ``--verify <root>`` performs the read-only seven-file
...
CORE_PATH = (ROOT / "comparison_bench" / "src" / "comparison_bench"
             / "formal_ir" / "v72p2d7_gf32_reverse_order_discriminator.py")
STATE_PATH = (ROOT / "docs" / "research_cycles"
              / "V72P2D7-GF32-REVERSE-ORDER-DISCRIMINATOR" / "cycle_state.yaml")

_SPEC = importlib.util.spec_from_file_location("d7f_reverse_order_discriminator_core",
                                               str(CORE_PATH))
...
def build_parser():
    ap = argparse.ArgumentParser(
        description="D7-F paired reverse-order cross-layer discriminator "
                    "(authorization-gated).")
    ...
    ap.add_argument("--dry-run", action="store_true",
                    help="print the frozen 128-slot matrix only")
...
def main(argv=None):
    ...
    if args.dry_run:
        ...
        for slot in slots:
            print("%d %s %d %s %s %s %s %d" % (
                ...
                slot["arm"], slot["role"], slot["condition"],
    ...
    if not _mod.is_authorized(state):
        print("D7-F execution is not authorized; refusing before any work")
        return 3
    ...
    if not _mod.model_f_root_matches(args.model_f_root, repo_root=ROOT):
        print("D7-F refused: --model-f-root must equal %s" % (_mod.MODEL_F_ROOT,))
        return 3
    try:
        result = _mod.run_reverse_order_discriminator(
