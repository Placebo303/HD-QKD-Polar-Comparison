"""D7-D schedule-discriminator runner (frozen R1; gate only, no execution).
...
module by file path.  Reads only the D7-D authorization state file; refuses
before any work/Model-F read/decoder bind/root creation while
``d7d_execution_authorized`` is false.  ``--dry-run`` prints the frozen
256-call matrix; ``--verify <root>`` performs the read-only seven-file
recomputation.  The frozen future command is not run in this task.
"""
...
CORE_PATH = (ROOT / "comparison_bench" / "src" / "comparison_bench"
             / "formal_ir" / "v72p2d7_gf32_schedule_discriminator.py")
STATE_PATH = (ROOT / "docs" / "research_cycles"
              / "V72P2D7-GF32-SCHEDULE-DISCRIMINATOR" / "cycle_state.yaml")

_SPEC = importlib.util.spec_from_file_location("d7d_schedule_discriminator_core",
                                               str(CORE_PATH))
...
def build_parser():
    ap = argparse.ArgumentParser(
        description="D7-D flooding-vs-row-layered schedule discriminator "
                    "(authorization-gated).")
    ...
    ap.add_argument("--dry-run", action="store_true",
                    help="print the frozen 256-call matrix only")
...
def main(argv=None):
    ...
    if args.dry_run:
        calls = _mod.frozen_call_matrix()
        print("calls=%d schedules=%r budget=%d" % (
            len(calls), list(_mod.SCHEDULES), _mod.MAX_CALLS))
        for call in calls:
            print("%d %d %s %s %d %s %s %d" % (
                call["call_idx"], call["identity_idx"], call["schedule"],
                call["condition"], call["seed"], call["f"], call["layer"],
                call["rows"]))
        return 0
    ...
    if not _mod.is_authorized(state):
        print("D7-D execution is not authorized; refusing before any work")
        return 3
    ...
    if not _mod.model_f_root_matches(args.model_f_root, repo_root=ROOT):
        print("D7-D refused: --model-f-root must equal %s" % (_mod.MODEL_F_ROOT,))
        return 3
    try:
        result = _mod.run_schedule_discriminator(
